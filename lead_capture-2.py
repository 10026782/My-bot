...
def capture_inbound_lead(identity, message: str) -> None:
    ...
    try:
        ...
        # display_name="" (identity.py) → טלפון נכתב כ-Name (Primary Field).
        ...
        # BUG FIX: airtable_add מחזיר dict (C53-A contract), לא string.
        # הקוד הישן: re.search(result) → TypeError: expected string, got dict
        # הקוד הישן: "✅" in result → בדק keys של dict, לא values → תמיד False
        # כתוצאה: lead_id="unknown", scoring לא רץ, score נשאר ריק (/100)
        result = airtable_add(Tables.LEADS, fields)
        lead_ok = isinstance(result, dict) and result.get("ok") is True
        lead_id = result.get("external_id", "") if isinstance(result, dict) else ""
        if not lead_id:
            lead_id = "unknown"
