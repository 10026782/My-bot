...
- A few modules are code-complete but **not currently imported by anything in the live pipeline** (verify with grep before assuming they're wired in): `profile.py` (Airtable-backed long-term user profile), `project_timeline.py` (Airtable timeline table generator, has its own CLI), `tenant_provisioner.py` (F08 `MULTITENANT` provisioning), `creative_generator.py` (`CREATIVE_GENERATOR` flag), and `knowledge_engine.py`/root-level `router.py` (Supabase-backed dynamic context, `KNOWLEDGE_ENGINE` flag). The former F13 `TenantConfig` module was removed; the remaining `providers/` shims are still not imported by the live pipeline and must not be wired without resolving the F12/F13 design decision.
