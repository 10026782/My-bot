...
- `scripts/classify_contacts_for_airtable.py`: single-file CSV/VCF classifier built on `contact_merge.py`'s parsing (imports its functions rather than duplicating them) — assigns `Type`/`Role Category` by keyword match on name, tags rows with `--owner`/`--referred-by`, writes a classified CSV + report to `--outdir`. `Owner`/`Referred By` are output-only columns, not live Airtable `Contacts` fields yet.
