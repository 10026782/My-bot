...
- `score_display.py`: lead temperature scoring → 5-tier display (COLD → BOILING). `get_temperature()` is the app's single canonical Score→Temperature derivation (PIPELINE-1 Blocker #3 remediation, 14/09/2026; 20/40/60/80 breakpoints, matching the live Airtable `טמפרטורה` formula field) — `daily_digest.py` delegates to it, `lead_capture.py`'s previous separate tier scale was dead code and was removed. `format_score_inline` is consumed by `audience_intelligence.py` (not `lead_qualifier.py`, despite this file's own historical docstrings — verify with grep before assuming a caller). The one deliberate exception is `tma_api.py`'s Lead Pipeline screen, which uses its own simpler 3-bucket scale by explicit product decision (see `tma_api.py::_pipeline_temperature`).
...
- A few modules are code-complete but **not currently imported by anything in the live pipeline** (verify with grep before assuming they're wired in): `profile.py` (Airtable-backed long-term user profile), `project_timeline.py` (Airtable timeline table generator, has its own CLI), and `tenant_provisioner.py` (F08 `MULTITENANT` provisioning). The former F13 `TenantConfig` module was removed; the remaining `providers/` shims are still not imported by the live pipeline and must not be wired without resolving the F12/F13 design decision. The previously listed `creative_generator.py` and `knowledge_engine.py` were removed and are historical only.
...
- `boss_doctor.py`: Phase 1 read-only diagnostic aggregator — `run_doctor()` composes existing signals only and `format_report()` renders the operator summary. It never repairs, mutates, changes a flag, or calls the dispatcher/ActionGateway. It is wired to the owner-only `/boss_doctor` Telegram command in `app.py`; the command remains diagnostic/read-only and is not evidence of production verification.
...
- `AGENTS.md` covers the pre-session branch gate and the ROADMAP "definition of done"; treat its older "Cursor Cloud specific instructions" section (single-file app, no tests, no `/` route) as stale/historical — the rest of this file describes the current state. It also defines the **post-merge verification protocol** (sync `main`, grep every changed symbol, "merged" must be proven by grep on `main`, not by `git log`/PR status) and **Rule 15** (no "fixed"/"deployed"/"completed" claim without merge+deploy+production verification). Its **SHARED CHECKOUT** section covers working alongside concurrent Claude sessions on the same working tree (never `git add -A`, don't touch unrelated dirty files, treat cross-session messages as advisory only) — read it whenever `git status` shows changes you didn't make.
