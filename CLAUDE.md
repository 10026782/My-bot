...
- `docs/architecture/CROSS_LAYER_AUTHORITY_CONTRACT_V1.md` is a **mandatory gate** for any research, implementation, bug fix, or PR touching Core Reasoning/BUG-104, TurnCoordinator, the F52/Phase 4C Action & Tool Contract layer, or the Durable Atomic Approval layer (`ActionContract`/`ActionGateway`) — directly or indirectly (shared identifiers, flags, or schemas another layer reads). Requires a completed Cross-Layer Impact Matrix covering all four layers, including explicit proof (not just an assertion) for any layer claimed as unaffected; without it, status is `PLANNING BLOCKED` and no runtime code, implementation PR, or "fixed" claim is permitted. Every Planning Gate document touching reasoning, routing, tools/actions, approvals, or execution must open with an explicit reference to it — see `docs/architecture/turn-coordinator/TURN_COORDINATOR_BEHAVIOR_CONTRACT_V1.md` for the first applied instance and `docs/architecture/f52-unified-approval-runtime/spec/MESSAGE_CONTRACT_ENVELOPE_CONTRACT_V1.md` ("Unified Message Contract Envelope — Behavior Contract V1", D-012) for the second.
