...
- `core/usage_telemetry.py`: single durable recording point (PostgreSQL `usage_events`, see `core/migrations/002_usage_events.sql`) for every paid AI provider call — Anthropic text, OpenAI text fallback, OpenAI Whisper STT. `core/model_pricing.py` holds the canonical `(provider, service, model)` → price table (exact match only; an unrecognized model logs loudly and uses a documented fail-safe price instead of guessing silently). `core/cost_watchdog.py::daily_watchdog()` (08:15) computes the day's real cost from durable data, projects it into `AI_Usage_Daily` (a projection, not a source of truth), and auto-triggers `EMERGENCY_STOP_AI` if `COST_DAILY_LIMIT` is exceeded; `cost_monitor.py` at the repo root does the hourly $ alert only (same durable data, no stop).
