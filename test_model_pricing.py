...
This is a NEW pricing table for the new usage_events path — it does not
replace cost_monitor.py's own _PRICE table (which still drives the
EMERGENCY_STOP_AI trigger, unchanged in this PR). Still, this table has
the same failure mode to avoid: cost_monitor.py's table keys Haiku as
"claude-haiku-4-5", a string nothing in the codebase actually sends (every
real call site sends "claude-haiku-4-5-20251001") — this module keys on
the real strings instead, and treats any other mismatch as loud, not
silent.
