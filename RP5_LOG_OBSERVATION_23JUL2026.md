...
Both remain **registered, not verified against today's logs, not fixed** — same status as in `BUG_AUDIT_LOG.md`. Listed here so this report doesn't silently omit them from the day's picture. **Update below (§6): BUG-137 was independently confirmed via Airtable, not logs.**

## 6. Airtable ground-truth cross-reference — what logs alone couldn't show

Following up on the "what's missing from logs-only analysis" question: Render app logs are deliberately scrubbed of message text/PII (by design — `core/turn_envelope.py`'s own docstring, reinforced by this session's own BUG-139 filing which also removed `reply=`/`text=` from a log line). Pulling the matching `ActionContracts`/`Approvals`/`Leads` records directly from Airtable (base `app4bcgoX7t0HUVnm`, via Airtable MCP) for the same session closed several gaps §3-§5 above left open:

- **BUG-134 — now confirmed with dated evidence, not just log-inferred.** The `Approvals` table has only 4 rows total; 3 are the exact contracts flagged getting 404/409 in production logs today, still `pending`/`ממתין`: `recyoMWRE2Lv8Fzvk` (created 20/07, 4 days stuck), `recnFF6VCBVcR8apL` (19/07, 5 days), `rec9VBFoLUoEX71bD` (09/07, 2 weeks). See `BUG_AUDIT_LOG.md`'s BUG-134 entry for the full addendum.
- **BUG-137 — confirmed via Airtable where the Hebrew-marker log search failed (§5).** Contract `recoeWLkqGLxDxnMs` (the exact record ID in BUG-137's original citation) got another `domain:"finance"` update today at 19:20:35Z. The record's own stored `domain` field is `"finance"` — a real clue (not proof) that the leaked domain value traces to the matched Lead record's existing field, not `identity.domain_id`. See `BUG_AUDIT_LOG.md`'s BUG-137 addendum.
- **BUG-138 — the exact batch identified.** The 4-task batch in §3 includes `recE0N2reMAt6TVe7`: **"להכין מודעה ולפרסם בפייסבוק ובוואטסאפ"** — a verbatim match to BUG-138's original report text. BUG-138's stuck-button complaint happened inside this exact batch (same session as Findings #8/#9), though causal connection isn't established.
- **New: BUG-140, filed.** A different contract in the same session (`0e8a155c-...`, still `pending`) shows the user explicitly asking to add a *new* lead ("דנה כהן") but the system generated an `airtable_update` against a completely unrelated existing lead ("ישראל כהן") that happens to share the same phone number — the exact phone-collision risk BUG-130 flagged as unconfirmed. Contract not yet approved — no data damage yet, but it's live.

This is the concrete answer to "what would improve the log-only workflow": pair every log-based timing/count analysis with a direct Airtable pull for the structured entities involved (`ActionContracts`/`Approvals`/`Leads`) — Airtable MCP access makes this a same-session addition, not a separate ask to the owner.
...
2. **Filed as BUG-140** (`BUG_AUDIT_LOG.md`) — the phone-collision misrouting (§6). Contract `0e8a155c-...` is still `pending` — recommend the owner reject it manually before it can be accidentally approved.
3. Pull a wider staging window (multiple days) filtered to `no_evidence.*response_claim=failure` to see if the ~33% rate holds or was specific to today's session.
4. RP5 9-state coverage (§1) is meaningfully further along than the production-only view suggested (5/9 states with real staging data vs. 0/9 from production alone) — worth reflecting in `RP5_PREFLIGHT_BLOCKER.md` §3's tracking the next time that document is updated.
5. BUG-138 still needs a real confirmed-callback reproduction — today's data doesn't provide one on either service. BUG-136 (§5) still needs a working non-ASCII marker export — worth fixing the exporter's Hebrew-marker/curl issue if it comes up again.
