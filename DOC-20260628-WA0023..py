...
def run_agent(
    ...
    _out_domain:         list | None = None,   # CXX: mutable container לdomain הסופי אחרי Router
) -> str:
    ...
    # CXX: RequestContext — נבנה פעם אחת, מועבר לכל השכבות.
    # מונע: Sessions נקרא 4 פעמים, domain drift, double identity lookup.
    try:
        from core.request_context import RequestContext
        _req_ctx = RequestContext.from_identity(identity, domain=domain_from_channel)
    except Exception:
        _req_ctx = None  # fallback — לא שובר אם core חסר
    ...
    if identity.role == Role.LEAD:
        try:
            ...
            _lc_result = capture_inbound_lead(identity, user_text)
            # CXX: עדכן lead_record_id ב-RequestContext אם נוצר
            if _req_ctx and _lc_result and getattr(_lc_result, "record_id", ""):
                _req_ctx.lead_record_id = _lc_result.record_id
        except Exception as e:
    ...
    # CXX: domain_drift fix — עדכן RequestContext ו-_out_domain לאחר Router
    if _req_ctx and route.domain:
        _req_ctx.update_domain(route.domain)
    if _out_domain is not None and route.domain:
        _out_domain[0] = route.domain
...
def _webhook_whatsapp_impl():
    ...
    _out_domain = [domain_from_channel]  # CXX: mutable — run_agent יעדכן לdomain אמיתי
    ...
    # CXX: domain_drift fix — _out_domain[0] = domain שנפתר ע"י Router+Context
    final_domain = _out_domain[0]
