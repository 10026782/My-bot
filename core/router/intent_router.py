...
_RULES: list[tuple[str, str, float]] = [
    ...
    (r"(סגור|סיים|השלם|סמן.*סיים|complete).*(משימ|טאסק|task)", Intent.COMPLETE_TASK, 0.95),
    ...
    # אחרי היעד, לא לפניו) לא נתפס ע"י התבנית verb-then-target הרגילה למעלה.
    # תבנית נפרדת, ממוקדת ל-"סמן"/"mark" בלבד (לא כל טקסט שמזכיר משימה+בוצע,
    # כדי לא להתנגש עם LIST_TASKS על "אילו משימות כבר בוצעו").
    (r"(סמן|mark).{0,40}(משימ|טאסק|task).{0,20}(כבוצע|בוצעה|done|complete)", Intent.COMPLETE_TASK, 0.95),
