...
_RULES: list[tuple[str, str, float]] = [
    ...
    # BUG-163: "השלם" חסר מקבוצת-הפעלים המקורית — נופל ל-UNKNOWN. נוסף כאן,
    # לפני קבוצת-הפעלים→יעד הרגילה, כי "השלם את המשימה" כבר תואם את הסדר
    # verb-then-target הקיים (אין צורך בשינוי מבני).
    (r"(סגור|סיים|השלם|סמן.*סיים|\bcomplete\b).*(משימ|טאסק|task)", Intent.COMPLETE_TASK, 0.95),
    # BUG-163: ניסוח יומיומי "סמן משימה X כבוצע/כבוצעה" — סדר המילים ההפוך
    # (הפועל "סמן" מקדים את שם-המשימה, אבל מילת-ההשלמה "כבוצע/כבוצעה" מגיעה
    # אחרי היעד, לא לפניו) לא נתפס באמצעות התבנית verb-then-target הרגילה למעלה.
    # תבנית נפרדת, ממוקדת ל-"סמן"/"mark" בלבד (לא כל טקסט שמזכיר משימה+בוצע,
    # כדי לא להתנגש עם LIST_TASKS על "אילו משימות כבר בוצעו").
    # BUG-163 addendum (code review, 07/08/2026): "done"/"complete" ללא \b
    # תפסו גם כתת-מחרוזת בתוך "incomplete", ולא היה guard לשלילה
    # ("do not mark task X done") — התבנית התעלמה מ"not"/"don't"/"never"
    # שמקדימים את "mark"/"סמן" ישירות. הוספו \b לשתי המילים ו-negative
    # lookbehind ל-4 מילות-שלילה נפוצות (אנגלית + עברית) שמקדימות ישירות
    # את "mark"/"סמן". לא מכסה נטיות פועל עבריות עם קידומת ("תסמן") — מחוץ
    # לסקופ הממצא המקורי.
    (r"(?<!not )(?<!don't )(?<!never )(?<!לא )(?<!אל )(סמן|mark).{0,40}(משימ|טאסק|task).{0,20}(כבוצע|בוצעה|\bdone\b|\bcomplete\b)", Intent.COMPLETE_TASK, 0.95),
