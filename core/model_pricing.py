# core/model_pricing.py — Cost Telemetry Reliability, Part D
#
...
#
# The previous table (cost_monitor.py, now retired) keyed Haiku as
# "claude-haiku-4-5" — a string nothing in the codebase ever actually
# sends (every real call site sends "claude-haiku-4-5-20251001") — so
# every Haiku call silently fell back to the most-expensive-model price.
# That was a 5x overcount on the model that carries the overwhelming
# majority of traffic (see context.py::_select_model). Silent fallback is
# the failure mode this module exists to remove: an unrecognized
# provider/service/model logs loudly and is counted, instead of quietly
# guessing.
#
...
# the silent dict.get(..., default) fallback this module replaces.
