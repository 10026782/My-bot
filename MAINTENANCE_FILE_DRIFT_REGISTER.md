...
| I3 | Provider-specific column reused generically | `"Telegram File ID"` still holds Telegram file_id OR Twilio MessageSid OR Meta message id OR TMA sha256 pseudo-id (`media_gateway.py:64`; `app.py:6459-6463,6748-6752`; `media_handler.py:574,616,658`) | `Logical Media Key` exists and is used for provider-neutral identity/idempotency (`airtable_schema.py:636`; `media_gateway.py:72`), but it does not replace the legacy Telegram-named storage column; I3 remains active, not deferred | HIGH (identity ambiguity) | #3 Data Contract (routed from #13) |
...
| #3 Data Contract | **OPEN — CURRENT DATA CONTRACT GAP**; 5 original findings CLOSED (#1, #3, #4, #5, #6); #2 **DEFERRED — LIVE/SCHEMA CONTRACT DECISION**; I3 **HIGH CURRENT GAP** (active, not deferred) |
