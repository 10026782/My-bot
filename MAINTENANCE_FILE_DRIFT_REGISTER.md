...
| `knowledge_engine.py` (root) | 0 importers repo-wide (only a comment at `tools/audit_result_parsing.py:15`); imports root `router.py` | LEGACY UNWIRED / ORPHAN SUSPECT; C95A recommended formal retirement ("C98"), superseded functionally by `core_knowledge.py` + `cmd_update.get_recent_business_context()` | OPEN (retirement decision) → **RESOLVED / #12 CLOSED** (module removed; decision moot) | #12 | REMOVED (deletion commit `b393313`) — **#12 CLOSED**. |
...
| I8 | COST_WATCHDOG dual override names | `COST_WATCHDOG_LIVE` vs direct-env `COST_WATCHDOG_ENABLED` overriding it (`core/cost_watchdog.py:74-79`; M01 READ_PATH_DRIFT/MEDIUM) | Intentional two-layer split for now; precedence documented; consolidation deferred | MED | #19/#23 |
...
| # | Finding | Current state / evidence | Mark | Owner | Track 8 disposition (30/08/2026) | Track 8B live disposition (30/08/2026) | Track 8C disposition (30/08/2026) |
|---|---|---|---|---|---|---|---|
| K1 | `TurnEnvelope.turn_id` nullable (`str \| None`) until TurnCoordinator exists; ownership fields caller-supplied | Documented in envelope foundation spec + contract V1 + decision log + multilayer plan (MESSAGE_CONTRACT_ENVELOPE_FOUNDATION_SPEC.md:21; CONTRACT_V1.md:69,177,225-232) | NEEDS_DATA_CONTRACT_AUDIT | #3 | Out of Track 8 scope (TurnEnvelope, not a Deal/Payment/Lead/Task/Session/Approval entity) — not re-verified, unchanged | Not investigated by Track 8B (TurnEnvelope, out of live-verification scope). | Out of Track 8C scope (TurnEnvelope, not a schema/cache-code item) — not investigated. |
| K2 | `resolve_identity()` None-assumption mismatch (never returns None vs documented hard-fail) | C05-C07 F8, deferred (= R-C06-8) | NEEDS_DATA_CONTRACT_AUDIT | #3/#15 | Out of Track 8 scope (identity, not a schema/table contract) — not re-verified, unchanged | Not investigated by Track 8B (identity, out of live-verification scope). | Out of Track 8C scope (identity, not a schema/cache-code item) — not investigated. |
| K3 | Select-option whitespace baked into live schema | LeadOutcome trailing spaces on 7/8 options; renamed trimmed 22/08; read paths keep permanent `.strip().lower()` hardening (`airtable_schema.py:361-380`, `core/adapters/leads_adapter.py:260-262`) | NEEDS_DEDICATED_SCHEMA_AUDIT (verify other tables' option strings) | #2 | Reconfirmed only, not closed: LeadOutcome hardening unchanged on current main. Track 8 additionally confirmed the *general mechanism* — `schema_cache.json` is name-only (no select-choice data at all; see `feature_flags.get_select_value_validation_state()` docstring, "name_only" seed mode always skips value validation by design) — so "verify other tables' option strings" still requires live access. Static portion re-verified; live portion (Deal/Payment Term enum option strings included) still open under #2 | **Live-verified for the `Leads.Business Outcome` field specifically**: `get_table_schema` on live field `fldVa5wSmAqcKLi86` confirms all 8 options exact-match `LeadOutcome`'s trimmed values, zero trailing spaces — the 22/08/2026 rename is live and complete for this field. The broader "verify other tables' option strings" claim was only spot-checked for the Deal/Payment/Payment-Term enum surfaces (`DealStage`, `PaymentStatus`, `VATRule`, `PaymentTermCalcType/Basis/Trigger/Cadence`, `LeadStatus`, `DealFields.PRIORITY`, `RiskLevel` — all exact live matches except `PaymentStatus.CANCELLED`, see new finding N1 in the Track 8B report), not exhaustively for every select field in the base. See `docs/governance/TRACK_8B_LIVE_SCHEMA_RECONCILIATION_30082026.md` §3-4. | Not touched by Track 8C — this slice regenerated `schema_cache.json` (field names only, still no select-choice data by design) and fixed the one enum-value drift it was scoped to (`PaymentStatus.CANCELLED`, see N1 below); the broader "verify other tables' option strings" claim is unchanged, still requires live `get_table_schema` spot-checks. |
| K4 | Checkbox-absent-means-False contract | EmergencyStopStore `Enabled` checkbox omitted by Airtable when unchecked; readers must default missing→False; duplicate Flag Names undetectable by type system (`airtable_schema.py:1495-1508`) | NEEDS_DATA_CONTRACT_AUDIT | #3 | Out of Track 8 scope (Emergency Stop Flags, not in the prioritized entity list) — not re-verified, unchanged | Not investigated by Track 8B (Emergency Stop Flags table confirmed to exist live with its expected 7 fields as part of F7's resolution, but the checkbox-absent-means-False contract itself was not re-tested). | Out of Track 8C scope (Emergency Stop Flags) — not investigated. |
| K5 | PR #851 tenant_config deletion residue in docs/comments | Active doc references in CLAUDE/ROADMAP corrected by PR #896; `CURRENT_STATE_MAP.md:197`, CHANGELOG, BUG_AUDIT_LOG, master plan, POST_N15 survey, and `memory_store.py:15` comment remain historical/source-comment evidence outside this scope | **RESOLVED ACTIVE DOC SUBSET; HISTORICAL/CROSS-TRACK RESIDUE PRESERVED** | #19/#20 | Out of Track 8 scope — not re-verified, unchanged | Not investigated by Track 8B (documentation residue, out of live-verification scope). | Out of Track 8C scope (doc residue) — not investigated. |
| K6 | Media Files identity columns write-only | `DRIVE_FILE_ID` (:635) and `TELEGRAM_FILE_ID` (:642): sole writers `media_gateway.py:41,48` (+ transcript writer :214 status); ZERO lookup-by-ID readers anywhere; no Drive-file-id retrieval either | NEEDS_DEDICATED_SCHEMA_AUDIT | #2 | Out of Track 8 scope (Media Files was not in the prioritized entity list: Leads/Contacts/Deals/Payments/Payment Terms/Tasks/Sessions/Lead Events/Approvals) — not re-verified, remains open | Not investigated by Track 8B (Media Files field-existence confirmed live as part of the general table pass, but the write-only/no-reader claim itself was not re-tested). | Out of Track 8C scope (Media Files) — not investigated. |
| K7 | LeadSessions persistence | Original F4 gap (lead_draft in-memory only) FIXED (PR #845 `f2030b0`; persist+restore+round-trip self-test `session_store.py:519,693,887-890`). Residual: entire session state is one schema-less `State JSON` blob; keys evolve without versioning | Fixed; residual NEEDS_DATA_CONTRACT_AUDIT | #3 | Adjacent finding, not the same one: Track 8 independently found (its own Finding F7) that the live `Sessions` table itself (`Tables.SESSIONS`, distinct from the deprecated/never-created `LeadSessions`) has zero field coverage in `schema_cache.json`, confirmed pre-existing via `git show 28a44c0:schema_cache.json`. K7's schema-less-JSON-blob-versioning claim itself was not re-examined — stays open under #3 | Not investigated by Track 8B directly; adjacent confirmation only — the live `Sessions` table (18/18 fields, exact match to `SessionsFields`) is confirmed to exist and is fully populated, resolving F7's Sessions item, but K7's `State JSON` blob versioning claim was not re-examined. | Not touched by Track 8C — unrelated to this slice's Payment/Contact/cache-cleanup scope. |
| K8 | Structured-result contract complete; FileUploadResult.file_id dual-table overloading | Media pipeline fully structured today (`MediaError/MediaResult` dataclasses; all 3 entry points return MediaResult; rendering centralized :156-160). Overloading CONFIRMED: file_id = Media Files rec ID when type="drive_file" vs Decision Inbox rec ID when "inbox_file" (`session_store.py:102-115`); `_sync_to_db` writes LINKED_MEDIA_FILE only for drive_file to dodge cross-table INVALID_RECORD_ID (:536-540) | Overloading NEEDS_DATA_CONTRACT_AUDIT | #3 | Out of Track 8 scope (Media Files) — not re-verified, unchanged | Not investigated by Track 8B (Media Files dual-table overloading, out of live-verification scope). | Out of Track 8C scope (Media Files) — not investigated. |
| K9 | Contract field present only on some paths | `"Status"` hardcoded literal `_MEDIA_FILES_STATUS_FIELD` in media_handler (:200-203, Voice Inbox rows get Status=pending) — ABSENT from `MediaFileFields` constants (:623-647) and absent from media_gateway's AssetRecord mapping → gateway validation blind to the field | NEEDS_DEDICATED_SCHEMA_AUDIT | #2 | Out of Track 8 scope (Media Files) — not re-verified, remains open | Not investigated by Track 8B (Media Files `Status` field contract, out of live-verification scope). | Out of Track 8C scope (Media Files) — not investigated. |
| K10 | Owner-field semantic fragmentation | see I2 (12 constants, per-table semantics inline-commented only) | NEEDS_DEDICATED_SCHEMA_AUDIT | #13/#2 | Reconfirmed only, not closed: fragmentation pattern still present on current main. Track 8 found one additional confirmed instance introduced by the 30/08/2026 Canonical Deal/Payment Architecture track: `PaymentFields.OWNER = "owner"` (lowercase) vs `DealFields.OWNER = "Owner"` (capitalized) — both correct against their respective live-cached tables individually, but the naming inconsistency itself is not resolved. Stays open under #13/#2 | **Narrowed and reconfirmed live**: `get_table_schema` `linkedTableId` checked on Leads.Owner, Deals.Owner, Payments.owner (and consistent `Role (from Owner)` lookup naming implies the same for Assets/Tasks/Interaction Log/Contacts/Business Memory/Expenses) — every Owner-link field, regardless of `Owner`/`owner` casing, resolves to the same live Profile table (`tblwf2K8WZrdu8xI5`). Live-confirmed: this is purely a naming-casing inconsistency with zero relationship-integrity risk, not a semantic fragmentation. See `docs/governance/TRACK_8B_LIVE_SCHEMA_RECONCILIATION_30082026.md` §5. | **K10 itself NOT remediated** — Track 8C removed a *different*, adjacent stale entry (`tools/dispatcher.py`'s `_ALIAS_MAP["Payments"] = "תשלומים (Payments)"`, a table-*name* alias pointing at a table Track 8B confirmed no longer exists live; traced to confirm zero live write-path impact before removing) as part of its dead-alias cleanup. That is not K10's finding — K10 is about `Owner`/`owner` **field-name casing** fragmentation across tables, which Track 8C did not touch (out of this slice's declared scope). K10 remains open exactly as Track 8B left it. |

Full Track 8 findings (F1–F9) and terminal status: see `docs/governance/MAINTENANCE_AUDIT_LEDGER.md` §"Track 8 — Schema / Data-Contract Reconciliation".
