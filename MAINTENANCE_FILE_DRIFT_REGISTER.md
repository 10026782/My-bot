...
| I3 | Provider-specific column reused generically | `"Telegram File ID"` holds Telegram file_id OR Twilio MessageSid OR Meta message id OR TMA sha256 pseudo-id (`media_gateway.py:47-48`; `app.py:5974,6438-6442,6720-6730`; `media_handler.py:537-545`) | None (no neutral key in ingestion path) | HIGH (identity ambiguity) | #13/#3 |
...
| #9 Mock Fidelity | **CONSUMED, Audit #9 25/08/2026**: 4 findings (#9-1 HIGH, #9-2 MEDIUM-latent, #9-3/#9-4 LOW) — see `BUG_AUDIT_LOG.md` ("Audit #9 — Mock Fidelity (Phase 1, Read-Only)") and `docs/governance/HORIZON.md` §OPEN. #9 itself is **OPEN — CURRENT MOCK FIDELITY GAPS (25/08/2026)** — documentation-capture only, no remediation performed. |
