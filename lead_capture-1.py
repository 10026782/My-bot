...
def capture_inbound_lead(identity, message: str) -> None:
    ...
    try:
        ...
        # ROOT CAUSE FIX: display_name='ליד חדש' גרם ל-Name='ליד חדש' בכל ליד.
        # Name הוא Primary Field ב-Airtable — מופיע בכל Linked Record בכל טבלה.
        # לכן 'ליד חדש' נראה כאילו מופיע בכל השדות (visual corruption בלבד).
        # FIX: display_name='' ב-identity.py → כאן נכתב הטלפון כ-Name.
        _lead_name = identity.display_name or identity.external_id or "unknown"
        fields = {
            LeadFields.NAME:       _lead_name,               # Primary Field — טלפון אם אין שם
            LeadFields.PHONE:      identity.external_id,     # phone בלבד
