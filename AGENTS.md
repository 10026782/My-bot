...
- הערה: במקום להריץ ידנית `git branch -a --no-merged main` + `git log main..{branch}`, השתמש ב־`daily_git_audit.py` כדי להפיק דוח ענפים מפורט (ה־scheduler כרגע כבוי). זה מריץ את אותו logic ומוסיף בדיקות מול `ROADMAP.md` ו gates נוספים.
