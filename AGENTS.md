...
```
ANTHROPIC_API_KEY=<key> TELEGRAM_TOKEN=<token> python3 bot.py
```

- Flask listens on `0.0.0.0:10000` by default (override with `PORT` env var).
- At startup, `bot.py` attempts to set a Telegram webhook pointing to the hardcoded Render URL. This call will fail in local dev (expected; caught by try/except). The Flask server still starts normally.
...
- **No linter config**: No `pyproject.toml`, `setup.cfg`, or linter configuration is present. If needed, run `python3 -m py_compile bot.py` to check for syntax errors.
- **Module-level side effects**: `bot.py` creates the `TeleBot` instance and calls `bot.set_webhook()` at module load time (lines 11, 23-24). Any import of `bot.py` triggers these calls.
