...
def cmd_done(msg):
    ...
    if not identity or identity.role not in ("owner", "admin"):
        return
    try:
        from tma_api import _at_list, _at_patch, _at_post, record_fields, record_id, relation_payload
        ...
        if coins > 0:
            _at_post(Tables.COINS_LOG, {
                ...
                CoinsLogFields.QUEST:         relation_payload(record_id(quest, required=True)),
...
# TC6 (app.py integrator) — סמן terminal_outcome פנימי-בלבד, נוסף לסמנים
# הקיימים APPROVAL_QUEUE_ERROR / APPROVAL_QUEUE_ORPHANED /
# APPROVAL_QUEUE_NEVER_ATTEMPTED. לעולם לא מעוצב ישירות למשתמש — המפתח
# "message" נושא תמיד את הטקסט הבטוח הקיים בפועל; הסמן הזה נקרא רק
# על-ידי לוגיקת ה-tool-loop (run_agent(), ראו Branch A/Branch B למטה)
...
def _ownership_for_contract_or_none(gateway, contract_id: str | None):
    """TC6 (app.py integrator): גוזרת את ה-ActionLifecycleResult הקנוני לפי
    contract מדויק, עבור contract_id שהקריאה הזו כבר יודעת שהוא אמיתי (הוא
    זה עתה נוצר/נמצא ע"י propose_action() באותה הפונקציה).

    מחזירה ``None`` — לעולם לא ``ActionLifecycleResult`` מומצא — בכל כשל
    קריאה. קוראים חייבים להתייחס ל-``None`` כ"החזר את הטקסט הבטוח
    כבר-מחושב של הקריאה הזו עצמה, מתויג עם
    ``_APPROVAL_OWNERSHIP_VERIFICATION_FAILED``", לעולם לא כרישיון להמציא
    מצב מחזור-חיים/אישור/ביצוע סינתטי, ולעולם לא כרישיון ליפול בשקט
    לבעלות-תשובה של ה-Agent (עקרון-העיצוב של TC6: אין המצאה, אין
    ברירת-מחדל שקטה ל-Agent, אין סמכות בעלות שנייה ב-app.py).
    """
    try:
...
_COMMERCIAL_DRAFT_CREATE_TOOLS = frozenset({"crm_create_payment_term", "crm_create_charge_payment"})
...
def _run_commercial_draft(entity_type: str, tool_name: str, tool_inputs: dict, identity, channel: str, user_chat_id: str) -> "_DealDraftOutcome":
    """BUSINESSDRAFT PHASE 4A (PaymentTerm/Payment) -- the seam body for
    `crm_create_payment_term`/`crm_update_payment_term`/
    `crm_create_charge_payment`/`crm_update_payment`, called once from
    inside `_queue_approval_detailed_impl` before any dedup/fingerprint
    layer or `ActionGateway.propose_action()` sees the payload. Mirrors
    `_run_deal_business_draft`'s Phase 3 CREATE+UPDATE shape exactly
    (BusinessDraft CREATE/UPDATE -> Sessions persistence (CAS) -> confirm()
    -> ConfirmedSnapshot) -- kept as a SEPARATE function rather than a
    generalized Deal helper so the runtime-verified Deal path is never
    touched by this change. Neither Payment Term nor Payment has an owner
    field or a Domain select field, so this omits Deal's owner-resolution
    and domain-canonicalization steps entirely (they do not apply here).

    `crm_create_charge_payment` is `model_exposed=False` -- it is never
    called directly by the Agent tool_use loop. It reaches this seam only
    via CommercialCompletionRouter's `_inspect()`/finalize step (its own
    `self._queue(tool, inputs)` callback resolves to `_queue_approval_
    detailed`, which always calls `_queue_approval_detailed_impl` --
    unconditionally, regardless of whether the call originated from a model
    tool_use event or from the completion router's own internal finalize).
    That is the exact same route Deal CREATE already takes today
    (MUTATION_TOOLS["deal"] == "crm_create_deal", also model-exposed and
    also DIAMOND-reachable) -- this is not a new pattern, just the second
    entity to use it. `crm_create_payment` (the legacy, flat writer) is
    deliberately absent from `_COMMERCIAL_DRAFT_CREATE_TOOLS` below and
    from `commercial_completion_routing.MUTATION_TOOLS` -- it is a
    separately-classified legacy/compatibility surface, untouched, never
    the BusinessDraft "payment" CREATE binding (see
    ENTITY_CONTRACTS["payment"]'s own "unresolved_rules" note and
    test_business_draft_phase4_payment.py's routing-reachability checks).
    """
    from core.business_draft import (
        BusinessDraftError, DraftConflictError, DraftOperation,
        create_draft, fields_from_airtable_record,
    ...
    operation = DraftOperation.CREATE if tool_name in _COMMERCIAL_DRAFT_CREATE_TOOLS else DraftOperation.UPDATE
    ...
    # kwargs today (every Golden-Writer-accepted CREATE/UPDATE key has a
    # completion field equivalent) -- passthrough_kwargs is always empty
    # here, kept only for shape parity with the Deal seam / a future entry.
    if operation is DraftOperation.CREATE:
        mapped, unrecognized, passthrough_kwargs = fields_from_primitive_create(entity_type, tool_inputs)
    else:
        mapped, unrecognized, passthrough_kwargs = fields_from_primitive_update(entity_type, tool_inputs)
    ...
    if operation is DraftOperation.UPDATE:
        ...
        # 4. Pre-read the current record to build original_fields. Same
        # reader Deal's seam uses; authorization for this read is the
        # enforce() role check above (matches Deal's existing posture
        # exactly -- tenant scope is re-checked at actual dispatch time by
        # the dedicated crm_update_payment_term/crm_update_payment
        # dispatcher case, same as Deal's crm_update_deal case).
        ...
        # 5. UPDATE empty-value handling: same fail-closed boundary as
        # Deal's seam -- an explicit clear on a mapped field has no
        # canonical BusinessDraft representation yet.
        cleared_fields = sorted(
            name for name, value in mapped.items() if value in _DEAL_EMPTY_VALUES
        )
        if cleared_fields:
        ...
        # CREATE: empty optional values are simply omitted -- there is no
        # existing value to "clear" (same as Deal's CREATE branch).
        mapped = {k: v for k, v in mapped.items() if v not in _DEAL_EMPTY_VALUES}
...
_COMMERCIAL_DRAFT_TOOLS = (
    "crm_create_payment_term", "crm_update_payment_term",
    "crm_create_charge_payment", "crm_update_payment",
)
        ...
        if not _gw_result.ok:
            ...
            if _generic_lifecycle and _flag("FEATURE_SINGLE_SPEAKER_APPROVAL_UX"):
                ...
                if _generic_action_lifecycle_result is None:
                    # Branch B: אותו כלל fail-closed כמו בענפי
                    # existing_pending_blocks_agent למעלה — קיים contract
                    # מאומת (_generic_found_contract), אך סמכות-הבעלות עצמה
                    # לא ניתנה לאישור. לעולם לא להמציא, לעולם לא ליפול
                    # לבעלות Agent, לעולם לא _orphan_cleanup_failure_
                    # response() (ניסוח שגוי למקרה הזה — ראו ההערה
                    # המפורטת למעלה). משתמשת חוזרת באותו
                    # _generic_lifecycle.safe_user_message כבר-מחושב
                    # וכבר-בטוח.
                    return {
...
def run_agent(
    ...
    try:
        ...
        while True:
            ...
            for tu in ([] if _multi_mutation_mismatch else tool_uses):
                ...
                if meta.requires_approval:
                    ...
                    tool_results_log.append({
                        ...
                        # TC6 (app.py integrator): ה-ActionLifecycleResult
                        # הקנוני, מוקרן לפי contract מדויק — סמכות
                        # בעלות-התשובה של Branch A/Branch B ב-run_agent()
                        # למטה, לא ה-scalar "reply_owner" התאם-לאחור בלבד.
                        "action_lifecycle_result": _approval_outcome.get("action_lifecycle_result"),
            ...
            # TC6 (app.py integrator): Branch A (בעלות Gateway קנונית) ו-
            # Branch B (turn מתואם, בעלות לא-ניתנת-לאימות) נגזרות שתיהן
            # מאותה רשומת tool_results_log מתואמת — לא משני היגדים
            # בלתי-תלויים על אותו log (סוגר את הפער-כפול-סמכות שתועד
            # ב-BUG-162_SINGLE_SPEAKER_CLOSURE_AUDIT_20260807.md §2.4).
            # נוכחות ה-sentinel לבדה לעולם אינה מספיקה — רק
            # action_lifecycle_result.reply_owner=="gateway" האמיתי, המוקרן
            # לפי contract מדויק, קובע.
            _gateway_owned = None
            ...
            if _correlated_approval_entry is not None:
                if _correlated_approval_entry.get("terminal_outcome") == _APPROVAL_OWNERSHIP_VERIFICATION_FAILED:
                    # Branch B: contract אמיתי נגע ב-turn הזה, אך הקרנת
                    # הבעלות שלו לפי contract מדויק לא ניתנה לאישור.
            ...
            if (
                _ownership_verification_failed_entry is not None
                and _flag_enabled("FEATURE_SINGLE_SPEAKER_APPROVAL_UX")
            ):
                # Branch B — עצירת-בטיחות, לא סמכות בעלות-תשובה שנייה. שום
                # ActionLifecycleResult (אמיתי או מומצא) לא עומד מאחורי
                # הענף הזה; שום reply_owner="gateway" לא נטען; ה-Agent
                # לעולם לא מקבל סבב נוסף ב-turn הזה. משקף בדיוק את הצורה
                # של owner_notified/final_response_count של Branch A, כדי
                # שהודעה שכבר נמסרה לעולם לא תוכפל. הערת-תיקון טלמטריה:
        ...
        try:
            from core.turn_envelope import build_ownership_signal, log_ownership_signal
            # TC6 (app.py integrator): reply_owner כאן נגזר מאותה
            # action_lifecycle_result מתואמת-מדויקת שגם Branch A למעלה
            # קורא — לא עוד ניבוי-נוכחות-בלבד עצמאי על אותו
            # tool_results_log (סוגר את הפער-כפול-סמכות שתועד
            # ב-BUG-162_SINGLE_SPEAKER_CLOSURE_AUDIT_20260807.md §2.4).
            # מגיע לכאן רק כשה-early-return של Branch A/Branch B למעלה לא
            # הופעל (הדגל כבוי, או שאף ענף לא תאם) — לכן לא יכול לסתור
            # אותם מבנית, ששניהם קוראים את אותה הרשומה באותו אופן.
            ...
            _ownership_signal = build_ownership_signal(
                ...
                reply_owner=_signal_reply_owner,
            )
