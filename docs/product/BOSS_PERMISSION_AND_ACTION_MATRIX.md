...
| Venture → Deal conversion action | **STATIC WIRED; TMA/API MISSING** | `Deals.VENTURE_LINK` and `commercial_crm.py::create_deal(venture_id=...)` are reachable through the PR1153 registry wiring, but no TMA route/button or production canary exists |
...
| Deal creation with VAT/payment-term calculation | **STATIC WIRED; RUNTIME NOT ESTABLISHED** | PR1153 registers `crm_create_deal`/`crm_create_payment_term`/`crm_create_payment` in `tool_registry.py`, `tools/dispatcher.py`, and `tools/schemas.py` with approval/tenant/emergency-stop policy. A real canary and first-class TMA surface are still missing; do not create a parallel generic path that bypasses the calculation contract |
...
| Deal/Payment/Payment-Term creation with correct VAT math | **STATIC WIRED; RUNTIME NOT ESTABLISHED** | PR1153 wired the three `commercial_crm` tools; the first-class TMA surface and real canary remain open |
