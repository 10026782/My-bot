...
**Canonical writer (fully built, zero callers):** `commercial_crm.py::create_deal(name, domain, owner_id, *, origin_lead_id, venture_id, contact_ids, amount, stage=OPPORTUNITY, priority, risk_level, notes)`. Not registered in `tool_registry.py`, `tools/dispatcher.py`, or `tools/schemas.py` — **BACKEND EXISTS BUT UNWIRED**, confirmed zero live callers anywhere including tests.
...
**If built:** this is the single largest genuinely-new build in this spec — it means wiring `commercial_crm.py` for the first time (registry entry, dispatcher case, schema, TMA route following the existing `_queue_or_owner_execute` pattern), or a first-class TMA write path that still calls the same VAT-aware writer rather than bypassing it with a generic `airtable_add`.
...
**Canonical writer (fully built, zero callers):** `commercial_crm.py::create_payment()` (always creates `status=pending`) + `calculate_payment()` (18%-VAT-aware, `fixed`/`percentage` calc types, `none`/`add`/`included` VAT rules) + `create_payment_term()`. **The only live write path into Payments today** is the one wired agent tool, `crm_mark_payment_paid` (owner/partner, high-risk, approval-gated, Telegram/tool-loop only — no TMA route calls it).
