...
**Truth Reset SHA:** `origin/main` = `9df318e1c473782096cf48fdaa983950a8485832` (31/08/2026). This is the merge containing this execution map; all runtime claims below remain static unless explicitly labelled runtime-verified.
...
**Findings carried to §10**: the Sunday 08:00 collision (`audience_report` D04 vs `weekly_quest_reset` Game) and Sunday 08:30 collision (`attribution_report` D05 vs conditionally registered `weekly_summary`) remain static findings. `weekly_summary` is the only job whose flag is checked at registration time rather than per-run.
...
8. **Two Sunday scheduler collisions remain**: 08:00 (`audience_report` D04 vs `weekly_quest_reset` Game) and 08:30 (`attribution_report` D05 vs conditionally registered `weekly_summary`). The nearby offset of `daily_usage_report` to 08:15 does not resolve either collision.
...
10. **Truth Reset SHA:** this document is tracked on `origin/main` at `9df318e1c473782096cf48fdaa983950a8485832`; earlier audit SHA references are historical and do not establish current state.
