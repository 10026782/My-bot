...
**Truth Reset SHA:** `origin/main` = `5387c909818d80af667d439e297e1f255508b610` (verified via `git rev-parse origin/main`, confirmed twice). Local working HEAD during this audit was `64ccb3bf5d8eb2349e06cd23c591cf50ac9b590d` (`codex/r10-readiness-status-reconcile`) — `git diff --stat HEAD origin/main` shows exactly 4 changed files, **zero `.py` differences**: `BUG_AUDIT_LOG.md`, `ROADMAP.md`, one N18 spec doc, `docs/governance/HORIZON.md`. Every code citation below is therefore identical on both commits; only doc-state citations (ROADMAP/HORIZON current-state prose) should be taken from `origin/main`, not local HEAD.
...
| **Contacts** | `crm.py::create_contact_from_fields()` (255-273) → `find_or_create_contact()` (242, lock-serialized) → `airtable_create()`; update via `crm.py::update_contact()` (276-280) | Yes | **CANONICAL**, + 1 confirmed **DEAD** adapter, + 1 tracked **LEGACY** import | `core/reasoning_ports.py::_ProductionContacts.find_or_create()` (146-152) imports a non-existent `contact_resolver.find_or_create_contact` — always raises `ImportError`, silently swallowed, returns a fabricated empty match. Unobservable today because its only callers are gated behind `FEATURE_DECISION_HUB`/`FEATURE_CORE_REASONING_LEADS_STATE` (both off), but is real dead/broken code. `lead_conversion.py` imports `crm.crm_add_contact` directly (bypasses dispatcher) — self-documented, tracked in `tools/audit_dispatcher_bypass.py`, owner-only `/convert`, `LEAD_AUTO_CONVERT` off |
| **Deals / Payment Terms / Payments** | New canonical: `commercial_crm.py::create_deal()` (110), `create_payment_term()` (170), `create_payment()` (251), `calculate_payment()` (49) | **No** — zero callers anywhere outside the file itself and its own tests; not registered in `tool_registry.py`/`tools/dispatcher.py`/`tools/schemas.py` | **UNWIRED** (confirms `ROADMAP.md`'s `SCHEMA_DATA_CONTRACTS` row, "still open" as of 30/08/2026) | Old `crm.py::crm_add_deal/crm_update_deal_status/crm_list_deals/crm_add_payment/crm_upcoming_payments/crm_overdue_payments` are explicitly marked in `commercial_crm.py`'s own header as orphaned/real-estate-shaped, not wired to any registry/dispatcher entry either — **DEAD**, except `crm_mark_payment_paid` which **is** wired (`tool_registry.py:347-355`, `high_risk=True`, `requires_approval=True`) — **CANONICAL** for the mark-paid update only. The Agent can still write raw `Deals`/`Payments` rows via the generic `airtable_add`/`airtable_update` tools (both tables are in dispatcher's `_TENANT_AWARE` set), completely bypassing `commercial_crm.py`'s VAT/payment-term calculation contract |
...
| audience_report (D04) | weekly Sun 08:00 | `_job_audience_report` (`:467`) | `AUDIENCE_INTELLIGENCE` (off) | yes | **duplicate slot with weekly_quest_reset**, see below |
...
| weekly_quest_reset | weekly Sun 08:00 | `_job_weekly_quest_reset` (`:628`) | `GAME_SCHEDULER` (off) | no | per-quest write unconditionally via ActionGateway; **duplicate 08:00 Sun slot with audience_report** |
...
| daily_usage_report | daily 08:15 | `_job_daily_usage_report` (`:841`) | none | no | comment notes 08:15 was deliberately offset from the D04/Game 08:00 cluster — **but the D04/weekly_quest_reset collision itself was left in place** |
...
**Findings carried to §10**: two Sunday 08:00 schedule collisions (`audience_report` D04 vs `weekly_quest_reset` Game) were never deduplicated, even though the team clearly noticed the general cluster problem enough to offset `daily_usage_report` to 08:15. `weekly_summary` is the only job whose flag is checked at registration time rather than per-run.
...
| Gmail | `tools/gmail_tools.py` shim → `tools/google_tools.py` | `GOOGLE_CLIENT_ID`/`SECRET`/`REFRESH_TOKEN` (WARNING-level) | None, `timeout=10-15` | Fail-closed; Gmail only ever drafts, never sends directly | None | CLAUDE.md says Workspace is "currently frozen"; `ORACLE_MIGRATION_M0.md` (28/08/2026) reports these vars **are** set live on Render — open, unresolved discrepancy, not settled by this audit |
...
| `core/reasoning_ports.py::_ProductionContacts.find_or_create()` | found while tracing Contacts writers | Read + import-path check | References a non-existent `contact_resolver.find_or_create_contact` — always `ImportError`, silently swallowed, fabricated empty result. Unobservable today because gated behind `FEATURE_DECISION_HUB`/`FEATURE_CORE_REASONING_LEADS_STATE` (both off) | DEAD/BROKEN — latent, will surface the moment either gating flag is turned on |
...
| Deal/Payment/PaymentTerm canonical writers (`commercial_crm.py`) | MERGED_STATIC per `ROADMAP.md`, confirmed STATIC VERIFIED but **zero callers** (§2) | N/A | RUNTIME NOT ESTABLISHED — cannot be reached at all | Not registered in tool_registry/dispatcher/schemas |
...
| Schema governance pipeline (Track 8/8B/8C) | STATIC VERIFIED + LIVE SCHEMA VERIFIED per `ROADMAP.md`'s `SCHEMA_DATA_CONTRACTS` row (30/08/2026, via a read-only Airtable MCP pass) | N/A (schema state, not a runtime toggle) | RUNTIME VERIFIED for the schema shape itself, not for any code path built on it | Register the 3 Deal/Payment tools (open) |

---

## 10. Contradictions Discovered

1. **`core/action_gateway.py` is not "largely dormant/shadow"** as its own docstring and `feature_flags.py`'s comments frame it. Ingress-side context tracking runs on every inbound message, and at least 6 production callers (scheduler jobs, TMA generic write, Interaction Engine, Lead Tier-1 preview, `core/lead_service.py`) invoke it unconditionally, with no flag check on `approve()`'s actual dispatch. `FEATURE_ACTION_GATEWAY` only controls blocking strength for one specific caller (`app.py::_queue_approval`). This is a real characterization gap between the module's own comments and its actual reach, not a re-litigation of any closed F52/Turn-Coordinator finding.
...
6. **`core/reasoning_ports.py::_ProductionContacts.find_or_create()` is silently broken** (`ImportError` on a non-existent `contact_resolver.find_or_create_contact`, swallowed, returns a fabricated empty match) — currently unobservable because every caller sits behind `FEATURE_DECISION_HUB`/`FEATURE_CORE_REASONING_LEADS_STATE` (both off), but will misbehave the instant either is activated. Not previously recorded anywhere found in this pass.
7. **Google Workspace credential status is unresolved between two sources**: CLAUDE.md calls Workspace "currently frozen"; `docs/operations/ORACLE_MIGRATION_M0.md` (28/08/2026) reports the three Google OAuth env vars are actually set live on Render. Neither this audit nor the cited doc resolves which is current — flagged as open, not settled.
8. **Two Sunday 08:00 scheduler collisions were never deduplicated** (`audience_report` D04 vs `weekly_quest_reset` Game) even though a nearby comment shows the team was aware enough of clustering to deliberately offset `daily_usage_report` to 08:15.
9. **`commercial_crm.py`'s Deal/Payment/PaymentTerm writers remain completely unwired** — zero callers outside their own file and tests, absent from `tool_registry.py`/`tools/dispatcher.py`/`tools/schemas.py`, despite being described in `ROADMAP.md` as a "foundation slice." This confirms (does not newly discover) ROADMAP's own "still open" note, with file:line specificity added.
10. **Every research subagent independently flagged uncertainty about the Truth Reset SHA** during tracing — this document resolves it directly: `git rev-parse origin/main` = `5387c909818d80af667d439e297e1f255508b610`, confirmed twice, and it is code-identical to the local working HEAD used during tracing (`git diff --stat` shows 4 doc-only files differing). No code citation above is affected by which of the two SHAs is treated as canonical.
