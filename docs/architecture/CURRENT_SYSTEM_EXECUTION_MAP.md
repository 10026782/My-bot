...
| Google Drive | `tools/drive_tools.py` shim; separately, `drive_adapter.py` (F16 media storage, used by `media_handler.py`/`cmd_decision.py`) | Same 3 Google vars + `GOOGLE_DRIVE_FOLDER_ID` | None | Fail-closed | None | **SUPERSEDED (01/09/2026):** `ORACLE_MIGRATION_M0.md`'s malformed, triple-concatenated `GOOGLE_DRIVE_FOLDER_ID` env var finding is corrected — the correctly-named key now exists with the original value, the malformed key was deleted, service restarted, no regression. Config-level fix only; a real Drive-write functional canary is still open (`BOSS_CURRENT_STATE.md` TR-29). |
