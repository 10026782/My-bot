...
**Erratum (27/07/2026, PR #471, `c64da20`, added by a Context Librarian metadata audit — not a re-run of this document's own method):** the AP-02 row and the callback-routing prose below describe the Telegram approve/reject callback as resolving its `ActionContract` only by recomputed business fingerprint. This is no longer the primary path. `_approval_callback_data()` (`app.py:1116`) now embeds the contract's exact `contract_id` in `callback_data` (`action:action_id:contract_id`); `_handle_approval_callback_impl()` resolves that exact contract via `ActionGateway.lifecycle_result()`/`find_contract()` first and falls back to fingerprint matching only for callbacks issued before this change. Separately, the reject path (BUG-144, `app.py` reject branch) now resolves and closes the canonical `ActionContract` via `reject_with_lifecycle_result()` before reporting cancellation — the "reject does not update ActionContract" characterization implied by this document's approve-only framing no longer holds for reject either. Both changes are gated behind the existing `FEATURE_ACTION_GATEWAY`/`FEATURE_SINGLE_SPEAKER_APPROVAL_UX` flags (both default off); this erratum does not itself re-run this document's audit method or renumber its entry-point/direct-execution-path counts.
