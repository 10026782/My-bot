...
| R9 | Close remaining non-universal PR2 paths. | One snapshot; no ambiguous mutation; no Agent continuation. | R9.2 CODE_DONE / STATIC_VERIFIED | Reconfirmation, combined cancellation, TMA, and runtime/deployment work remain separate. |
| R9.1 | Migrate the terminal receipt for single-contract textual confirmation through the existing approval MessageContract boundary. | ActionContract/ActionGateway lifecycle authority and evidence remain unchanged; reconfirmation and multi-contract paths remain specialized. | CODE_DONE / STATIC_VERIFIED | Runtime/deployment verification and the remaining specialized paths. |
| R9.2 | Migrate multi-contract, disambiguation, and combined confirmation receipts through the existing approval MessageContract boundary. | Selection, sibling closure, ActionContract/ActionGateway authority, and cancellation semantics remain unchanged. | CODE_DONE / STATIC_VERIFIED | Reconfirmation, combined cancellation, TMA, and runtime/deployment work. |
