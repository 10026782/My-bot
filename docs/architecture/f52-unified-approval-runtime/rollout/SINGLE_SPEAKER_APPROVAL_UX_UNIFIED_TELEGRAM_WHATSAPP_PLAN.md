...
Evidence level: **MERGED / STATIC VERIFIED** (PR #1141, merge
`327ded5552a3ea98405f8b0ff2a5954b902bf457`; verified reachable from
`origin/main` `3ac37fb4cd2f86197bcb79078c13dfd607ad50d9`). No deployment or
runtime claim is made here.

### R8.7-B Gateway-only Telegram inline approve MessageContract migration

R8.7-B routes the valid ActionContract Telegram approve callback through the
existing approval-lifecycle MessageContract adapter. The existing
`MessageContract.SUCCESS` semantic is used only after the R8.7-A evidence
handoff proves the same-turn execution. Missing evidence remains
`OUTCOME_UNKNOWN`; the no-contract/stale callback path remains fail-closed and
is not migrated. Telegram callback data, ActionGateway execution/approval
authority, and final delivery ownership remain unchanged except that callback
acknowledgments stay transport-only.

Evidence level: **MERGED / STATIC VERIFIED** (PR #1142, merge
`3ac37fb4cd2f86197bcb79078c13dfd607ad50d9`). No deployment or runtime claim
is made here.
...
| R8.7-A | Hand off same-turn Gateway execution evidence to the approval MessageContract adapter. | Existing `SUCCESS` requires verified evidence; `completed` alone remains `OUTCOME_UNKNOWN`; no-contract fallback stays separate. | MERGED / STATIC VERIFIED (PR #1141, `327ded555`) | Runtime/deployment verification and Telegram transport changes. |
| R8.7-B | Migrate the valid Gateway-backed Telegram inline approve presentation through the existing MessageContract adapter. | Verified Gateway evidence is required for `SUCCESS`; no-contract/stale callbacks remain fail-closed and separate; callback ack is transport-only. | MERGED / STATIC VERIFIED (PR #1142, `3ac37fb`) | Runtime/deployment verification and unrelated fallback paths. |
| R9 | Close remaining non-universal PR2 paths. | One snapshot; no ambiguous mutation; no Agent continuation. | MERGED / STATIC VERIFIED (PRs #1147–#1148) | Reconfirmation, combined cancellation, TMA, and runtime/deployment work remain separate. |
| R9.1 | Migrate the terminal receipt for single-contract textual confirmation through the existing approval MessageContract boundary. | ActionContract/ActionGateway lifecycle authority and evidence remain unchanged; reconfirmation and multi-contract paths remain specialized. | MERGED / STATIC VERIFIED (PR #1147, `20f8e5d`) | Runtime/deployment verification and the remaining specialized paths. |
| R9.2 | Migrate multi-contract, disambiguation, and combined confirmation receipts through the existing approval MessageContract boundary. | Selection, sibling closure, ActionContract/ActionGateway authority, and cancellation semantics remain unchanged. | MERGED / STATIC VERIFIED (PR #1148, `3b99aeb`) | Reconfirmation, combined cancellation, TMA, and runtime/deployment work. |
