...
**Erratum (27/07/2026, PR #471, `c64da20`, added by a Context Librarian metadata audit — this note does not re-run this document's own research passes or re-verify any other file:line citation below):** §1.5's "three separate, non-unified key spaces" characterization of `approval_id`/`action_id`/`contract_id` is now only partially accurate for the Telegram callback path. `_approval_callback_data()` (`app.py:1116`) now builds `callback_data` as `action:action_id:contract_id` when a contract exists, and `_handle_approval_callback_impl()` resolves the exact `ActionContract` from that embedded `contract_id` before falling back to the fingerprint-based resolution this section describes. The `action_id`/`contract_id` split itself still exists (this is not a merge into one key space, and WhatsApp/text-confirmation paths are unaffected), but the callback boundary specifically is no longer "correlates to `event_bus`'s store, a *different* pending mechanism than `ActionContract`'s `contract_id`" with no bridge between them — a bridge now exists for buttons issued after this PR. Re-verify §1.5, §4, and any other section that depends on this characterization before treating them as still fully accurate.
...
**Addendum (23/07/2026):** the contract now has a **26th scenario** ("Stale Cross-Tier Reply Resolution" family, real `my-bot-approval-staging` production evidence), added after this planning gate document was finalized — also tagged `2→3`. It is the same shape as the other `2→3` scenarios (pure precedence-decision logic, constructed signals in / decision out) and belongs in the same two test files this section already proposes (`test_turn_coordinator_shadow_decision.py`, `test_turn_coordinator_shadow_comparison.py`) once those are written — **not folded into the tables below**, which reflect this document's original finalized scope and are left as-is rather than hand-patched.
