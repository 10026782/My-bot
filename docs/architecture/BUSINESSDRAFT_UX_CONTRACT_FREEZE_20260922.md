...
*Tests:* new `test_business_draft_phase4b_generic_bypass.py` (220/220). Three existing test files were updated only because of the intended behavior change: `test_bug_commercial_crm_dispatcher_bypass_closure.py` (the tamper-proof check now runs in canonical space), and `test_bug074_approval_authority.py`/`test_bug076_lead_confirmation_policy.py` (these used a malformed generic Deals payload as an arbitrary non-lead mutation; they now use a non-commercial table). The Phase 4A dispatcher-fallback descriptions were relabeled.
