...
**Phase 4 implementation status (2026-09-23): PHASE4_STATIC_COMPLETE, code done / not yet merged.** Payment Term and Payment — the two remaining entities flagged `MISSING`/`MISSING — REQUIRED` in the Create/Update Authority Matrix above — are migrated onto BusinessDraft using the exact Phase 3 seam pattern (`app.py::_queue_approval_detailed_impl`, scoped to four additional tool names). PR #1261, branch/worktree `worktree-businessdraft-phase4-payment`, 3 commits on top of `origin/main` (`5c32659`, `e0697e6`, `050005f`).

*UPDATE (`5c32659`):* `crm_update_payment_term`/`crm_update_payment` wired through the same BusinessDraft CREATE/UPDATE → Sessions CAS → confirm → `ConfirmedSnapshot` → `ActionContract` seam as `crm_update_deal`, reusing `core/business_draft.py`'s field-translation helpers unchanged.

*CREATE (`e0697e6`):* `crm_create_payment_term`/`crm_create_charge_payment` are deliberately **not** exposed as direct agent tools — their canonical path is the deterministic `CommercialCompletionRouter` route (proven, not exposed), matching this document's own Commercial CRM First Contract table.

*Routing closure (`050005f`):* `crm_update_payment_term`/`crm_update_payment` were registered and BusinessDraft-fronted but missing from `context._ROLE_TOOLS` — the same reachability gap the Phase 3 `crm_update_deal` fix closed, just not yet applied to Phase 4's own two UPDATE tools. Fixed for Owner/Partner/Manager, same posture as `crm_update_deal`. Adds a `core/router/test_router.py` case proving `CREATE_PAYMENT_TERM` reaches `Handler.TOOL` deterministically, and expands `test_business_draft_phase4_payment.py` to assert the full `Intent → Handler.TOOL → CommercialCompletionRouter → MUTATION_TOOLS → BusinessDraft` seam chain across all 7 roles (replacing a prior `model_exposed=True`-implies-reachable overclaim with real `context._filter_tools()` checks). Also confirms `PHASE4B_SHARED_COMMERCIAL_GENERIC_BYPASS_PENDING`: the generic `airtable_add`/`airtable_update` bypass around the BusinessDraft hook (already documented as a Deal-only accepted gap in Phase 3's own "Known, accepted, out-of-scope gap" note above) applies identically to Payment Term/Payment — closing it requires touching the shared `_queue_approval_detailed_impl` choke point, out of scope for this phase.

*Tests:* `test_business_draft_phase4_payment.py` (new, 165/165), `core/router/test_router.py` (60/60), `test_commercial_crm.py` (119/119), `test_commercial_crm_dispatcher_wiring.py` (45/45), `tests/test_commercial_charge_from_term.py`, `smoke_tests.py`, `test_integration.py` (4/4) — all green, `compileall` clean.

Status is `PHASE4_STATIC_COMPLETE` only — not `MERGED_STATIC`, not deployed, not runtime-verified.

**Phase 4B implementation status (2026-09-23): PHASE4B_STATIC_COMPLETE, code done / not yet merged.** Closes `PHASE4B_SHARED_COMMERCIAL_GENERIC_BYPASS_PENDING` (and Phase 3's "Known, accepted, out-of-scope gap" above) for Deal / Payment Term / Payment. Branch `claude/phase-4b-generic-bypass-h5gmvu`, on top of `origin/main` `1dc1bf5` (PR #1261).

*Seam:* `core.action_gateway.resolve_canonical_call()` — already invoked by `app._queue_approval_detailed_impl()` before BusinessDraft / `executed_action_cache.compute` / pending-fingerprint lookup / `ActionGateway.propose_action()`, and again inside `propose_action()` — now translates a generic `airtable_add`/`airtable_update` on Deals / Payment Terms / Payments into `crm_create_deal`/`crm_update_deal`, `crm_create_payment_term`/`crm_update_payment_term`, `crm_create_charge_payment`/`crm_update_payment` with a primitive writer-space payload. The existing Phase 3 / Phase 4A seams then run unchanged (their own `enforce(canonical_tool, identity)`, owner resolution, Sessions CAS, `ConfirmedSnapshot`, cleanup). No generic draft lifecycle was added. The second canonicalization inside `propose_action()` is a proven no-op (idempotent).

*Single mapping authority:* the generic-column → canonical-kwarg maps, protected-table alias resolution, legacy-vs-V2 Payment selection and link unwrapping moved out of `tools/dispatcher.py` into the pure `core/commercial_generic_canonicalization.py` (no I/O, no identity). The dispatcher re-binds the same objects under their old private names. Mapping drift closed explicitly: the Deal map gained `Counterparty Contact`/`Counterparty Organization`/`Start Date` (writer-supported; without them no generic Deal CREATE could satisfy the `one_of_required` counterparty invariant), and Payments UPDATE recognizes the V2-only writer-updatable columns (`Method`, counterparty, document). Financial immutability is still enforced by the writer/BusinessDraft allowlists.

*Fail closed before any ActionContract:* ambiguous or absent table, non-dict `fields`, an unmapped column, invalid link cardinality, a malformed UPDATE `record_id`, empty fields, an unknown Deal Domain word, and a generic Payments CREATE that resolves to the legacy flat `crm_create_payment` shape (owner decision: generic `airtable_add` is no longer a bridge to the legacy writer; the direct primitive is untouched). The user-facing reason comes from `CommercialCanonicalizationError.user_message`, with `terminal_outcome=APPROVAL_QUEUE_NEVER_ATTEMPTED`.

*Backward compatibility:* the dispatcher's generic → commercial redirects are kept as an **execution fallback only**, for ActionContracts approved under a generic tool identity before Phase 4B. That fallback is not BusinessDraft-fronted. Charges and Organizations keep their existing (pre-4B) state and are out of scope.

*Known limitations:* (1) a direct `ActionGateway.propose_action()` caller that names a generic commercial table would get the canonical tool identity but no BusinessDraft. The audit found no such caller; every direct caller targets Tasks, Leads, Quests, Interaction Log or `tma_write`. (2) The Deal enrichment flow's generic update now goes through BusinessDraft UPDATE, so an enrichment whose values all equal the current record fails closed as an empty delta instead of queuing a no-op write.

*UX parity:* `crm_update_deal` now gets the same label-aware Deal summary (`deal_field_business_summary`, fed by the inverse of the shared Deal map) in the approval prompt and in the reconfirmation/lifecycle descriptions that the generic Deals path showed, so the enrichment flow's messages do not regress.

*Tests:* new `test_business_draft_phase4b_generic_bypass.py` (223/223). Four existing test files were updated only because of the intended behavior change: `test_bug_commercial_crm_dispatcher_bypass_closure.py` (the tamper-proof check now runs in canonical space), `test_diamond_d3_final_select_resolver.py` (an invalid Deal Domain on a generic update is now blocked at proposal instead of at dispatch, which is stronger), and `test_bug074_approval_authority.py`/`test_bug076_lead_confirmation_policy.py` (these used a malformed generic Deals payload as an arbitrary non-lead mutation; they now use a non-commercial table). The Phase 4A dispatcher-fallback descriptions were relabeled.

Status was `PHASE4B_STATIC_COMPLETE` at merge (PR #1262, merge commit `507e12b3`).

**Phase 4B runtime status (2026-09-23): partial `RUNTIME_VERIFIED`.** Deployed SHA `507e12b3` confirmed live on Render (`my-bot-jqz2.onrender.com`). A real Telegram owner session exercised the Deal enrichment flow's generic `airtable_update` on `עסקאות (Deals)` in production; Render application logs directly confirm canonicalization to `crm_update_deal` before `ActionContract` creation, a canonical primitive `payload_keys` shape (never `table`/`fields`), dispatch reaching the dedicated writer case (not the compatibility redirect), a correct live write (`external_id=recFi9Vpdxv2DTuIg`), and a business-readable, non-leaking approval message — with `agent_calls=0`, confirming this write never passed through the Agent/LLM tool-use loop. Full detail and raw log excerpts: `docs/evidence/PHASE4B_DEAL_UPDATE_RUNTIME_20260923.md`.

**Payment Term generic UPDATE is also `RUNTIME_VERIFIED`** as of the same session: a free-text owner request ("תעדכן ב-Airtable את שדה ההערות בתנאי התשלום ... ל-'בדיקת קנוני 4B'") was independently confirmed by reading the live `ActionContracts` and `Payment Terms` records directly via the Airtable connector (not logs) — `tool_name=crm_update_payment_term`, `normalized_payload={"notes": "בדיקת קנוני 4B", "record_id": "recZ6IkYGDOucA8Cl"}`, `status=completed`, and the live record's `Notes` field holds the exact value. This same test surfaced a real messaging bug (both `crm_update_payment_term` and `crm_update_payment` had no branch in the pending-approval/completion/reconfirmation text builders, falling through to a generic, failure-sounding fallback) — the underlying write was correct throughout; only the text was wrong. Fixed in commit `207019c3` with 20 new regression checks (243/243 total); the fix itself has not yet been independently redeployed/reverified live. Full detail: `docs/evidence/PHASE4B_PAYMENT_TERM_UPDATE_RUNTIME_20260923.md`.

Still open (`STATIC_VERIFIED` only, the 243-check `test_business_draft_phase4b_generic_bypass.py` suite against the same shared `core/commercial_generic_canonicalization.py` module): Deal generic CREATE, Payment Term generic CREATE, Payment generic CREATE/UPDATE, the legacy-Payment fail-closed block, and role-authorization denial for a generic call on these tables.

**Phase 4C implementation status (2026-09-23): `STATIC_VERIFIED`, code done / not yet merged.** Closes the gap Phase 4B left open: even after 4B, a plain conversational Deal/Payment Term/Payment field UPDATE ("עדכן בעסקה X את הסטטוס ל-Y") still reached the Agent's own tool_use loop to pick the record and the tool, before landing on the same canonicalization/BusinessDraft seam. Branch `claude/phase-4c-deterministic-update-routing`, on top of `origin/main` `b679c54d` (post-#1263 merge).

*Recognition:* new `core/deterministic_commercial_update.py` — a bounded two-shape Hebrew grammar (entity-first, and the exact field-first live-incident phrasing) plus a narrow commercial-update GUARD (update verb + exactly one bare-or-ב/ל-prefixed protected-entity mention) that fires when the strict grammar can't fully parse an otherwise-clear commercial mutation attempt (field-before-entity order, colon/equals syntax, a reference with no field/value). Field aliases are cross-checked at import time against `commercial_crm.py`'s own `_DEAL_UPDATE_ALLOWED`/`_PAYMENT_TERM_UPDATE_ALLOWED`/`_PAYMENT_UPDATE_ALLOWED` — no duplicate allowlist.

*Required invariant:* for any message confidently classifiable as a commercial UPDATE for Deal/Payment Term/Payment, the result is always `DETERMINISTIC_UPDATE`, `DETERMINISTIC_CLARIFY`, or `DETERMINISTIC_FAIL_CLOSED` — never `Handler.AGENT` merely because parsing or record resolution was incomplete. Concretely: every branch of the new `app.py` `Handler.TOOL` dispatch block for these three intents unconditionally returns; a Payment resolved by typed name (no resolver exists for it in this codebase — no Name-like field) terminates as its own deterministic clarify ("לא ניתן לזהות בוודאות את התשלום לפי השם הזה...") rather than falling through.

*Record resolution priority:* explicit Airtable record id → unique typed name (`commercial_crm.lookup_human_reference`, Deal/Payment Term only) → the "just created" session marker (new `session_store.set_/get_last_commercial_create`, stamped generically for all three entities in the existing shared post-approval continuation hook, valid under 1 hour) → deterministic CLARIFY. Never a fuzzy guess, never "first result."

*Router seam:* `core/router/router.py` gains a deterministic override mirroring the existing `CREATE_DEAL`/S2C gates — `Handler.TOOL` is assigned whenever the parser structurally recognizes the message (full grammar or guard), never merely on keyword confidence. `_ROLE_TOOLS`, `airtable_update`/`airtable_get` exposure, and Payment writer permissions are all unchanged; `crm_update_deal`/`crm_update_payment_term`/`crm_update_payment` remain reachable by the Agent for anything this layer doesn't recognize. Phase 4B (`core/commercial_generic_canonicalization.py`) is untouched and remains the defense-in-depth boundary for any unexpected generic proposal.

*Tests:* new `test_business_draft_phase4c_deterministic_update.py` (42/42, 99 checks) — parser/resolver unit coverage, router `Handler.TOOL` assignment per entity, full `run_agent()` E2E proving `build_context()`/Anthropic are never called (`agent_calls=0`) for every recognized shape including the exact live-incident text and all 5 guard examples, plus explicit negative controls (a read-only question, an unrelated back-reference, general discussion, other-entity UPDATE) proving the guard doesn't over-fire. Full repo suite (415 `test_*.py` scripts): 0 failures.

Status was `STATIC_VERIFIED` at merge (PR #1264, merge commit `a41f01ca`). Tool-exposure cutover (removing `crm_update_*` from `_ROLE_TOOLS`) is explicitly deferred to a later phase.

**Phase 4C runtime status (2026-09-23): live-found-and-fixed gap, redeploy pending.** Deployed SHA `a41f01c` confirmed live on Render. A real Telegram owner session sent a fresh, well-formed Phase 4C UPDATE command ("תעדכן בעסקה TEST-4B-DELETE-ME את ההערות ל-בדיקת Phase 4C") immediately after an abandoned `CommercialCompletionRouter` CREATE clarification (a failed payment-term-creation attempt still parked awaiting a Deal name). Production logs show the command was swallowed as a literal answer to the stale session — the Airtable `SEARCH()` query logged was the entire new message, not the intended Deal reference, and the reply was the stale session's own generic "לא מצאתי התאמה" rather than anything from Phase 4C's own dispatch block. Root cause: `app._is_fresh_deterministic_command()` — the shared escape hatch every other deterministic command family already used to break out of a parked session (`BUG-S2C-STALE-SESSION-SWALLOWS-NEW-COMMAND`) — never checked `parse_deterministic_commercial_update()`, since that parser didn't exist when the escape hatch was written. Fixed by adding it there (gated on `matched`, so a guard-recognized-but-incomplete UPDATE also escapes to its own deterministic CLARIFY). New `test_36` in `test_business_draft_phase4c_deterministic_update.py` (43/43, 103 checks) reproduces the exact live text plus a negative control confirming the escape hatch stays precise. Full repo suite: 415/415. The underlying Phase 4C routing itself was never wrong in this incident — the fix is entirely in the pre-existing session-precedence layer Phase 4C didn't originally register with. Not yet redeployed/reverified live.
