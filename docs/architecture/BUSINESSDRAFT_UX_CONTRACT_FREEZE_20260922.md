...
Status is `STATIC_VERIFIED` only — not merged, not deployed, not runtime-verified. Tool-exposure cutover (removing `crm_update_*` from `_ROLE_TOOLS`) is explicitly deferred to a later phase.
