...
This covers Deal generic UPDATE only. Deal generic CREATE, Payment Term generic CREATE/UPDATE, Payment generic CREATE/UPDATE, the legacy-Payment fail-closed block, and role-authorization denial for a generic call on these tables remain `STATIC_VERIFIED` only (the 223-check `test_business_draft_phase4b_generic_bypass.py` suite against the same shared `core/commercial_generic_canonicalization.py` module) — not yet independently runtime-verified.
