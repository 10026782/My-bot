...
| Pre-deploy migration command | `python -m core.database_migrations` — runs every `*.sql` file under `core/migrations/` in sorted order, `CREATE TABLE/INDEX IF NOT EXISTS` throughout → **idempotent, safe to re-run on every deploy**. No-ops (exit 0) if PostgreSQL isn't configured at all. | `core/database_migrations.py`, `docs/operations/DEPLOYMENT.md` |
...
11. Re-run `tools/phase_4b_rollout_readiness.py` — must be GO.
...
15. Run `tools/phase_4b_rollout_readiness.py` again — must be GO.
