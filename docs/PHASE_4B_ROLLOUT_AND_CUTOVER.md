...
| Render frontend (TMA) | Vercel project rooted at `tma-frontend/`, `VITE_API_URL` → backend above | `docs/operations/DEPLOYMENT.md` |
...
| Pre-deploy command | `python3 -m core.predeploy` — runs `core/database_migrations.py::run_migrations()` (every `*.sql` file under `core/migrations/` in sorted order, `CREATE TABLE/INDEX IF NOT EXISTS` throughout → **idempotent, safe to re-run on every deploy**, no-ops if PostgreSQL isn't configured), then the Emergency Stop preflight. | `core/predeploy.py`, `core/database_migrations.py`, `docs/operations/DEPLOYMENT.md` |
