...
- **A malformed env var key existed on Render**: literally named `GOOGLE_DRIVE_FOLDER_IDGOOGLE_DRIVE_FOLDER_IDGOOGLE_DRIVE_FOLDER_ID` (alongside a separate, correctly-named `GOOGLE_DRIVE_ARTIFACT_FOLDER_ID`). Almost certainly a dashboard copy-paste artifact. Was reconfirmed still present and unresolved earlier on 01/09/2026 (`BOSS_CURRENT_STATE.md` TR-25). **RESOLVED, same day (`BOSS_CURRENT_STATE.md` TR-29):** the correctly-named `GOOGLE_DRIVE_FOLDER_ID` key was created with the malformed key's original value, the malformed key was then deleted, and the service was restarted with no regression. Config-level fix only — a real Drive-write functional canary confirming files land in the intended folder is still open.
- `SETUP_WEBHOOK=1` is set permanently in production, though `.env.example` documents it as a one-time flag meant to be removed after first webhook registration. Not changed here; carried into `.env.oracle.example` with a note. **Reconfirmed still `1` on 01/09/2026** in the same fresh pull — unchanged since this snapshot.
