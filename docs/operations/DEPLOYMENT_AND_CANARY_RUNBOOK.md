...
**Source:** `docs/operations/MASTER_VERIFICATION_MATRIX.md` (01/09/2026). **Truth Reset SHA:** `origin/main` = `b58b27f8771c8ffd4c633a84a28b4009178fbeca` — one commit behind current tip `a45f304` (CI/test-hygiene only). **Type:** execution sequence for the next deployment window, derived directly from the matrix's per-capability "Canary Required"/"Runtime Evidence Required" columns.

> **RECONCILIATION ADDENDUM (01/09/2026, same-day, `a45f304`):** Phase 2 items 1-2 below (WhatsApp/Voice) are **SUPERSEDED — DO NOT EXECUTE AS WRITTEN**: `config.py::OWNER_USER_ID_MAPPINGS` is empty, so both fail closed regardless of flag state; owner must supply real mapping values first (Voice additionally needs `VOICE_IVR` turned on). Phase 2 items 4/6 (Decision Hub, Media) are **ready sooner than this ordering implies** — `FEATURE_DECISION_HUB`/`FEATURE_MEDIA_UPLOAD`/`FEATURE_VOICE_NOTES` are already `true` in live Render env, not pending an owner flag decision as items 4/6 state; only the functional canary itself remains. Current authority: `BOSS_CURRENT_STATE.md` TR-21–TR-27. **Revised near-term priority order:** Commercial CRM (item 3, unaffected) and Decision Hub/Media (items 4/6, flag already on) are ready now; WhatsApp/Voice (items 1-2) are not, pending the owner-mapping prerequisite.
...
- **A1** Confirm current Google OAuth env vars in Render. The prior frozen-vs-live contradiction was owner-confirmed as unfrozen on 31/08; current values still require live evidence.
...
1. **SUPERSEDED — DO NOT EXECUTE AS WRITTEN.** ~~Lead creation — WhatsApp (§2.1)~~: owner approval (ROADMAP N18) → `WHATSAPP_CANONICAL_LEAD_WRITE=true` for one non-furniture WhatsApp number → send one real test inbound message → confirm new Lead record via `core.whatsapp_lead_cutover.create_whatsapp_inbound_lead`, correct Owner via `core/source_owner_mapping.py`, no duplicate. **Missing precondition (added 01/09/2026): owner must first populate `OWNER_USER_ID_MAPPINGS.whatsapp_destination` — without it this step fails closed, it does not test what it claims to.** Rollback: flag back to false.
2. **SUPERSEDED — DO NOT EXECUTE AS WRITTEN.** ~~Lead creation / Voice (§2.1, §2.16)~~: requires step 1's GO plus a **separate** owner approval (this touches a live-bypass path with no current safety net) → `VOICE_CANONICAL_LEAD_WRITE=true` → one real test call through the IVR to completion → confirm `create_voice_inbound_lead()` ran (not the legacy `airtable_add()` bypass), Owner resolved correctly. **Missing preconditions (added 01/09/2026): owner must first populate `OWNER_USER_ID_MAPPINGS.voice_destination` AND turn on `VOICE_IVR` (currently off) — without both this step fails closed/is unreachable.** Rollback: flag back to false — legacy bypass resumes, zero-risk since that's already current production behavior.
3. **Deals / Payments / Finance (§2.3, §2.4, §2.21)**: owner-approved canary for the already-registered `commercial_crm.py` tools (`crm_create_deal`/`crm_create_payment_term`/`crm_create_payment`), with `requires_approval=True`, `tenant_scoped=True` → one real test Deal + Payment Term via the agent → confirm Airtable records match the VAT/calculation contract and no orphaned raw writes bypass it. TMA surface and raw-write ownership remain separate decisions. Rollback: disable the registered entries; no data migration is expected for pre-existing records.
4. **Decision Hub (§2.12)**: **`FEATURE_DECISION_HUB` is already `true` in live Render env as of 01/09/2026 — the "for owner-only testing" flag-flip framing below is no longer a pending step, only the canary itself is.** → one real `/decision new` → `/decision status` cycle → confirm full lifecycle completes via the correct storage adapter. Rollback: unset the flag if a real problem is found (not required merely to "undo" a still-pending activation — there isn't one). *(The Contacts adapter this depends on is confirmed already fixed on `main` — see matrix §0.3/§2.2, no separate contact-adapter canary needed.)*
...
6. **Media (§2.11)** — recommended addition, not in the older runbook: **`FEATURE_VOICE_NOTES` and `FEATURE_MEDIA_UPLOAD` are already `true` in live Render env as of 01/09/2026 — there is no pending flag decision, only the untested live path.** Note that **zero tests cover this now-flag-ON path**. Recommended next step: one real Telegram voice-note and one real photo upload against the already-active flags, specifically to catch a live-wiring regression no test would catch first.
...
| Scheduler (§2.17) | No test asserts `.at()` schedule times — the remaining Sunday 08:30 collision (`weekly_summary`/`attribution_report`) is invisible to CI; the 08:00 pair was retimed to 08:05 by PR1153 | Not a flag — 08:30 still needs a code/config decision, and no regression test proves the retime stays fixed | Resolve/accept the 08:30 overlap and add a schedule-collision assertion |
