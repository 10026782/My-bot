...
| C3 | Owner decision to register the 3 commercial_crm.py tools (ROADMAP "still open" item — this is new wiring work, not just a flag) | Add `create_deal`/`create_payment`/`create_payment_term` to `tool_registry.py`+`tools/dispatcher.py`+`tools/schemas.py`, `requires_approval=True`, `tenant_scoped=True` | One real test Deal + Payment Term via the agent, owner-approved | Airtable records match `commercial_crm.py`'s VAT/calculation contract | Contract math correct, no orphaned raw writes bypassing it | Calculation mismatch, or the generic `airtable_add` path still gets used instead | Remove the registry/dispatcher/schema entries — no data migration needed since nothing wrote through them before |
...
| F4 | `core/reasoning_ports.py::_ProductionContacts.find_or_create()`'s broken import | Fix, not retire — this is currently dead code that will break the moment `FEATURE_DECISION_HUB` or `FEATURE_CORE_REASONING_LEADS_STATE` goes live; fix it in step D8/E-adjacent work, before either of those flags reaches "on" for real traffic |
