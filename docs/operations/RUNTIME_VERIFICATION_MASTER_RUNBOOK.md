...
**Companion to:** `docs/architecture/CURRENT_SYSTEM_EXECUTION_MAP.md` (the execution map this runbook sequences against). **Truth Reset SHA:** `origin/main` = `b58b27f8771c8ffd4c633a84a28b4009178fbeca` (01/09/2026) — one commit behind current tip `a45f304` (CI/test-hygiene only).

> **RECONCILIATION ADDENDUM (01/09/2026, same-day, `a45f304`) — DO NOT RUN §C1/§C2 AS WRITTEN:** a Grade-A pass found `config.py::OWNER_USER_ID_MAPPINGS` is empty. Both §C1 (WhatsApp) and §C2 (Voice) below assume flipping the flag and sending one test message is sufficient — it is not: `create_whatsapp_inbound_lead()`/`create_voice_inbound_lead()` fail closed on empty owner mapping regardless of flag state, so running §C1/§C2 today would produce a `blocked` result, not the GO/STOP signal these steps describe. **Missing precondition, not currently listed in either step's "Preconditions" column:** owner must first supply real `OWNER_USER_ID_MAPPINGS` values. Separately, §D's "Decision Hub/Media activate per owner decision" framing (step 8) is stale for the *flag* dimension — `FEATURE_DECISION_HUB`/`FEATURE_MEDIA_UPLOAD`/`FEATURE_VOICE_NOTES` are already `true` in live Render env; only the functional canary remains, the flag decision is not still pending. Current authority: `BOSS_CURRENT_STATE.md` TR-21–TR-27.
>
> **SECOND ADDENDUM (01/09/2026, same day, `origin/main` now `809ffc9`):** the owner-mapping *mechanism* referenced above (env-JSON override for `OWNER_USER_ID_MAPPINGS`) is merged and deployed (PR #1159); the env var itself remains unset, so §C1/§C2 are still not ready to run — no change to the guidance above. Unrelated to owner-mapping: §E2's PA-01 malformed live value (`"shadow."`) and the Google Drive malformed-key finding referenced elsewhere in this runbook's companion docs are both now corrected and restart-verified. See `BOSS_CURRENT_STATE.md` TR-28/TR-29 for detail — this does not change §C1/§C2's STOP condition.
...
| Step | Preconditions | Flag/config | Canary | Evidence | GO | STOP | Rollback |
|---|---|---|---|---|---|---|---|
| C1 | **SUPERSEDED RUNBOOK STEP — DO NOT EXECUTE AS WRITTEN (see 01/09/2026 addendum above).** Owner approval (per ROADMAP N18 "remaining work") **+ owner must first populate `OWNER_USER_ID_MAPPINGS.whatsapp_destination` (missing precondition)** | `WHATSAPP_CANONICAL_LEAD_WRITE=true` for one non-furniture WhatsApp number only | Send one real test inbound WhatsApp message from that number | New Lead record created via `core.whatsapp_lead_cutover.create_whatsapp_inbound_lead`, Owner correctly resolved via `core/source_owner_mapping.py` | Record correct, Owner correct, no duplicate | Missing/duplicate lead, wrong Owner, **or a `blocked`/"owner mapping missing" result — expected today, not a real test failure** | Set flag back to false |
| C2 | **SUPERSEDED RUNBOOK STEP — DO NOT EXECUTE AS WRITTEN (see 01/09/2026 addendum above).** C1 GO, separate owner approval (Voice canonical write touches a live-bypass path with no current safety net) **+ owner must first populate `OWNER_USER_ID_MAPPINGS.voice_destination` (missing precondition) + `VOICE_IVR` must be turned on (currently off in live env)** | `VOICE_CANONICAL_LEAD_WRITE=true` | One real test call through the IVR to completion | Lead created via `create_voice_inbound_lead()` instead of the legacy `airtable_add()` bypass; Owner resolved | Record correct, Owner correct | Any regression vs the legacy path's current (admittedly worse) behavior | Set flag back to false — legacy bypass resumes, which is the current production behavior, so rollback is zero-risk |
