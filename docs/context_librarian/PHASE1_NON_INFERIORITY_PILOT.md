...
`--assert-main` that fails closed off `main`, addressing this run's Critical
commit/branch mislabeling at the tool level instead of relying on whoever
writes about a bundle afterwards to get it right. It also addresses an
unrelated, more consequential bug found while making these changes:
`_render()` only ever emitted `notes[0]` for every node — every note past the
first was silently dropped from every bundle ever built, for every layer (6/6
