...
## Addendum — 14/08/2026 (documentation-drift reconciliation, evidence re-verified against `origin/main` @ `904ce13b`)

The three findings below were re-verified directly (grep + `gh run`/`gh pr` against
the exact commit named), not re-derived from this document's original text. This
addendum does not alter the historical findings below it — it supersedes them for
current-state purposes only where noted.

1. **CI-red finding (§ "Main's own CI is red right now", line 15) — superseded, then recurred differently.** The specific gate this audit named ("Context Librarian authoritative post-merge refresh check", 4 STOP/21 REVIEW_REQUIRED/98 WARNING @ `134148e`) passed cleanly on multiple later main commits (confirmed via `gh run view` on the CI run for `cb03b24`, 2026-08-14 00:05 UTC: step "Context Librarian authoritative post-merge reconciliation" → `success`). **However, CI is red again at the actual current cut SHA `904ce13b`** — both the "CI" workflow and a newer, separate "Context Librarian Reconciliation" workflow (added 2026-08-14 by PR #634, not present when this audit was written) show `conclusion: failure`. The `904ce13b` CI run shows the reconciliation step itself failing, which cascaded to skip the TC10/pytest-librarian steps entirely (not pass, not fail — skipped). This is a **current, live gap**, not a stale historical one. **Root cause identified**: running `python3 -m tools.context_librarian reconcile --check` directly against `904ce13b` returns `"outcome": "OWNER_DECISION_REQUIRED"` with exactly 5 unregistered sources, all from PR #623/#640 (not from the PR #611/#628/#634/#638 batch that targeted this same gate) — `marketing_fact_authority.py` (**STOP**: "unregistered source may change authority"), `marketing_creative_renderer.py`, `marketing_creative_templates.py`, `tools/tool_runtime_snapshot.py`, `data/tool_registry/runtime_snapshot.json` (all REVIEW_REQUIRED). Fix is catalog registration (an owner decision on each source's role), not a code change — out of scope for this documentation-only pass.
2. **ws2/ws3 "zero references" finding (§ "Branch-only implementation found" / "dormant parallel authority", lines 44 & 105) — the literal grep result is now stale.** Re-running the same grep at `904ce13b` finds real call sites: `core/action_gateway.py` now imports and calls `build_evidence_result`/`build_action_lifecycle_result` from `core.evidence_projection`/`core.lifecycle_projection`. Tracing the call sites: they are `approval_status()`, `execution_status()`, and `reply_ownership_for_contract()` — each explicitly docstring-labeled as a **read-only WS2/TC6 projection/query method**, not part of the reply-composition text path. `TC9`'s `_message_contract_for_fact()` still appears to be the sole function that builds the actual reply text (not independently re-traced end-to-end here). **Recommendation: re-word the "zero references" claim to "no references from the reply-composition text path" — the broader risk this section flags (an unregistered second source of truth) is narrowed but not fully closed; route any further wiring decision through `docs/governance/GAP_QUALIFICATION.md` per its own policy, not this audit.**
3. **TC10 21/21 baseline (lines 76, 84-89) — historical, preserved as-is.** This remains accurate for `134148e` and is not being rewritten. For current-state purposes, the newest actually-executed TC10 result (as of `904ce13b`, where the step was skipped due to finding 1 above) is **PASS at commit `7d742f7`** (2026-08-13, confirmed ancestor of `904ce13b`, CI run `31760895350`). Use `7d742f7` as the current baseline; `134148e`'s 21/21 remains valid historical evidence of the same suite passing four days earlier.
