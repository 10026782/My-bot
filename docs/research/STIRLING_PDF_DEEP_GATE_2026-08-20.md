...
**Last Updated: 2026-08-20 — Authentication correction (see note below).**
...
| **Authentication** | Off by default (`SECURITY_ENABLELOGIN`/`DOCKER_ENABLE_SECURITY: "false"` in every official compose example) — confirmed still the default at v2.14.3. For a single-caller (BOSS-only) deployment this is an acceptable default *only* combined with network isolation — BOSS should still set `security.customGlobalAPIKey` and send it as `X-API-KEY` on every call, per the prior audit's own integration sketch. **⚠️ Correction (added during PR #784, the Stirling-PDF Capability POC):** this gate originally characterized `customGlobalAPIKey` as "a single shared API key, MIT-licensed mechanism, not a proprietary-tier feature." Deeper source inspection during the POC found that's wrong — the *enforcement* of this mechanism (`UserAuthenticationFilter`, `UserService`, and the login system it depends on) lives entirely in `app/proprietary/`, under the proprietary **Stirling PDF User License**, not the MIT core. It is **not** an MIT-core capability, and production use of it is **not license-cleared** by this gate — that requires a separate license decision (paid User License, or a non-Stirling auth layer such as network-isolation-only with a reverse-proxy-injected header). See `docs/research/STIRLING_PDF_CAPABILITY_POC.md` ("Authentication — and a licensing nuance discovered while building this") for the full finding; that POC uses this mechanism only for local evaluation, not as a production clearance claim. **New finding this round (unaffected by the above):** avoid the `/api/v1/pipeline/*` endpoint family specifically — see CVE-2026-57485 below; BOSS's planned direct-endpoint-per-tool integration (`pdf_merge`, `pdf_compress`, etc., not the pipeline feature) never touches this at all. |
