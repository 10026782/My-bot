...
## n8n: boundary-scoped verdict (revision, 2026-08-19)

**The single "REFERENCE ONLY" label in the original comparison table conflated four different use cases with very different risk profiles. This revision separates them — the underlying license and security findings below (§5/§10 of the full n8n audit) are unchanged; only the verdict-per-use-case framing changes.**

Prior BOSS research already classified n8n as *optional internal automation / non-core* — not a candidate to become BOSS's Router or Orchestrator. This audit's blanket "REFERENCE ONLY" verdict was really scoped to that one role (n8n competing with `core/router/*` and `run_agent()` to decide what happens to a customer request) and should not be read as "avoid n8n across the board." The full audit's own §17 already hedges this ("n8n remains useful as a pattern/reference... and, in principle, as a tightly-wrapped internal-only utility... but this deeper pass surfaces a security-advisory track record severe enough that even that narrow use should not be adopted without a deliberate, owner-approved hardening plan") — this section makes that distinction the headline rather than a footnote.

| Use case | Verdict | Why |
|---|---|---|
| **BOSS core/router/orchestrator** — deciding what happens to a customer request, holding tenant credentials, making approval/routing decisions | 🔴 **REJECT (unchanged)** | n8n is architecturally built to be an orchestrator itself (its own scheduler, execution queue, credential store, and — in recent releases — its own agent/approval nodes). Wiring it into BOSS's live customer path means either bypassing its intended design or standing up a second, ungoverned decision authority alongside `tool_registry.enforce()`/the approval flow — the exact disqualifying pattern in §3 of the full audit below. This is the only use case the license risk (§5) and the 152-advisory security track record (§10) were originally weighed against most heavily, since it's the one that would put n8n on BOSS's inbound customer-facing path. |
| **Internal/personal automation for the BOSS team** — e.g. Webhook → GitHub → Telegram notification, Form → DB → alert, scheduled reports, sync jobs, no BOSS tenant data or credentials involved | 🟢 **CANDIDATE / POC** | This is the narrow use case the full audit already found license-compliant (§5/§9: "FREE WITH INFRA COST" for internal-only use — no tenant workflows/credentials means no commercial-license trigger). The security findings in §10 still apply in full regardless of scope: any internet-facing n8n instance inherits the same 152-advisory/22-critical CVE surface whether it serves internal or customer traffic. A POC here needs the hardening plan the full audit already names (no community nodes, aggressive patch SLA, network isolation, no credential reuse with BOSS's own secrets) before real adoption — a license check alone is not sufficient. |
| **Glue layer between BOSS and external SaaS APIs** — `BOSS → n8n workflow adapter → SaaS A/B/C`, with BOSS retaining all authority over who may act, whether approval is required, and business state | 🟢 **CANDIDATE, conditional** | Architecturally sound *only* if n8n never becomes the decision-maker: it must sit strictly downstream of `tool_registry.enforce()`, hold no BOSS tenant credentials of its own, and never write business state directly — the same containment already specified in §3 of the full audit ("BOSS's dispatcher calls out to an n8n workflow as a single governed tool... the n8n workflow itself must never write to Airtable/CRM directly, never hold BOSS's own credentials"). Under that framing, this is a legitimate way to avoid hand-building and maintaining many one-off SaaS integrations, at the cost of the same ongoing security/ops burden documented in §10/§12 — not a free win. |
| **Developer-workflow automation** — n8n as the glue running an `Issue → n8n → Claude/Codex → GitHub branch → tests → review-notification` pipeline, i.e. orchestrating dev tooling, not writing code itself and not touching BOSS's customer runtime at all | 🟡 **AUDIT / POC CANDIDATE — not evaluated by this audit** | This is genuinely different from every row above: it's dev tooling, not customer-facing BOSS runtime, so the license risk in §5 (specifically about hosting workflows/credentials *for BOSS's own business customers*) is largely moot here. It was not in scope for this audit and has no verdict from it — the open questions worth a dedicated, narrower follow-up are: how GitHub/CI tokens would be held inside an n8n instance given the CVE history in §10, and whether that residual risk is acceptable for a dev-only tool with no customer-facing exposure. |

**Net effect on the "Top 3 not to include" list below:** n8n's entry there should be read as scoped to the core/router row above, not as a recommendation against the internal-automation, SaaS-glue, or dev-workflow use cases — those remain open, license-compliant-if-scoped candidates, gated on the same hardening plan, not on n8n itself being unusable.
...
1. **n8n as BOSS's core/router/orchestrator** — REJECT for this role specifically (see "n8n: boundary-scoped verdict" above for the other three use cases, which remain open CANDIDATE/POC territory, not rejected). For the core/router role: n8n is architecturally built to be an orchestrator itself, which is exactly the role BOSS's architecture reserves for its own `core/router/*` + `run_agent()`. Independent of the license question, the 152-advisory/22-critical security track record (including a CISA-KEV actively-exploited RCE discovered in the last month) means even a scoped, internal-only POC needs an owner-approved hardening plan (no community nodes, aggressive patch SLA, network isolation, no credential reuse) before adoption — the CVE exposure doesn't go away just because the use case is internal.
...
- **n8n vs. Dify vs. Flowise — same slot, same objection, but only for the core/router role.** All three compete to become BOSS's "second orchestration authority" (workflow/agent runtime with its own credential store and execution engine) — the exact anti-pattern BOSS's architecture is built to prevent. None is preferred over the others for *that* role; all three are excluded from it. This does **not** extend to n8n's other use cases (internal automation, SaaS glue, dev-workflow tooling — see "n8n: boundary-scoped verdict" above), where n8n isn't competing for orchestration authority at all and isn't in the same bucket as Dify/Flowise. If the core-orchestrator category is ever revisited: Flowise is moot (dead project); n8n has the broadest connector catalog but the worse license/security profile; Dify has the most BOSS-relevant unique capability (RAG) but the sharpest license conflict (multi-tenant restriction almost verbatim matching BOSS's own model). Flag for the librarian: **Activepieces** (MIT core, surfaced by the n8n audit as the natural next candidate) removes n8n's disqualifying license risk and isn't in either canonical catalog yet — worth a future look if this category is reopened.
