...
|---|---|---|---|---|---|---|---|---|---|
| **n8n** | Node-based workflow automation, 1500+ connectors | HIGH vs `scheduler.py`/`worker.py`/Google Workspace `tools/` | Sustainable Use License (source-available, not OSI) — blocks multi-tenant hosted use without paid agreement | Yes (Docker+Postgres, Redis for scale) | Free internal-only; PAID REQUIRED once tenant data is touched; Enterprise price unpublished | **HIGH** — 152 advisories (22 critical, 62 high); CISA-KEV actively-exploited RCE; fix-bypass CVE, all within last month | MEDIUM-HIGH (patch cadence is load-bearing) | 3/10 | **REFERENCE ONLY** |
...
1. **n8n** — REFERENCE ONLY. License structurally conflicts with BOSS's multi-tenant shape beyond internal-only use, and the 152-advisory/22-critical security track record (including a CISA-KEV actively-exploited RCE discovered in the last month) rules out even the narrow internal use case without an owner-approved hardening plan.
...
- **VERIFIED — official source** (GitHub repo banner + Discussion #6727, fetched 2026-08-19): The `FlowiseAI/Flowise` repository was **archived by the owner on August 13, 2026** and is now **read-only**. A banner at the top of the repo reads: *"Flowise has been archived. Refer to [Future of Flowise]"* (Discussion #6727).
- **VERIFIED — official source** (Discussion #6727, fetched 2026-08-19): Official timeline —
