...
| Analytics | usage_events shadow + reports | unknown | Postgres/reporting (existing `get_daily_usage()` function, currently unused), Metabase (AGPL-3.0 self-hosted; no free Cloud tier, confirmed 2026-08-18) | hosted analytics | low — read-only DB role against existing Postgres (Metabase path only) | `usage_events` table already exists and is already populated; the aggregation function already exists too — the only gap is that nothing calls it | low if read-only role enforced (Metabase); none for the custom-route option | per project | ~$0 for a custom route (reuses existing app); ~$7-10/mo for self-hosted Metabase, no free Cloud tier | Build minimally first — wire the existing `get_daily_usage()` into a small owner-only route; Metabase verdict is `FUTURE`, revisit only if the need broadens past this one table | Metabase/usage_events Deep Audit 2026-08 confirmed: data and aggregation logic both already exist unused; Metabase itself is not yet justified |
