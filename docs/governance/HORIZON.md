...
| Decision Hub | ACTIVE | STATIC AUDIT COMPLETE | YES | Stage 0–1 and formula-safety fixes are statically verified; DH-S1 CLOSED / STATIC VERIFIED; DH-S2 access policy drift OPEN / OWNER DECISION REQUIRED; DH-S3 fail-closed reads STATIC VERIFIED; DH-S4 partial-persistence observability OPEN | REMEDIATION REQUIRED; runtime/deployment evidence NOT ESTABLISHED | resolve `DECISION_HUB_ACCESS_POLICY`, then align code/tests/docs and remediate partial-persistence observability |
