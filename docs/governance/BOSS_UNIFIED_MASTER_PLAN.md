...
| DECISION_HUB | Decision Hub | Trust/Decision loop | H3 | ACTIVE | PROGRAM STATUS DRIFT FOUND — CALLBACK / RECORD-SCOPE REMEDIATION REQUIRED — RUNTIME NOT ESTABLISHED. Stage 0-1 and formula-safety fixes are merged/static. DH-S1 formula safety is CLOSED / STATIC VERIFIED; DH-S2 is DOC/POLICY DRIFT / REMEDIATION REQUIRED: the shared-capability decision supersedes stale Owner-only wording; DH-S3 fail-closed reads are STATIC VERIFIED; DH-S4 stakeholder/event partial-persistence observability is OPEN. DH-CB-01–DH-CB-09 record callback/record-scope gaps. Protected CLI paths remain positive evidence; the full authorization layer is not claimed broken. Feature flag remains off; no runtime/deployment evidence established. | MERGED | Bounded callback/record-scope remediation: propagate identity/tenant/domain context, scope enumeration and matching, validate Inbox↔Decision links, harden ignore, add callback regression coverage; then remediate partial-persistence observability. Do not activate before runtime evidence gates or broaden permissions | true | false | false | 2026-08-30T00:00:00Z | `BUG_AUDIT_LOG.md` Decision Hub callback/record-scope audit; `ROADMAP.md`; `docs/governance/HORIZON.md`; `docs/specs/DECISION_HUB_MASTER_SPEC.md` |
