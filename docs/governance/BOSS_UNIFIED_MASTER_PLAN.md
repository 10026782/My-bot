...
| BUG-099a — הרחבת `_NAME_STOP` (אוצר-מילים תיאור-נכס) | Core/Security | H0 | **אומת שמספיק, טרם מומש.** בדיקה ישירה: הוספת "קומה"/"חדרים"/"ראשונה"..."שישית"/"מרפסת"/"מטבח"/וכו' ל-`_NAME_STOP` גורמת ל-`_extract_name_from_window()` **לדחות נכון** את שני מקרי ה-garbage-name שנצפו, דרך מנגנון הדחייה שכבר קיים בקוד (`if any(w in _NAME_STOP for w in name.split()): continue`) — **לא נדרשת ארכיטקטורה חדשה**. Regression-safe: ה-control case (שם תקין) ממשיך לעבוד. | תיקון קטן, בטוח, מוכן למימוש + טסטים. |
