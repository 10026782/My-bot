...
| Approval Policy Single Source (F52→C83) | Core/Security | H0 | ✅ C83 סגור ומאומת — `event_bus.ACTIONS_REQUIRING_APPROVAL` הוא alias טהור ל-`tool_registry.TOOLS_REQUIRING_APPROVAL`, לא רשימה עצמאית (ר' ROADMAP.md §C83). **באותה בדיקה אומת מחדש (לא נפתח חדש) BUG-077** (פתוח, P0, קובץ/מנגנון אחר — `action_gateway.py`/`propose_action()`): `_write_one_lead:354` מעביר `requires_approval=False` כפרמטר מת שהכתיבה בפועל עוקפת. **תיקון לטעות בבדיקה ראשונית של סשן זה:** זה **לא** דורמנטי מאחורי `FEATURE_AUTO_CAPTURE` — Tier 3 (`_handle_mixed_batch`, `core/lead_candidate_handler.py:615-621→746`) קורא ל-`_write_one_lead` ללא שום בדיקת flag, על כל דיקטציית owner/staff עם batch מעורב-ביטחון. חי בפרודקשן היום. | לתקן את BUG-077 עצמו (`tool_registry.get_tool_meta()` fail-closed לפני שורה 468 ב-`action_gateway.py`) — לא ליצור מעקב כפול; ר' `BUG_AUDIT_LOG.md` BUG-077 לפרטים המלאים ולתוספת מ-06/07/2026. |
...
C-CORE-01 lead_memory Persistence · C-CORE-02 Airtable Write Queue (תלוי V3 מאומת) · C-CORE-04 N10 Rollback אוטומטי · **C-CORE-05 BUG-077 fix** — `propose_action()` חייב לאמת `requires_approval` מול `tool_registry.get_tool_meta(tool_name).requires_approval` (fail-closed, לפני שורה 468 ב-`core/action_gateway.py`), במקום לסמוך על ה-keyword שהקורא מצהיר; **חי בפרודקשן היום** דרך Tier 3 dictation (`core/lead_candidate_handler.py`, ללא תלות ב-`FEATURE_AUTO_CAPTURE`) — לא "לא דחוף", ר' `BUG_AUDIT_LOG.md` BUG-077.
