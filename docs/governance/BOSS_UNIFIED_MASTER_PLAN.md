...
| DECISION_HUB | Decision Hub | Trust/Decision loop | H3 | ACTIVE | ACTIVE / STATIC AUDIT COMPLETE / REMEDIATION REQUIRED / RUNTIME NOT ESTABLISHED. Stage 0-1 and formula-safety fixes are merged/static. DH-S1 formula safety is CLOSED / STATIC VERIFIED; DH-S2 is DOC/POLICY DRIFT / REMEDIATION REQUIRED: the shared-capability decision supersedes the stale Owner-only wording; DH-S3 fail-closed reads are STATIC VERIFIED; DH-S4 stakeholder/event partial-persistence observability is OPEN. Focused negative authorization/data-scope tests pass; broader callback/record-scope review and runtime evidence remain open. Feature flag remains off; no runtime/deployment evidence established. | MERGED | Remediate partial-persistence observability; complete broader callback/record-scope review; do not activate before runtime evidence gates or broaden permissions | true | false | false | 2026-08-30T00:00:00Z | `BUG_AUDIT_LOG.md` Task 9 Decision Hub policy gate; `ROADMAP.md`; `docs/governance/HORIZON.md`; `docs/specs/DECISION_HUB_MASTER_SPEC.md` |
