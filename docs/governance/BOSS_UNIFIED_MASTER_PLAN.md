...
| 🔴 BUG-098 hotfix — `_FOLLOWUP_WORDS` substring match + `last_lead_candidate_batch` TTL | Core/Security | H0 | **דחוף, טרם ממומש.** Root cause מאומת במלואו (קוד + רשומת Airtable חיה) — ר' `BUG_AUDIT_LOG.md` BUG-098. תגובת "✅ בוצע" כוזבת נשלחה **פעמיים ברצף** לאותה הודעה חוזרת; ליד ("גיל חביב") לא נוצר בכלל, בלי tool attempt/log. Scope מוגבל בכוונה: (1) `_FOLLOWUP_WORDS` — word-boundary matching במקום substring (המילה "קומה" מכילה "ומה"); (2) `last_lead_candidate_batch` — TTL + flag "resolved", זהה למנגנון הקיים כבר ב-`pending_lead_preview`/`active_lead_candidate` (1800s). **לא כולל** את שלושת ה-follow-up items שלמטה — הפרדה מכוונת כדי לא לערבב hotfix דחוף עם שינויים רחבים יותר. | לממש את שני התיקונים המוגבלים, טסטים (כולל regression על "קומה"), PR נפרד, ואז production verification. |
