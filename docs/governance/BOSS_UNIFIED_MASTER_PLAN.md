...
| DECISION_HUB | Decision Hub | Trust/Decision loop | H3 | ACTIVE | STATIC REMEDIATION COMPLETE / RUNTIME NOT ESTABLISHED. Stage 0-1 and formula-safety fixes are merged/static. DH-S1 formula safety is CLOSED / STATIC VERIFIED; DH-S2 is DOC/POLICY DRIFT / REMEDIATION REQUIRED: the shared-capability decision supersedes stale Owner-only wording; DH-S3 fail-closed reads are STATIC VERIFIED; DH-S4 stakeholder/event partial-persistence observability is OPEN. DH-CB-01–DH-CB-09 are CLOSED / STATIC VERIFIED with direct callback and scope regressions. Protected CLI paths remain positive evidence; the full authorization layer is not claimed broken. Feature flag remains off; no runtime/deployment evidence established. | MERGED | Remediate DH-S4 partial-persistence observability; then pursue separately gated runtime evidence. Do not activate before runtime evidence gates or broaden permissions | true | false | false | 2026-08-30T00:00:00Z | `BUG_AUDIT_LOG.md` Decision Hub callback/record-scope remediation; `ROADMAP.md`; `docs/governance/HORIZON.md`; `docs/specs/DECISION_HUB_MASTER_SPEC.md` |
