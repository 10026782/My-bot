...
**Current-state and relationship rules:** `CURRENT STATE` is determined only by
`origin/main` at a recorded Truth Reset SHA. Open PRs, branches, local commits
and drafts are `PROPOSED / OPEN / NOT YET CURRENT` and never override it.
Read-only discovery/audit/gate conclusions that materially change status, phase,
Next, dependency, blocker or architecture must be persisted in a canonical
current-state document, even when no code PR is produced. Program relationships
are valid only when explicitly marked as `IMPLEMENTATION_OF`, `DEPENDS_ON`,
`BLOCKED_BY`, `MERGED_INTO` or `CONTINUES`; otherwise the relationship is
...
| UNIFIED_APPROVAL_ACTIONGATEWAY | Unified Approval / ActionGateway | canonical action lifecycle, approval and execution boundaries | H0 | ACTIVE | lifecycle and approval paths merged; controlled-staging cutover canary passed 02/09/2026 on `a45f304` | RUNTIME_VERIFIED | post-cutover soak/monitoring; unrelated capability canaries remain separate | false | false | false | 2026-09-02T12:40:41+03:00 | Render logs: `crm_create_deal` canary `09e1fd02-619f-401f-b419-aeb5cc5a815a`; execution `01cc8515-9364-4333-837b-d44370ec6603` |
...
| SCHEMA_DATA_CONTRACTS | Schema / Data Contracts | static and live schema/data-contract consistency | H0 | ACTIVE | Track #2/#3 remains reconciled. S2A is merged and live-schema verified. S2B's three narrow primitives are CODE_DONE + STATIC_VERIFIED on the PR branch, with no schema or record mutation and no cutover. | MERGED | review/merge S2B; writer/read switch remains separately gated | true | false | false | 2026-09-03T00:00:00+03:00 | git:main:23ab957;PR#1187;PR#1189;docs/evidence/COMMERCIAL_SCHEMA_S2A_20260903.md;docs/evidence/COMMERCIAL_S2B_MUTATION_PRIMITIVES_20260903.md |
| COMMERCIAL_COMPLETION_WRITER | Canonical Commercial Completion Writer | pure completion contracts and gated Commercial Schema V2 progression | H1 | ACTIVE | Foundation and S2A are merged. S2B universal Organization, Charge, and Charge-required actual-movement Payment primitives are CODE_DONE + STATIC_VERIFIED on the PR branch; completion engine and readers remain unwired. | MERGED | review/merge S2B; separately approve any completion-engine or writer/read cutover | true | false | false | 2026-09-03T00:00:00+03:00 | git:main:23ab957;PR#1187;PR#1189;docs/evidence/COMMERCIAL_S2B_MUTATION_PRIMITIVES_20260903.md |
...
| N18 | N18 Canonical Write Infrastructure | canonical Draft → Approval → Write → Evidence write framework generalized off the Lead build; Lead is the first and currently only consumer | H1 | ACTIVE | Read-only Truth Reset (30/08/2026) confirms no static implementation gap. Phase 1 (canonical `create_lead()` service) and Phase 2 (shared primitives: `structured_command.py`, `draft_flow.py`, narrow `turn_result.py`) closed 20-21/08/2026. Phase 3 Slice 1 (Telegram preview routed through `create_lead()`) closed 27/08/2026 (PR #1043, `3de2dcf`); dead-flag cleanup closed same day (PR #1044, `a4bd730`). Phase 4's Telegram approve/cancel buttons slice closed 28/08/2026 (PR #1065, `2484f3c`) — confirmed on `origin/main`, `test_n18_phase4_telegram_buttons.py`/`test_n18_slice1_lead_preview.py`/`test_n18_draft_dispatch_unification.py` passing. `lead_capture.py` confirmed already routing through `create_lead()`. Remaining work is runtime-only: `WHATSAPP_CANONICAL_LEAD_WRITE`/`VOICE_CANONICAL_LEAD_WRITE` default OFF pending owner activation + live canary. Phase 4's broader structured UX Contract (beyond the buttons slice), Phase 5 (Attribution), and Phase 6 (Downstream + Learning) are accepted-deferred future phases — not current blockers. A second entity consumer (Tasks/Payments/Deals/Contacts/Expenses) was never scheduled as part of N18's defined phases and requires a separate owner-approved slice; it must not be represented as a current N18 implementation gap. | MERGED | owner-gated WhatsApp/Voice canonical-write canary | true | false | true | 2026-08-30T00:00:00Z | git:main:2484f3c;PR#1065;git:main:3de2dcf;PR#1043;git:main:a4bd730;PR#1044;test_n18_phase4_telegram_buttons.py;test_n18_slice1_lead_preview.py;test_n18_draft_dispatch_unification.py |
...
| F52_UNIFIED_USER_MESSAGES | F52 Unified Approval Runtime — Unified User Messages | Message UX / Approval Runtime | H0 | ACTIVE | R3.2, R4, R4.1 and R6.1–R6.6 are merged/static; R7.1 and R7.2 are MERGED / STATIC VERIFIED (`3c45a87`, `1ff1cee`); F52 remains an active implementation program and is explicitly `IMPLEMENTATION_OF: UX-01` | MERGED | Continue the next gated R7 adapter slice; deployment/runtime verification remains separate | true | false | false | 2026-08-30T00:00:00Z | git:main:3c45a87;git:main:1ff1cee;PR#1102;PR#1103;docs/architecture/f52-unified-approval-runtime/rollout/SINGLE_SPEAKER_APPROVAL_UX_UNIFIED_TELEGRAM_WHATSAPP_PLAN.md |
...
| DECISION_HUB | Decision Hub | Trust/Decision loop | H3 | ACTIVE | STATIC COMPLETE / RUNTIME NOT ESTABLISHED. Stage 0-1 and formula-safety fixes are merged/static. DH-S1 formula safety is CLOSED / STATIC VERIFIED; DH-S2 is DOC/POLICY DRIFT — CLOSED: Decision Hub is a shared capability with scoped-data authorization, not an Owner-only product gate; DH-S3 fail-closed reads are STATIC VERIFIED; DH-S4 stakeholder/event partial-persistence observability is CLOSED / STATIC VERIFIED. DH-CB-01–DH-CB-09 are CLOSED / STATIC VERIFIED with direct callback and scope regressions. Structured persistence outcomes prevent false full success. Protected CLI paths remain positive evidence; the full authorization layer is not claimed broken. **Correction 01/09/2026 (`BOSS_CURRENT_STATE.md` TR-21, read-only Render env verification):** `FEATURE_DECISION_HUB` is actually `true` in live Render env — the "feature flag remains off" claim below was accurate for the code default only, not the deployed configuration. This does not change the STATIC COMPLETE / RUNTIME NOT ESTABLISHED disposition — a functional canary (`/decision new` → `/decision status`) still has not been run — but the flag-activation step this row's "Next" column implies is already satisfied. | MERGED | Run the owner-side functional canary (`/decision new` → `/decision status`) directly — the flag prerequisite is already met, no separate activation decision is pending | true | false | false | 2026-08-30T00:00:00Z | `BUG_AUDIT_LOG.md` Decision Hub DH-S2 reconciliation; `ROADMAP.md`; `docs/governance/HORIZON.md`; `docs/specs/DECISION_HUB_MASTER_SPEC.md`; `BOSS_CURRENT_STATE.md`:TR-21 |
| MEDIA_LAYER_F16 | Media Layer (F16) | Media/Context loop | H4 | ACTIVE | קוד ממוזג. **תוקן 01/09/2026 (`BOSS_CURRENT_STATE.md` TR-21, אימות Render env בקריאה בלבד):** `FEATURE_MEDIA_UPLOAD`/`FEATURE_VOICE_NOTES` הם בפועל `true` ב-Render — הניסוח "flag off" למטה נכון רק לברירת המחדל בקוד, לא לתצורה שפרוסה בפועל. שער ה-upload הבסיסי פעיל; זה נפרד מ-MoneyPrinterTurbo/M5 שנשארים staging-only ודורשים את שערי ה-hash/path/publishing-off הנפרדים למטה בכל מקרה. מאז הסטטוס הקודם: MoneyPrinterTurbo נשאר staging-only; Media Probe POC, Artifact Contract v1, StoredArtifact MIME support, Gateway readiness docs ו־fail-closed gateway canary harness מוזגו עד `6a0ba6a`. אין MPT production activation. | MERGED | הדגלים הבסיסיים כבר פעילים — קנרי אמיתי (הודעת קול + תמונה אחת דרך הזרימה הבסיסית) אפשרי כעת ללא החלטת activation נוספת. עבור MPT/M5 בלבד: artifact/hash/path validation, publishing-off invariant, deployed-SHA evidence, ו־rollback/gate checklist עדיין נדרשים לפני activation. | true | false | false | 2026-08-21T00:00:00Z | git:main:6a0ba6a; `BOSS_CURRENT_STATE.md`:TR-21 |
...
| UX_01_UNIFIED_BOSS_EXPERIENCE | UX-01 — Unified BOSS Experience | Product/UI (H6 Product UI) | H6 | ACTIVE | Implementation in progress through F52 / Single-Speaker Approval UX; R7.1 and R7.2 are MERGED / STATIC VERIFIED (`3c45a87`, `1ff1cee`); UX-01 remains the canonical identity and F52 is explicitly `IMPLEMENTATION_OF: UX-01` | MERGED | Continue the next gated R7 channel-adapter sequence; program completion remains pending; deployment/runtime verification remains separate | true | false | false | 2026-08-30T00:00:00Z | git:main:3c45a87;git:main:1ff1cee;ROADMAP.md;docs/architecture/f52-unified-approval-runtime/rollout/SINGLE_SPEAKER_APPROVAL_UX_UNIFIED_TELEGRAM_WHATSAPP_PLAN.md |
...
### Horizon 3 — Decision Hub Shared Capability
H3.1 מצב קיים (Stage 0/0.5/0.6 merged flag-off; Stage 1 Trust Layer merged not-verified; Stages 2-4 לא התחילו)
H3.2 לפני הדלקה: capability permission check, tenant/data-scope checks, action-specific authorization, Airtable fields, multi-select, Source Reliability UI, negative authorization/data-scope tests, **+ מיזוג/אימות בפרוד של תיקון BUG-DH-03/04**
H3.3 החלטת C60
