...
| **J. Observability closure (TC10)** | `turn-coordinator-full/TC10_OPERATIONAL_VERIFICATION_HARNESS.md` | Verification harness + rollout/rollback gates | **IMPLEMENTATION COMPLETE (10/08/2026)** — `scripts/run_isolated_regression.py`/`scripts/regression_matrix.py`/`scripts/staging_identity.py` (new), `scripts/verify_tc9_staging.py` (new), `scripts/verify_tc8_staging.py` fixed (no longer runs the full regression matrix against real staging — root cause of the BUG-122 contamination the TC8 handoff described). Not a runtime layer — no ActionGateway/TC7/TC8/TC9/F14/router/approval-policy code touched | N/A — tooling only, no flags | Isolated regression: **local evidence only** this session (39/39 callback hardening, 8/8 PR-0C, 11/11 BUG-158, 21/21 full matrix, stable across 2 repeated runs) — do not read as RUNTIME_VERIFIED yet: an earlier version of this same runner regressed 2 files (over-broad credential override), caught by this PR's own CI run and CodeRabbit review, root-caused and fixed within the PR (see harness doc §6.1/§8); a fresh real-CI run against the corrected commit had not landed as of this writing. Staging runtime (TC9 canary, TC8 PG checks): **not yet run — no session in this thread holds real staging secrets** | TC8 (H, MERGED), TC9 (I, MERGED) | Confirm this branch's real CI run is green on the corrected "TC10 isolated regression matrix" step; then run `scripts/verify_tc9_staging.py` + `scripts/verify_tc8_staging.py` against real staging and attach evidence |
