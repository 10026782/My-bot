...
| BOSS_CONTEXT_LIBRARIAN_PHASE_0 | BOSS Context Librarian (mandatory session-bootstrap index) | Documentation / developer tooling only | H0 | ACTIVE | מוזג ל-main (עשרות PRs, כולל תחזוקה אוטומטית שוטפת); `AGENTS.md` מגדיר אותו כ-bootstrap חובה-ידני לכל development agent, ו-`.github/workflows/context-librarian-reconcile.yml` מריץ reconciliation אוטומטי אחרי כל push ל-main (פותח PR לתחזוקה גבולית, נכשל בבירור על OWNER_DECISION_REQUIRED). עדיין index/governance layer בלבד — production code לא מייבא אותו, אינו מקור אמת מקביל (גבול שנשמר, ר' `docs/context_librarian/README.md`). | MERGED | אין שלב-הבא בודד ומתוזמן — תחזוקה שוטפת (auto-maintenance PRs). מ-ROADMAP.md N17 (סעיף 6, נשאר פתוח): multi-session coordination טרם תוכנן; dogfooding עצמי טרם הושלם (רק תכנון VCM מוזג). | true | false | false | 2026-08-18T10:08:28Z | git:main:6d51b4f |
...
| U1_UNDERSTANDING_LAYER | U1 — Understanding Layer Architecture Decision | Core/Architecture | H0 | OWNER_DECISION | **🟡 עדיין פתוח, ממתין להחלטת בעלים.** BUG-102/103/104 מיפו מנגנוני-הבנה קיימים בקוד (`core/reasoning_entity.py`+`leads_adapter.py`/`decision_adapter.py`). מאז הרישום המקורי: BUG-104 Phase 1 חיבר בפועל קורא read-only חדש ל-`leads_adapter.py` (`core/leads_reasoning_projection.py` → `tma_api.py`'s `GET /api/leads/<id>`, מאחורי `FEATURE_CORE_REASONING_LEADS_STATE`, כבוי כברירת מחדל) — כך שהטענה המקורית "0 קוראים חיצוניים" אינה מדויקת יותר; זהו עדיין projection קריא-בלבד, לא "שכבת הבנה כללית" רחבה. VERIFIED IN PROD לפי `BUG_AUDIT_LOG.md` (17/07/2026, לא שוחזר עצמאית בסשן הזה). ההחלטה הארכיטקטונית הרחבה — להרחיב/לחבר את הקיים או לבנות מנגנון נפרד — **עדיין לא הוכרעה**. **חוסם UX-01 (למטה).** | UNKNOWN | להכריע לפני שממשיכים לדון בשכבת-הבנה חדשה. השלב הבא המתועד ב-`BUG_AUDIT_LOG.md` (אם ההכרעה תומכת בהרחבה): Phase 2A — Current State Policy, audit+SPEC בלבד, ללא קוד. ר' `BUG_AUDIT_LOG.md` BUG-104. | false | false | true | 2026-08-16T00:00:00Z | BOSS_UNIFIED_MASTER_PLAN.md:§3.5:U1_UNDERSTANDING_LAYER:migration-no-explicit-evidence |
