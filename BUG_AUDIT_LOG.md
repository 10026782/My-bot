...
- **Merged:** לא עדיין
- **Deployed:** לא עדיין
- **Verified בפרודקשן:** לא עדיין — אין קנרית חיה עדיין (`LEAD_TO_DEAL` כבוי כברירת מחדל; הפעלה + `/dealfromlead <שם/טלפון קיים>` היא הקנרית הבאה).
- **סטטוס:** Fixed (STATIC_VERIFIED) — ממתין ל-merge + deploy + הפעלת flag + קנרית production חיה.
