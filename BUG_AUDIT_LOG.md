...
## BUG-129 — `_extract_name_from_window()` מקבל "זיהיתי" (מתוך הטקסט-תבנית של הבוט עצמו) כשם-ליד, במקום השם האמיתי שמופיע באותה הודעה — 🔴 נרשם, לא תוקן (טרם התקבלה החלטה על תיקון)
...
- **כיוון תיקון אפשרי (לא הוחלט, לא מומש כאן):** להוסיף מילות-הניסוח-העצמי של הבוט (כגון "זיהיתי", וכל שאר פעלי-גוף-ראשון שמופיעים בתבניות-התשובה שלו) ל-`_NAME_STOP` — תיקון צר, עם regression tests (למשל: הטקסט המדויק מהדגימה הזו → השם הנכון `"משה חביב"`). לא נבדק אם יש תבניות-תשובה נוספות עם אותה בעיה (למשל מילים אחרות שגם הן גוף-ראשון-פעלים בתבניות אחרות של הבוט).
- **סטטוס:** 🔴 נרשם, Contract Chain אומת ישירות בקוד (כולל trace שלב-אחר-שלב) — **לא תוקן**, לפי בחירת המשתמש. אין claim על תיקון/deploy עד להנחיה נוספת.
...
## BUG-135 — `core/ingress_classifier.py`: מילות פקודה/ציטוט-עצמי חסרות מ-`_NAME_STOP` הפיקו שם ליד מזויף — ✅ תוקן

- **תאריך:** 22/07/2026.
- **מקור:** דווח ע"י הבעלים כחוסם בדיקות RP5. שני repro-ים נפרדים, נעקבו ישירות ב-`core/ingress_classifier.py`:
  1. ציטוט-עצמי: הדבקת התבנית שהבוט עצמו שולח — `"📋 זיהיתי ליד: *משה חביב* (0501112222)"` — זוהתה כליד בשם *זיהיתי* (0501112222) במקום *משה חביב*.
  2. פקודות מחיקה: `"תמחק איש קשר 0536272637"` (וריאציות `מחק`/`הסר`) — אין שם אמיתי בטקסט כלל, אך זוהתה כליד בשם *תמחק איש קשר* (0536272637).
- **Root Cause (אומת ישירות בקוד):**
  1. `_HEBREW_NAME_RE.finditer()` תפס את "זיהיתי ליד" כמאץ' ראשון (ריצה רציפה של מילים עבריות) ו-"משה חביב" כמאץ' שני, נפרד. "ליד" כבר stop-word, "זיהיתי" לא — כך שהסגמנטציה בודדה "זיהיתי" (≥4 תווים) כמועמד. `_extract_name_from_window()` **חוזרת מיד** במאץ' הראשון שעובר ולידציה (`for`-loop עם `return`) — המאץ' השני, הנכון, אף פעם לא נבדק.
  2. למודול הזה **אין** טיפול ייעודי לכוונת-מחיקה (ה-intent היחיד שקיים בכלל ל-delete הוא `Intent.DELETE_TASK` ב-`core/router/intent_router.py`, שום דבר ל-contacts/leads) — כך שטקסט מחיקה נופל לאותה חילוץ שם+טלפון גנרית כמו יצירה/עדכון. "תמחק"/"מחק"/"הסר" לא היו stop-words כלל, אז הפועל שרד כחלק מ"השם".
- **תוקן:** `_NAME_STOP` הורחב ב-"זיהיתי" ובקבוצת פעלי-המחיקה "תמחק"/"מחק"/"הסר" (מראה קבוצת פעלי ה-DELETE_TASK הקיימת ב-router). זה לבדו עדיין השאיר "איש קשר" (הישרד ≥4 תווים, ללא "תמחק") כמועמד שקרי — "איש"/"קשר" **לא** הפכו ל-stop-words גורפים בכוונה (המשתמש אישר לשמר את ההתנהגות הקיימת: `"תוסיף איש קשר בדיקה טלפון X"` חייבת להמשיך לחלץ `"איש קשר בדיקה"` verbatim, ראה AskUserQuestion בסבב הזה). נוסף `_GENERIC_NAME_PHRASES = frozenset({"איש קשר"})` — reject רק על ה-phrase המדויק כשהוא כל מה ששרד, לא כשמילה נוספת (כמו "בדיקה") שורדת לצידו.
- **בדיקות:** `test_bug135_command_verb_name_stop.py` (10 assertions חדש) — שני ה-repro, regression לשם אמיתי אחרי `תעדכן`/`תמחק`, ורגרסיה מפורשת שה-"איש קשר בדיקה" הקיים לא השתנה. Full sweep (`test_*.py`, `smoke_tests.py`, `test_integration.py`, `compileall -q .`) — נקי, ללא רגרסיה (כולל `test_bug099b1_no_name_validation.py`/`test_bug099a/b/c`, `test_bug096/098/101/111/116` — כל טסטי ה-ingress_classifier הקיימים).
