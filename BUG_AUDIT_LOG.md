...
- **Deployed:** כן (מוסק מהקנרית החיה למטה — `LEAD_TO_DEAL` הופעל ב-Render וה-deploy כלל את הקוד הזה בפועל; לא נבדק ישירות מול Render dashboard commit hash).
- **Verified בפרודקשן:** ✅ כן — קנרית חיה: הבעלים (Eli) שלח `/dealfromlead גבי קניקובסקי` בטלגרם, BOSS השיב **הודעה אחת בלבד** — "הפעולה הושלמה: פתיחת עסקה: גבי קניקובסקי" (אין הודעה שנייה קפואה "ממתין לאישור", מאמת גם את DETERMINISTIC-QUEUE-DUPLICATE-REPLY-GUARD). אומת ישירות מול Airtable (base `app4bcgoX7t0HUVnm`, לא רק לפי הודעת הבוט): נוצרה **בדיוק רשומת עסקה אחת** בטבלת עסקאות (Deals), `recs4uu0IpTAad22y` ("גבי קניקובסקי", נוצרה 2026-09-02T16:37:51Z) — שדה **Origin Lead שלה מקושר בפועל** לרשומת הליד `rec67YjUdFVDlLesr` ("גבי קניקובסקי", domain=`recruitment`, טלפון `0525253545`, status `active`) — ה-inlet עובד קצה-לקצה, לא רק סטטית. שדה Domain בעסקה מציג `Recruitment` (casing חי של Airtable) — מאמת גם שה-slug הקנוני (`recruitment`) מהליד עבר נכון דרך לוגיקת ה-casing הקיימת (`resolve_live_select_value`) בלי טבלת תרגום שנייה. אין כפילות רשומות תואמות שם בטבלת העסקאות.
- **סטטוס:** ✅ VERIFIED IN PROD — merge + deploy + קנרית production חיה מאמתים: `/dealfromlead` יוצר עסקה אחת עם Origin Lead מקושר נכון, ללא הודעה כפולה.
