...
- **סטטוס:** ✅ תוקן ומאומת ב-staging (עם ההסתייגות המדויקת לעיל). **Verified בפרודקשן:** חלקי — המסלול העסקי (תיקון #1+#2+#3 יחד, קצה-לקצה) אומת חי 30/07/2026; הממיר הספציפי `sheets_append→airtable_add` (תיקון #1 בבידוד) עדיין לא נצפה חי.
