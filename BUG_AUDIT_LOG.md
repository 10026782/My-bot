...
- **✅ תוקן (26/07/2026, אומת ישירות מ-`main`) — root cause tracking נפרד, לא Claude:** PR #461 (`codex/pm460-drive-fail-closed`, commit `70093f0`, ממוזג ל-`main` ב-`719bb86`) מימש `_sheets_payload_to_airtable()` (`core/action_gateway.py`) — המרה אמיתית של payload בצורת Sheets ל-`table`/`fields`, כולל allowlist שדות (`FIELD_MAP`) וטבלה-קנונית (`TABLE_ALIASES`), עם `CanonicalizationError` (fail-closed) כשההמרה לא בטוחה (טבלה לא-מוכרת/שדות לא-מאושרים/`row_data` positional ללא ממיר מפורש). נקרא מ-`resolve_canonical_call()` (שמחליף כעת את הקריאה הישירה ל-`resolve_canonical_tool()` שתועדה כאן כשורש-הבעיה). **מאומת ב-git log/diff ישירות (לא רק PR status)** — לא נבדק מול production/staging traffic אמיתי בסבב הזה. שלושת ה-contracts הפגומים הקיימים (`ee5ffb68`/`00b84046`/`aa74244a`) עדיין `pending` בהיסטוריה — התיקון אינו רטרואקטיבי, ראו המלצת ניקוי ידנית.
- **סטטוס מעודכן:** ✅ תוקן ב-`main` (PR #461). `BUG_AUDIT_LOG.md`/`CHANGE_CONTROL_LOG.md` לא עודכנו ע"י ה-PR עצמו (לא נגע בקבצי governance) — זו ההשלמה.
...
- **✅ תוקן (26/07/2026, אומת ישירות מ-`main`) — הסתירה למעלה נפתרת:** PR #460 (`codex/july24-sampling-blockers`, commit `006506d`, ממוזג ל-`main` ב-`0c06f4c`) שיכתב את ענף ה-reject ב-`_handle_approval_callback_impl()` — הוא מוצא כעת את ה-contract הקנוני לפי fingerprint ו**קורא ל-`ActionGateway.reject(contract_id, rejected_by=...)`** בפועל, כולל בדיקת-transition (`_reject_after.status == "rejected"`) לפני שהמשתמש מקבל אישור-ביטול. זה בדיוק "צורת התיקון המוצעת" שתועדה למעלה. מסביר את התרחיש-4 "לכאורה סותר": ה-fix כבר היה ב-staging (PR #466 שם) בזמן שהדוח נכתב, מה שהופך את ה-PASS שם לאמיתי, לא לחלופת-מסלול. **מאומת ב-git log/diff ישירות** — לא נבדק מול production/staging traffic אמיתי בסבב הזה.
- **סטטוס מעודכן:** ✅ תוקן ב-`main` (PR #460). `BUG_AUDIT_LOG.md`/`CHANGE_CONTROL_LOG.md` לא עודכנו ע"י ה-PR עצמו — זו ההשלמה.
...
- **✅ תוקן (26/07/2026, אומת ישירות מ-`main`):** PR #460 (`006506d`, ממוזג ב-`0c06f4c`) הוסיף helper אחיד להודעה-סופית-יחידה ב-`app.py` — משווה `origin_chat_id` מול `cq.message.chat.id`: אם זהים, `edit_message_text()` הוא הערוץ היחיד; אם שונים, `send_message()` נפרד לצ'אט המקורי. מיושם **בשני הענפים** (approve success/failure **וגם** reject) — מקיף יותר מהצעת-התיקון המקורית שתועדה כאן (שדיברה על approve בלבד). **מאומת ב-git log/diff ישירות** — לא נבדק מול production/staging traffic אמיתי בסבב הזה.
- **סטטוס מעודכן:** ✅ תוקן ב-`main` (PR #460), כולל reject. `BUG_AUDIT_LOG.md`/`CHANGE_CONTROL_LOG.md` לא עודכנו ע"י ה-PR עצמו — זו ההשלמה.
...
## BUG-147 — `dispatch_tool`'s `airtable_add` case returns a plain string on blocked/violation paths instead of the structured `{ok, ...}` contract — 🔴 נרשם, לא תוקן
...
- **תיקון root cause (26/07/2026) — ההשערה למעלה הייתה שגויה, שורש אמיתי אחר נמצא:** בדיקה מדוקדקת יותר גילתה ש-`_leads_write_blocked_message()` (`tools/airtable_security.py:81-90`) **וגם** `TenantScopeViolation`'s ההודעה (`tools/airtable_security.py:124-126`) **כבר מתחילות ב-"❌"** — כלומר שני המסלולים ב-BUG-147's השערה המקורית (שורות 261/319 ב-`tools/dispatcher.py`) **כן** סווגו נכון כ-`failed` ע"י `verify_execution()`'s בדיקת ה-"❌"-prefix (שורה 423-424 ב-`core/anti_hallucination.py`), **לפני** שהגיעו בכלל לענף "expected structured result dict". הם **לא** התסמין שדווח.
  **השורש האמיתי, מאומת ישירות:** `tools/dispatcher.py:156-162` — `dispatch_tool()` מריץ את `action_validator.validate_action()` **לפני** ה-`match name:` הראשי, לכל tool. כשהתוצאה `ActionBlocked` (למשל presence-check שמזהה פרמטרים חסרים — `action_validator.py:100-102`/`156-162`), `dispatch_tool` מחזיר את `validation.reason` הגולמי — מחרוזת שאלה כמו `"לאיזו טבלה?"` **בלי שום "❌" prefix**. בדיוק payload פגום כמו זה של BUG-143 (`row_data`/`sheet_name` במקום `table`/`fields`) חסר את **שני** הפרמטרים החובה של `airtable_add` (`_REQUIRED["airtable_add"] = ["table", "fields"]`, `action_validator.py:25`) — מייצר בדיוק את התנאי הזה, ומגיע ל-`verify_execution()`'s ה-branch של "plain string, לא ❌, tool רשום כ-write" → בדיוק ההודעה שדווחה. מאומת ב-Python session ישיר (לא רק grep): `dispatch_tool("airtable_add", {"row_data":[...], "sheet_name":"Tasks"}, ...)` מחזיר `validation.reason` הגולמי; `verify_execution()` עליו מפיק את ההודעה המדויקת מהדוח.
- **✅ תוקן (26/07/2026) — Patch A יושם:** `tools/dispatcher.py:156-162` — כש-tool ברשימת `core.anti_hallucination._EVIDENCE_VALIDATORS` (structured write tools, כולל `airtable_add`) נחסם ע"י `action_validator`, מוחזר עכשיו `{ok: False, tool, user_message}` במקום `validation.reason` הגולמי. Tools לא-רשומים (read-only וכו') ממשיכים להחזיר מחרוזת רגילה, ללא שינוי. **בנוסף** (hardening, לא-הכרחי אך תואם לעיקרון "כל executor מחזיר מבנה אחיד"): שני המסלולים המקוריים בהשערה (`LeadsDirectWriteBlocked`/`TenantScopeViolation` בתוך `case "airtable_add"`) גם הם עודכנו להחזיר את אותו `_tool_result(...)` shape במקום `str(e)` — לא בגלל שהיו שבורים, אלא לעקביות. **לא תוקן ע"י Patch A:** `airtable_update`'s case לא נבדק/נגע (אותו gate כללי ב-`dispatch_tool` כן חל עליו, אבל הענפים הפנימיים של `case "airtable_update"` לא נבדקו לתסמינים דומים). בדיקת רגרסיה: `test_bug147_dispatcher_structured_error_shape.py` (8/8 עברו, מורץ מול `main` הנוכחי אחרי PR #460/#461).
- **הערה חשובה — לא כפילות עם PR #460/#461:** בניגוד ל-BUG-143/144/145 (למעלה), אף PR קיים ב-`main` לא נגע ב-`tools/dispatcher.py` — אומת ב-`git log --oneline -- tools/dispatcher.py` (רק merge-commit ישן, ללא שינוי-תוכן עד ל-Patch A). זה patch עצמאי, לא חופף לעבודת codex.
- **סטטוס מעודכן:** ✅ תוקן. root cause אמיתי אומת (לא ההשערה המקורית) + תיקון יושם ונבדק מול `main` הנוכחי (אחרי PR #460/#461).
