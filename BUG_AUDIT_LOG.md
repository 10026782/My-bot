...
## BUG-105 — פורמט טלפון בין-לאומי עם מקף — כבר מכוסה ב-origin/main

- **תאריך:** 12/07/2026
- **משפחה:** Family C (קלט לא-חד-משמעי → השמטה שקטה בלי דגל) — אותה משפחה כמו "אורי כדורי" (ראה `LEAD_CAPTURE_QUALITY_AUDIT` הקודם).
- **מקור:** נמצא כפער שיורי, מחוץ ל-scope, תוך כדי מימוש BUG-101a/b/c. הרשומה המקורית הייתה stale: ב-`origin/main` הנוכחי (`38d9226`) `_PHONE_RE` כבר כולל חלופה בינלאומית עם שני מפרידים פנימיים.
- **קובץ:** `core/ingress_classifier.py` — `_PHONE_RE`
- **כיסוי בפועל:** regex לפני/אחרי זהה לחלוטין: `r"\+?972[-\s]?\d{2}[-\s]?\d{3}[-\s]?\d{4}"`. מתקבלים: `+972 54-211-6211` → `0542116211`, `+972 53-396-8395`, `050-123-4567`. נדחים: `+972 54-21-6211`, `+972 54-211-621`, `050-12-34567`, `+972 54-211-62115`.
...
- **תוקן בקוד בסשן הזה:** לא — לא היה שינוי מקומי; הכיסוי כבר קיים ב-`origin/main`.
- **PR:** לא נפתח.
- **Merged:** ✅ כבר קיים ב-`origin/main`; לא נפתח merge חדש.
- **Evidence שנשמר:** 204 בדיקות suites עברו ועוד 11 בדיקות מטריצת פורמטים; כולל BUG-101 — 19/19, BUG-111 — 50/50, BUG-096 — 29/29, BUG-094 — 31/31; `py_compile` עבר.
- **קבצים ששונו בסשן BUG-105:** אף קובץ. קיימים שינויים קודמים ולא קשורים ב-6 קבצים אחרים, שלא נגעתי בהם.
- **סטטוס:** ✅ מכוסה ב-`origin/main`; אין עבודה פתוחה ל-BUG-105. אין לייחס לבדיקה הזו את 6 הקבצים המלוכלכים שהיו ב-worktree.
...
- **עדכון (14/08/2026) — PR1 (Authority/Foundation) מוזג, טרם מחווט:** PR #623 מוזג ל-`main` (שלושה commits: `f8c6332`, `4210acf`, `1066762`). מוסיף `marketing_fact_authority.py` (`ProtectedFact`), `marketing_creative_templates.py` (registry סגור), `marketing_creative_renderer.py` (`CreativeProposal`) — אך `cmd_marketing.py::_create_demand_and_generate_ideas()` (המסלול שבו הבאג בפועל) עדיין לא נוגע במודולים האלה בשלב זה. קיום foundation ≠ תיקון חי.
- **עדכון (14/08/2026) — PR2 (Cutover) מוזג ומחווט — סוגר את וקטור ה-`creative_ideas` באופן דטרמיניסטי:** PR #645 ("BUG-164 PR2: wire bounded creative authority into F23 creative_ideas", commit `07e5491`, מוזג ל-`main` דרך `e9d1ca8`) מחליף את הזרימה החופשית ("רעיון 1:/2:/3:" free-text + `_parse_three_ideas` regex) בתפריט סגור: `_build_creative_proposal_instruction()` מציע ל-AI רק צירופי `(angle_id, opening_style, cta_style)` מתוך `allowed_combinations()` ו-`fact_order` מתוך `ProtectedDemandFacts.renderable()`; התשובה נפרסת ב-`parse_creative_proposal()` ומעוצבת ב-`authority_filter_and_render()` (PR1, ללא שינוי) לפני `save_creative_ideas()`. **אין נתיב fallback לטקסט AI גולמי** — כל דחייה (JSON פגום/שדה לא מוכר/צירוף לא מורשה/עובדה לא-renderable/authority לא-CANONICAL) חוסמת שמירה כליל. 13 טסטים ייעודיים (`test_bug164_pr2_creative_cutover.py`) + מלוא ה-regression suite (PR1: 10/10, 10/10, 21/21; `smoke_tests.py`; `test_integration.py`) עברו. **זהו התיקון הדטרמיניסטי לוקטור שבו נצפה הבאג בפועל (`creative_ideas`).** סטטוס: IMPLEMENTED — קוד+טסטים בלבד, טרם live regression מול Anthropic אמיתי בפרודקשן.
- **עדכון (15/08/2026) — הגנת-עומק נוספת ל-3 סוגי המשימה הנותרים (`creative_review`/`ad_package`/`publishing_plan`), שעדיין free-text ולא כפופים ל-authority gate של PR2:** `compose_brief()` ב-`marketing_brief_composer.py` (המשמש את כל 4 סוגי המשימה, כולל אלו) לא הכיל שום מקבילה ל-`_INVENT_CONTRACT` שכבר קיים ב-`compose_production_handoff()` — שום הנחיה מפורשת ל-AI לשמר עובדות כמותיות (`Goal`, לוחות זמנים, כמויות) מ-`[פרטי הדרישה]` ללא שינוי בזמן ניסוח יצירתי. `GLOBAL_RULES` אוסר רק "להבטיח" דבר שלא אושר — לא אוסר במפורש "לשנות/לעגל/להמיר" ערך כמותי שכן סופק. **הערה: `creative_ideas` עצמו כבר לא עובר דרך `compose_brief()` — ראה עדכון PR2 למעלה — כך שהתיקון הזה הוא הגנת-עומק (defense-in-depth) לשלושת סוגי המשימה האחרים, לא תיקון לוקטור שבו נצפה הבאג.**
  - **תוקן ב-commit:** `b375602` — הוספת `_DEMAND_FIDELITY_RULE` ל-`compose_brief()`, מוזרק כ-`[כלל מחייב]` לכל task_type שעובר דרכה (לא נוגע ב-`compose_production_handoff()`/`_INVENT_CONTRACT` הקיימים).
  - **תוקן ב-branch:** `claude/continue-f23-dmbgr7`
  - **Verification ראיה (קוד בלבד):** `python3 marketing_brief_composer.py` + `python3 test_marketing_brief_composer.py` (5/5) + `python3 test_marketing_orchestrator.py` + `python3 test_cmd_marketing_status.py` + `python3 smoke_tests.py` (PASS) + `python -m compileall -q .` (PASS) — כולם עברו על commit `b375602`. `scripts/run_isolated_regression.py --repeat 2`: STABLE, 32/33 בשתי הריצות; הכשל היחיד (`test_phase_4b0_1b_concurrency.py`, תרחישי R15/R21) שוחזר זהה על `origin/main` נקי (worktree, `7d8cfce`, ללא השינוי) — pre-existing, לא regression מהתיקון. לא נבדק live מול Anthropic אמיתי (התיקון הוא prompt-level; אינו ניתן להוכחה דטרמיניסטית ב-unit test).
- **סטטוס:** `creative_ideas` — 🟢 **תוקן דטרמיניסטית (PR2, מוזג, `e9d1ca8`)**, טרם live-verified בפרודקשן. `creative_review`/`ad_package`/`publishing_plan` — 🟡 CODE DONE, NOT VERIFIED (הגנת-עומק ברמת prompt בלבד, ענף `claude/continue-f23-dmbgr7` טרם מוזג).

---

### BUG-165 — ActionGateway shadow evidence misclassifies accepted external submission
- **דווח:** 16/08/2026
- **דווח על ידי:** owner, בעקבות MPT Phase 1 staging E2E
- **מסך / מודול:** `core/action_gateway.py` evidence projection/shadow path;
  `core/external_execution_boundary.py`
- **תיאור:** לאחר approval של `external_execution.submit`, ה־ActionContract
  הקנוני עבר ל־`executed` עבור accepted submission, אך shadow evidence רשם
  `outcome_unknown`. באותו E2E ה־ExternalExecutionJob הדורבילי עבר
...
- **תאריך:** 09/08/2026
- **Scope:** `crm_add_contact()` ו-`lead_conversion.py::convert_lead_to_contact()` עוברים דרך `find_or_create_contact()` בלבד.
- **סמנטיקה:** `created` ו-`existing` ממשיכים; `ambiguous`, `invalid` ו-`lookup_error` נעצרים fail-closed ללא עדכון Lead.
...
## F16 Media — Remediation Slice 2 (F16-M2, F16-M3) — MERGED — CLOSED / STATIC VERIFIED (PR #1131, merged `3366baf`, corrected in Slice 3)
...
### F16-M2 — MIME allowlist, fail-closed — CLOSED / STATIC VERIFIED (merged)
...
- **Verdict:** CLOSED — STATIC VERIFIED (PR #1131 merged `3366baf`).

### F16-M3 — durable failure trace — CLOSED / STATIC VERIFIED (merged)
...
|----|--------|
| F16-M1 | **CLOSED — STATIC VERIFIED** |
| F16-M2 | **CLOSED — STATIC VERIFIED** *(PR #1131 merged — see post-merge update below; superseded)* |
| F16-M3 | **CLOSED — STATIC VERIFIED** *(PR #1131 merged — see post-merge update below; superseded)* |
| F16-M4 | **CLOSED — STATIC VERIFIED** |
| F16-M5 | OPEN (unchanged) |
| F16-M6 | OPEN (unchanged) |
| F16-M7 | OPEN (unchanged) |
...
**F16 = CORE REMEDIATION COMPLETE (merged) / LOW-RISK CLEANUP REMAINS / RUNTIME NOT ESTABLISHED.** *(Post-merge correction, recorded in Slice 3 below: PR #1131 merged `2026-08-30` as `3366baf`, superseding this section's original "MERGE PENDING" wording, which was accurate only at the time this section was written.)* All four owned static findings from the original audit (M1, M2, M3, M4) are now statically verified fixed; M5/M6/M7 remain open low-risk cleanup; M8 remains cross-track. **F16 overall is not closed** — no runtime/deployment/production claim is made.

## F16 Media — Remediation Slice 3 (F16-M5, F16-M6, F16-M7) — STATIC CLOSED / RUNTIME NOT ESTABLISHED

**Truth Reset:** `origin/main` at `327ded5552a3ea98405f8b0ff2a5954b902bf457`. Confirmed PR #1131 (F16-M2/M3) is merged and reachable (`git merge-base --is-ancestor` true; merge commit `3366baf`). M1/M2/M3/M4 all `CLOSED — STATIC VERIFIED` on this SHA. Scope: only F16-M5/M6/M7. M1–M4 not reopened; M8 not absorbed; no runtime/deployment work performed.

### F16-M5 — content-hash gap for non-TMA sources — BLOCKED / OWNER DECISION REQUIRED (unchanged)

- **Original finding:** no field in `MediaFileFields` holds a content hash; only TMA-sourced assets get one (folded into `Logical Media Key` as `tma:sha256:<hex>`) — Telegram/Twilio/Meta sources are keyed purely by provider ID, no hash computed/stored/verified for them at all. Severity LOW-MEDIUM. Classified explicitly as "not a proposed hash architecture," recorded status "RECORDED, NOT TO BE ACTED ON WITHOUT A SEPARATE DECISION."
- **Re-verified still present:** `media_handler.py`'s `logical_media_key()` is unchanged — `tma` still hashes file bytes; `telegram`/`twilio`(`whatsapp`)/`meta`(`whatsapp_meta`) still return `f"{namespace}:{file_id}"` with no hash. No `MediaFileFields` hash column exists.
- **Why this is not a cleanup item:** closing it would require computing and storing a content hash for three additional sources — a real change to `logical_media_key()`'s identity contract (what "the same media" means for retry/dedup), a new schema field, and a decision on backward compatibility with every already-persisted non-TMA `Logical Media Key` value. That is exactly the "architectural or owner-policy decision" this slice's own change-discipline rule requires stopping for.
- **Disposition:** **BLOCKED — OWNER DECISION REQUIRED.** No code changed. Remains open, unchanged from its original recorded status; not converted into a hash architecture without a separate decision.

### F16-M6 — `GoogleDriveArtifactStore.put()` / `ArtifactStore` Protocol drift — CLOSED / STATIC VERIFIED

- **Original finding:** `core/google_drive_artifact_store.py:43` `put()` omitted the `mime_type` parameter present in the `core/artifact_store.py:26` `ArtifactStore` Protocol; confirmed non-load-bearing (no polymorphic dispatch across `ArtifactStore` implementations — `core/moneyprinterturbo_adapter.py`'s `_configured_artifact_store()` is statically wired to `GoogleDriveArtifactStore()`). Severity LOW, STATIC GAP (interface drift, no runtime impact).
- **Re-verified still present:** signature was still `put(self, *, path, identity, metadata)` on `origin/main` before this fix; the sole live caller (`core/moneyprinterturbo_adapter.py:154`) never passed `mime_type`.
- **Remediation:** added `mime_type: str | None = None` to `GoogleDriveArtifactStore.put()`, matching the Protocol exactly. Wired it in (not just accepted-and-ignored): `resolved_mime_type = mime_type or "video/mp4"` now drives both the Drive `body["mimeType"]` and the `MediaFileUpload(..., mimetype=...)` call, preserving the exact current hardcoded behavior when the parameter is omitted (the only way the sole live caller uses it today). No new subsystem, no dispatch logic added, no other `ArtifactStore` implementation touched.
- **Tests:** `test_google_drive_artifact_store.py` — 2 new tests (`test_put_accepts_mime_type_matching_artifact_store_protocol` asserting an explicit `mime_type` flows into the Drive upload body; `test_put_default_mime_type_unchanged_when_omitted` asserting the default stays `video/mp4`), plus all 8 pre-existing tests in that file re-run and passing unchanged. `test_moneyprinterturbo_adapter.py` (the sole caller) re-run — 11/11 passed, confirming no regression to the existing caller.
- **Disposition:** **CLOSED — STATIC VERIFIED.**

### F16-M7 — stale `MediaPersistenceState` docstring — CLOSED / STATIC VERIFIED

- **Original finding:** `airtable_schema.py:727-729` `MediaPersistenceState`'s docstring still said "foundation-only in PR1. No runtime media flow reads or writes them until the durable lookup/reconciliation PRs land" — `media_handler.py` already reads/writes all five states throughout `handle_file_upload()`. Severity LOW, DOC DRIFT.
- **Re-verified still present:** docstring unchanged on `origin/main` before this fix; re-confirmed all five states (`PENDING`, `DRIVE_UPLOADED`, `ASSET_PERSISTED`, `PARTIAL`, `FAILED`) are actively read/written across 10 call sites in `media_handler.py`, including the `FAILED` state (`media_handler.py:763`, drive-upload-failure path) and the M3 durable-trace fallback (`PARTIAL`, merged in PR #1131).
- **Remediation:** replaced the stale docstring with a one-line, currently-true statement. Pure documentation change; zero behavior change; no test required (nothing executable changed) beyond a compile check.
- **Disposition:** **CLOSED — STATIC VERIFIED.**

### F16-M8 confirmation

Unchanged. Remains **CROSS-TRACK REFERENCE, NOT A NEW FINDING** — the CI test-invocation gap (4 pytest-style media test files silently executing 0 tests under `python3 <file>.py`) is still owned by the pre-existing #8/#9 CI-wiring concern, not F16. No remediation performed here; ownership not duplicated or reassigned.

### Verification run

```
python3 -m py_compile airtable_schema.py core/google_drive_artifact_store.py test_google_drive_artifact_store.py media_handler.py   # OK
python3 -m pytest -m "not integration and not airtable and not live" test_google_drive_artifact_store.py test_f16_m1_meta_media_host.py test_f16_m2_mime_allowlist.py test_f16_m2_voice_caller_mime_contract.py test_f16_m3_durable_trace.py test_audit3_i3_media_contract.py test_c02_c04_finding1_remediation.py test_c02_c04_finding2_pr2.py test_c02_c04_finding2_pr3.py test_c02_c04_finding3_remediation.py test_c02_c04_findings_7_8_remediation.py test_f52_g2_voice_edit_replacement.py test_f15_idempotency_retry.py test_f5_voice_idempotency_retry.py test_whatsapp_media.py --tb=short -q
  # 77 passed
python3 test_c90_structured_file_capture.py   # 45 passed, 0 failed
python3 test_pr0c_writer_migration.py         # 16 passed, 0 failed
python3 test_media_layer.py                   # 33/33 passed
python3 media_handler.py                      # self-test OK, unchanged output
python3 smoke_tests.py                        # PASS: all smoke checks passed
python3 -m pytest test_moneyprinterturbo_adapter.py -q --tb=short   # 11 passed (sole caller of the changed put())
```

No new pre-existing-failure baseline check was needed this slice — `test_provider_portability_envelope1d.py` is untouched by this diff (same file, same content as prior slices).

### Ledger status after Slice 3
...
| TR-10 | No Deal/Payment TMA surface → Admin spec + map §2 → **MARK AS SAME TRACK AS TR-03** → **OPEN, API/surface missing** | Schema/Data Contracts; no duplicate track |
| TR-11 | No task lifecycle API, dispatcher tenant gap, divergent creation paths → Admin spec → **ADD OPEN ITEM** → **OPEN, backend contract** | Tasks/TMA; lifecycle + tenant-scope design |
...
### BUG-LEAD-DOMAIN-FURNITURE-NOT-A-DOMAIN — "furniture_import" removed as a Lead domain; correct domain is "import"
- **דווח:** 02/09/2026 (בעלים, תגובה ישירה על BUG-LEAD-DOMAIN-LABEL-GAP לעיל, תוך כדי אותה עבודה)
- **דווח על ידי:** בעלים: "הענף הנכון הוא ייבוא (IMPORT) ולא רהיטים — שזו רק דוגמא למוצר, ולא הענף בעצמו."
- **מסך / מודול:** `core/lead_service.py` (`CANONICAL_LEAD_DOMAINS`, `_LEAD_DOMAIN_LABELS`), `core/noninteractive_lead_cutovers.py` (`create_furniture_inbound_lead()`)
- **תיאור:** מיד לאחר תיקון BUG-LEAD-DOMAIN-LABEL-GAP (שהשלים תווית עברית ל-`furniture_import` כאילו הוא דומיין קנוני לגיטימי), הבעלים תיקן את ההנחה הבסיסית עצמה: `furniture_import` **אינו** דומיין עסקי — "רהיטים" הוא רק דוגמת מוצר בתוך עסק הייבוא, לא ענף/דומיין נפרד. `CANONICAL_LEAD_DOMAINS` כלל את `furniture_import` כאחד מ-8 הדומיינים הקנוניים, ו-`core/noninteractive_lead_cutovers.py::create_furniture_inbound_lead()` (הקורא היחיד שכתב בפועל רשומת Lead עם דומיין זה — מהפאנל הדטרמיניסטי הייעודי `furniture_lead_funnel.py`) כתב `domain="furniture_import"` במפורש לכל ליד מהערוץ הזה.
- **חקירת scope לפני תיקון:** אומת בקוד (לא ניחוש) ש-`furniture_import` מופיע בשני **מושגים נפרדים ולא-קשורים** בקוד:
  1. כערך ב-`CANONICAL_LEAD_DOMAINS` (הדומיין העסקי של Lead) — **זה הפגם שתוקן**.
  2. כערך "Demand Type" (בורר פרופיל שיווקי, לא דומיין) ב-`cmd_marketing.py::DEMAND_TYPES`, `marketing_domain_profiles.py::PROFILES`, `marketing_fact_authority.py` — מושג **נפרד לגמרי**, מתועד ככזה כבר ב-`airtable_schema.py`'s `DEMAND_TYPE` comment ("בורר פרופיל, לא תחום"). **לא נגוע ולא שונה** — זה כבר היה נכון.
  גם `config.py::get_domain()` ו-`furniture_lead_funnel.py::DOMAIN` משתמשים במחרוזת `"furniture_import"` — אבל אלה משמשים אך ורק כ**מפתח ניתוב פנימי** (החלטה איזה handler יטפל בהודעה — הפאנל הייעודי או הסוכן הכללי), ולעולם לא נכתבים ישירות כערך Domain של רשומת Lead. **לא שונו** — שינוי שלהם היה שובר את הניתוב עצמו (ראה "שיקולי סיכון" למטה).
- **Severity:** Low — תיקון מודל-נתונים/סיווג, לא חוסם כתיבה, לא באג פונקציונלי (הליד עדיין נוצר בהצלחה); משפיע על סיווג הדומיין של לידים עתידיים מהפאנל הייעודי בלבד.
- **Root Cause:** בחירת מידול מוקדמת שהתייחסה לקו-מוצר ייעודי (מיטת עץ מלא, ראה `furniture_lead_funnel.py::PRODUCT = "triple_solid_oak_bed"`) כאילו הוא דומיין עסקי נפרד, במקום להכיר בו כמוצר בודד בתוך דומיין "ייבוא" הקיים כבר.
- **שיקולי סיכון שנבדקו לפני התיקון:** נבדק במפורש שה-Lead write path (`core/whatsapp_lead_cutover.py`, המסלול הכללי) **אף פעם לא** מקבל `domain_from_channel="furniture_import"` בפועל — `app.py`'s gate (`domain_from_channel != "furniture_import"`) מדלג עליו במכוון, כי הפאנל הייעודי כותב את הליד בעצמו דרך `create_furniture_inbound_lead()`. משמעות: שינוי ה-domain הנכתב בפועל **אינו** דורש כל שינוי בניתוב (`config.py`/`app.py`/`furniture_lead_funnel.py`) — רק בערך שנכתב סופית לרשומה. תוקן במינימום השטח הנדרש, בלי לגעת בהתנהגות ניתוב live.
- **תוקן ב-commit:** (ראה מיד לאחר merge, אותו commit כמו BUG-LEAD-DOMAIN-LABEL-GAP)
- **תוקן ב-branch:** `claude/fix-lead-domain-labels-completeness`
- **תיקון:** (1) `core/lead_service.py::CANONICAL_LEAD_DOMAINS` — הוסר `"furniture_import"`, נשארו 7 דומיינים קנוניים. (2) `core/lead_service.py::_LEAD_DOMAIN_LABELS` — הוסרה התווית התואמת שנוספה זה עתה. (3) `core/noninteractive_lead_cutovers.py::create_furniture_inbound_lead()` — `domain="furniture_import"` → `domain="import"`. (4) `test_noninteractive_lead_cutovers.py` — עודכן לאמת `domain == "import"`.
- **Verification:** חבילת regression מלאה ירוקה, אפס רגרסיות (כולל marketing tests שאינם נוגעים ב-domain כלל — מוודא שה-Demand Type concept לא נפגע): `test_noninteractive_lead_cutovers.py` (4/4), `test_n18_slice1_lead_preview.py` (6/6), `test_lead_service_phase1.py` (109/109), `test_draft_flow.py` (16/16), `test_n18_draft_dispatch_unification.py` (8/8), `test_structured_command.py` (11/11), `test_bug_lead_02_single_word_name_clarification.py` (17/17), `test_f52_g3_s7_structured_lead_capture.py`+`test_r4_1_optional_note.py`+`test_n18_phase4_telegram_buttons.py`+`test_marketing_fact_authority.py`+`test_marketing_creative_templates.py` (32/32, pytest). `python3 -m compileall -q .`, `smoke_tests.py`, imports (`app`/`tma_api`/`tools.dispatcher`/`core.lead_service`/`core.noninteractive_lead_cutovers`/`furniture_lead_funnel`), `tools/audit_turn_coordinator_bypass.py`, `tools/status_sync_validator.py`, `git diff --check` — כולם עברו.
- **Merged:** לא עדיין
- **Deployed:** לא עדיין
- **Verified בפרודקשן:** לא עדיין
...
### BUG-CRM-BYPASS-DEAL-DUPLICATE-REPLY — הודעה כפולה על אותה בקשת-אישור ל-crm_create_deal; ההודעה הלא-אינטראקטיבית נשארת "ממתין לאישור" גם אחרי שאושרה — רגרסיה בהעברה מ-create_task
- **דווח:** 02/09/2026 (בעלים, קנרית production חיה "בדיקת-קנרית 11": "צור עסקה בשם בדיקת-קנרית 11 בתחום Import")
- **דווח על ידי:** בעלים (Telegram). אבחון ראשוני שגוי מצדי (סתירת "הושלם" מול "ממתין") תוקן ישירות על ידי הבעלים: "זו לא סתירה יש הודעה כפולה כשממתין לאישור והשנייה נשארת סטטית גם אחרי שאושר והקודמת משתנה זה באג ישן של כפל הודעות שכבר דיווחתי עליו גם בסבבים הקודמים" ואז: "תוקן בעבר במסגרת UX עכשיו חזר כחלק מהרגרסיה של הסבבים שלנו".
- **מסך / מודול:** `app.py` (`_queue_deterministic_create_deal()`)
- **תיאור:** כשמשתמש (בדרך כלל הבעלים עצמו, שגם המבקש וגם המאשר) שולח בקשת create_deal מובנית, `_queue_approval_detailed_impl()` שולח הודעת Telegram **אינטראקטיבית** ל-`owner_chat_id` עם מקלדת אישור/ביטול inline (`bot.send_message(owner_chat_id, _pending_text, reply_markup=kb)`) — הודעה זו **כן** מתעדכנת במקום (`edit_message_text`) כשהבעלים לוחץ אשר/בטל, כי ה-callback_data שלה נושא את ה-action_id/contract_id. בנוסף, `_queue_deterministic_create_deal()` **תמיד** החזיר גם את `outcome["message"]` (טקסט פשוט, "יש פעולה שממתינה לאישור: ...") כתשובה נפרדת לבקשה המקורית — כשה-requester וה-owner הם אותו chat (המקרה הרגיל בבוט חד-בעלים זה), המשתמש רואה **שתי** הודעות על אותה פעולה. ההודעה השנייה (הטקסט הפשוט) אינה נעקבת בשום מקום לצורך עריכה, ולכן נשארת קפואה לנצח עם הניסוח "ממתין לאישור" גם אחרי שהפעולה כבר אושרה ובוצעה בפועל דרך ההודעה הראשונה.
- **Severity:** Medium — UX בלבד (לא security, לא כתיבה כפולה בפועל ל-Airtable — ה-contract עצמו נוצר פעם אחת). אבל מטעה: המשתמש רואה הודעה "ישנה" שממשיכה לטעון "ממתין לאישור" ללא הגבלת זמן אחרי שהפעולה כבר הושלמה, מה שנראה בדיוק כמו הבאג המדווח בטעות כ"סתירת completed/pending" בדיווח הראשוני.
- **Root Cause (מאומת, לא רק תיאורטי):** `app._queue_deterministic_create_task()` **כבר** נושא הגנה מפורשת לבדיוק המקרה הזה (`if outcome.get("owner_notified") and str(owner_chat_id) == str(chat_id): return ""` — מתועד גם ב-`test_first_pending_notification_failure_suppression.py` תחת השם `duplicate_reply_suppressed`, ותוקן בעבר במסגרת עבודת UX קודמת, כפי שהבעלים אישר). `app._queue_deterministic_create_deal()` — שנכתב במפורש "מראה את _queue_deterministic_create_task() בדיוק" (ראה ה-docstring שלו) — **לא כלל את ההגנה הזו בכלל**. זו לא בעיה ארכיטקטונית חדשה, אלא רגרסיה נקודתית בהעתקה: כשנוסף המסלול הדטרמיניסטי ל-Deal (BUG-CRM-BYPASS follow-up, 01/09/2026), ההגנה הספציפית הזו — שכבר הייתה קיימת ופתורה ב-Task — לא הועתקה יחד עם שאר התבנית.
- **תוקן ב-commit:** `715d518` (guard), `1244329` (מסמך ארכיטקטורה, נדרש לשער status-sync) — ראה merge commit `056f60c`.
- **תוקן ב-branch:** `claude/fix-crm-deal-duplicate-approval-reply`
- **תיקון:** נוסף ל-`_queue_deterministic_create_deal()` בדיוק אותו guard הקיים ב-`_queue_deterministic_create_task()`: אחרי קבלת `outcome` מ-`_queue_approval_detailed()`, אם `outcome.get("owner_notified")` אמת וה-`owner_chat_id` (מ-`OWNER_TELEGRAM_ID`/`ELIYAHU_CHAT_ID`/`DIGEST_CHAT_ID`) זהה ל-`chat_id` של המבקש — מוחזר מחרוזת ריקה (לא נשלחת הודעה שנייה) במקום `outcome["message"]`. כשה-requester **שונה** מה-owner (מקרה תיאורטי/עתידי, לא הבוט חד-הבעלים הנוכחי), ההודעה השנייה **כן** ממשיכה להישלח — היא לגיטימית שם, כי המבקש (שאינו ה-owner) צריך לדעת שהבקשה שלו נכנסה לתור, ורק ה-owner רואה את ההודעה האינטראקטיבית. נוספה גם השלמת `out_meta["reply_owner"]`/`out_meta["final_response_count"]` לענף היצירה, לעקביות עם `_queue_deterministic_create_task()`.
- **Verification:** קובץ regression חדש (`test_bug_crm_deal_duplicate_approval_reply.py`, 8 בדיקות) — מוכיח: (1) כשה-requester הוא ה-owner, ה-send השני מדוכא (מחרוזת ריקה), רק שליחת Telegram אחת קורית בפועל, וה-contract הפנימי עדיין נוצר פעם אחת בלבד; (2) כשה-requester **שונה** מה-owner, שתי ההודעות ממשיכות להישלח כצפוי (לא נשברה התנהגות לגיטימית); (3) הרצה מלאה דרך ה-parser האמיתי עם הטקסט המדויק מהקנריה ("צור עסקה בשם בדיקת-קנרית 11 בתחום Import") משחזרת ומוכיחה את הסגירה. `python3 -m compileall -q .`, `smoke_tests.py`, `tools/audit_turn_coordinator_bypass.py`, `tools/status_sync_validator.py`, `git diff --check`, ו-`test_bug_crm_bypass_create_deal_deterministic_route.py`, `test_first_pending_notification_failure_suppression.py`, `test_bug_crm_bypass_airtable_update.py`, `test_pr1_single_speaker_approval_ux.py`, `test_f52_status_reply_reconciliation.py`, `test_bug_approval_callback_hardening.py`, `core/router/test_router.py` — כולם ירוקים, אפס רגרסיות. **פוסט-merge:** אומת ישירות מול `origin/main` (`056f60c`) — `git show origin/main:app.py | grep duplicate_reply_suppressed` מציג את ה-guard בפועל ב-`_queue_deterministic_create_deal()`, לא רק ב-PR.
- **Merged:** כן — PR #1180, `origin/main@056f60c`.
- **Deployed:** כן (להנחה, לא אומת ישירות מול Render dashboard — ראה "Verified בפרודקשן" למטה שמספק ראיית התנהגות חיה התומכת בכך שה-deploy קרה).
- **Verified בפרודקשן:** ✅ כן — הבעלים סיפק צילום מסך שיחת Telegram חי המראה שלוש קנריות רצופות על אותה תבנית ("צור עסקה בשם בדיקת-קנרית N בתחום Import"): קנרית #10 (08:28) וקנרית #11 (12:05) — **לפני** ה-merge (09:35 UTC) — שתיהן מציגות בדיוק את הבאג המדווח: הודעת סטטוס ("❌.../✅ הפעולה הושלמה: ...") **ואז** הודעה שנייה נפרדת "יש פעולה שממתינה לאישור: פתיחת עסקה: בדיקת-קנרית N" — כלומר הכפילות אכן שוחזרה חיה בפרודקשן, כצפוי, לפני שהתיקון הגיע. קנרית #12 (12:40, לאחר ה-merge/deploy) מציגה **הודעה אחת בלבד** — "הפעולה הושלמה: פתיחת עסקה: בדיקת-קנרית 12" — ללא הודעת "ממתין לאישור" שנייה/קפואה בעקבותיה. זו בדיוק ראיית ה-RUNTIME_VERIFIED שה-PR ביקש ("Live canary ... should now leave exactly ONE Telegram message behind").
- **סטטוס:** ✅ VERIFIED IN PROD — merge + deploy + קנרית production חיה (#12) מאמתים שאחרי אישור נשלחת הודעה **אחת** בלבד, ללא שאריות "ממתין לאישור" קפואות. הראיה מבוססת על צילום מסך שהבעלים סיפק (לא נבדקה עצמאית מול Render dashboard commit hash).

---

### DETERMINISTIC-QUEUE-DUPLICATE-REPLY-GUARD — helper משותף + שער CI שישי, כדי שמחלקת הבאג של BUG-CRM-BYPASS-DEAL-DUPLICATE-REPLY לא תוכל לחזור על כתיבה חדשה
- **דווח:** 02/09/2026 (בעלים, ישירות בעקבות סגירת BUG-CRM-BYPASS-DEAL-DUPLICATE-REPLY): "כדי שלא נחזור על הסבבים הללו בכל בנייה מחדש נתעד סוג של טמפלט או תבנית יצירת כותב חדש? ... או יש דרך עדיפה?" — הוצעה ואושרה הגישה המועדפת: helper משותף (מבני) + שער CI, לא רק תיעוד (תיעוד כבר נכשל פעם אחת בדיוק כאן — ה-docstring של `_queue_deterministic_create_deal()` הצהיר "מראה את `_queue_deterministic_create_task()` בדיוק" ועדיין החסיר את ההגנה).
- **דווח על ידי:** בעלים, אישור מפורש להתחיל מ-"ה-shared helper וה-CI guard".
- **מסך / מודול:** `app.py` (`_finalize_deterministic_queue_outcome()` חדש + refactor לשלוש הפונקציות הקוראות), `tools/audit_turn_coordinator_bypass.py` (שער CI שישי חדש)
- **תיאור:** חולצה פונקציה משותפת אחת, `_finalize_deterministic_queue_outcome()`, מהזנב החוזר של שלוש פונקציות ה-`_queue_deterministic_*()` (`create_task`/`create_deal`/`task_update`) — כולל את הגנת `duplicate_reply_suppressed` (owner_notified + same-chat → מדכא הודעה שנייה). תוך כדי הבנייה התגלה **מופע שלישי, סמוי**, של אותה מחלקת באג בדיוק: `_queue_deterministic_task_update()` (המשמש `update_task`/`complete_task`) מעולם לא נשא את ההגנה הזו כלל — אותה חשיפה מבנית בדיוק כמו ב-Deal לפני BUG-CRM-BYPASS-DEAL-DUPLICATE-REPLY. נסגר באותו PR, לא נפתח כפריט נפרד.
- **Severity:** Medium (הכתיבה הקיימת) / Governance-critical (השער) — התיקון עצמו הוא UX בלבד (כמו קודמו); השער נועד למנוע חזרה על מחלקת-הבאג ב-writer עתידי, לא רק לסגור מופעים קיימים.
- **תוקן ב-commit:** (ראה מיד לאחר merge)
- **תוקן ב-branch:** `claude/deterministic-queue-duplicate-reply-guard`
- **תיקון:**
  1. `app._finalize_deterministic_queue_outcome(outcome, chat_id, out_meta, log_prefix, fallback_message)` — הפונקציה האחת שבה חי guard ה-duplicate-reply-suppression מעכשיו. `_queue_deterministic_create_task()`, `_queue_deterministic_create_deal()`, ו-`_queue_deterministic_task_update()` כולן עברו refactor להעביר את ה-`outcome` שלהן דרכה במקום לבנות תשובה בעצמן — ללא שינוי התנהגות לשני הנתיבים שכבר עבדו נכון, וסגירת הכפילות הסמויה ב-task_update.
  2. שער CI שישי, `DETERMINISTIC_QUEUE_DUPLICATE_REPLY_SUPPRESSION`, ב-`tools/audit_turn_coordinator_bypass.py` — באותו דפוס בדיוק כמו שער 5 (`PROTECTED_BUSINESS_TABLE_RAW_UPDATE`): `find_deterministic_queue_functions()` מחלץ (regex) כל `def _queue_deterministic_*(...)` מ-`app.py`, והשער מוודא שכל אחת קוראת ל-`_finalize_deterministic_queue_outcome(` בגוף שלה. **נבדק בפועל** (לא רק תיאורטית) — נוצר עותק זמני שבו `_queue_deterministic_create_deal()` חוזר לבנות את התשובה בעצמו (בדיוק צורת הרגרסיה המקורית); אומת שהשער נכשל ומצביע במדויק על הפונקציה הפגומה בלבד (לא על השתיים האחרות), אומת גם שהסרת ה-helper עצמו נתפסת, ואז שוחזר ואומת שהשער עובר שוב.
- **Verification:** `test_audit_turn_coordinator_bypass.py` — 6 טסטים חדשים (28/28 עם הקיימים). `test_bug_deterministic_queue_duplicate_reply_suppression.py` (קובץ חדש, 9 טסטים) — helper ברמת יחידה (owner כ-requester → מדוכא; requester אחר → לא מדוכא; `out_meta` רק כש-contract נוצר בפועל; fallback message) וה-e2e של `_queue_deterministic_task_update()` שנסגר. `python3 -m compileall -q .`, `smoke_tests.py`, `tools/audit_turn_coordinator_bypass.py`, `tools/status_sync_validator.py`, `git diff --check`, ו-`test_bug_crm_deal_duplicate_approval_reply.py` (8/8), `test_first_pending_notification_failure_suppression.py` (14/14), `test_bug_crm_bypass_create_deal_deterministic_route.py`, `test_turn_coordinator_task_runtime_integration.py`, `test_bug_crm_bypass_airtable_update.py`, `test_pr1_single_speaker_approval_ux.py`, `test_f52_status_reply_reconciliation.py` (51/51), `test_bug_approval_callback_hardening.py` (41/41), `core/router/test_router.py` (54/54) — כולם ירוקים, אפס רגרסיות.
- **Merged:** כן — PR #1182, `origin/main@c836124`. אומת ישירות: `git show origin/main:app.py | grep "def _finalize_deterministic_queue_outcome"` (שורה 1062) ו-`grep "_finalize_deterministic_queue_outcome("` (4 מופעים — ההגדרה + שלוש נקודות הקריאה בשורות 1175/1245/1282); `git show origin/main:tools/audit_turn_coordinator_bypass.py | grep "check_deterministic_queue_duplicate_reply_suppression"` (שורה 575).
- **Deployed:** לא אומת ישירות מול Render dashboard.
- **Verified בפרודקשן:** לא עדיין (אין דרך ישירה "לבדוק" שער CI בפרודקשן — הוכחת התוקף שלו היא ה-unit/regression tests + הבדיקה האמפירית שתועדה למעלה; ה"production verification" הרלוונטי היחיד הוא שהתנהגות ה-runtime של שלושת הנתיבים ממשיכה ללא שינוי, מה שיתאמת ממילא בכל שימוש הבוט הרגיל)
- **סטטוס:** ✅ CODE MERGED TO MAIN (מאומת ב-grep מול origin/main) — לא deployed/runtime-verified באופן עצמאי.

---

### LEAD-TO-DEAL-ORIGIN-LINK — למסלול הדטרמיניסטי של יצירת עסקה לא היה שום inlet ל-Origin Lead; עסקה מקושרת לליד הייתה אפשרית רק דרך הסוכן
- **דווח:** 02/09/2026 (בעלים, אודיט יזום לפער Lead→Deal, ללא קנרית production — נמצא לפני שנכתב, לא לאחר תקרית).
- **דווח על ידי:** בעלים — אודיט מפורט (7 סעיפים) הראה ש-`origin_lead_id` נתמך תקין ב-writer (`commercial_crm.create_deal`), ב-schema, וב-dispatcher (מסלול Agent), אבל למסלול הדטרמיניסטי (`core.router.router.parse_deterministic_create_deal` → `app._queue_deterministic_create_deal`, agent_calls=0) לא היה שום דרך לספק אותו — DeterministicDealParse לא נושא שדה כזה כלל, וה-payload שהפונקציה בונה קשיח ל-`{name, domain, owner_id}`. שאלת ארכיטקטורה נלווית: "איך נתאים לגישה שבנינו בסשן שלנו שיהיה הכל מסלול אחיד?" (התייחסות ל-DETERMINISTIC-QUEUE-DUPLICATE-REPLY-GUARD, האנטרי הקודם).
- **מסך / מודול:** `app.py` (`_queue_deterministic_create_deal()`), `lead_conversion.py` (פונקציה חדשה), `tools/audit_turn_coordinator_bypass.py` (שער CI שביעי חדש)
- **תיאור:** זה לא writer שבור — זה חור ב-inlet. הצעה ראשונית לחבר endpoint TMA חדש נבדקה ונמצאה שגויה: ל-TMA אין היום שום נתיב לקריאת כלי קנוני בשם (`crm_create_deal`) — רק כתיבות גולמיות `{"op":"post","table":...,"fields":...}` ישירות ל-Airtable (`_queue_or_owner_execute`/`_queue_tma_write_approval`), ו-`_queue_deterministic_create_deal()` עצמה בנויה סביב `chat_id`/`channel` טלגרם ושליחת הודעה אינטראקטיבית — קריאה לה מ-Flask route הייתה דורשת plumbing חדש לגמרי (Cross-Layer Planning Gate: FULL, לא SINGLE-LAYER). הוחלף בתקדים קיים ומתאים בקוד: `lead_conversion.py`'s `/convert` — פקודת owner מפורשת שמחפשת ליד לפי שם/טלפון, עם `chat_id`/`channel` טלגרם אמיתיים כבר קיימים.
- **Severity:** Medium/Feature-gap — לא היה תיעוד/מסלול לפעולה עסקית חשובה (Lead→Deal מקושר), לא regression בקוד קיים.
- **תוקן ב-commit:** (ראה מיד לאחר merge)
- **תוקן ב-branch:** `claude/lead-to-deal-origin-link`
- **תיקון:**
  1. `app._queue_deterministic_create_deal()` קיבלה פרמטר אופציונלי חדש `origin_lead_id: str = ""` — נכנס ל-payload רק כשסופק (`if origin_lead_id: deal_inputs["origin_lead_id"] = origin_lead_id`) — ללא שינוי לחלוטין לצורת ה-payload של המסלול הטקסטואלי הקיים ("צור עסקה בשם X בתחום Y", שאף פעם לא מספק את זה).
  2. פקודת טלגרם חדשה, `/dealfromlead [שם/טלפון]` (owner בלבד, דורש `LEAD_TO_DEAL`, כבוי כברירת מחדל). `lead_conversion.resolve_lead_for_deal(query)` — resolve בלבד, לא ביצוע — מחפש ליד יחיד (משתמש ב-helper משותף `_resolve_single_lead_by_query()` שחולץ מ-`convert_lead_to_contact()`, לא מעתיק את לוגיקת החיפוש בשנית), מחלץ name/domain (`domain` כבר slug קנוני בשדה הליד — `core.lead_service.resolve_domain_word()` מופעל כשער אימות/מעבר, לא ניחוש, בדיוק אותה טבלה משותפת שהמסלול הטקסטואלי כבר עובר דרכה — לא טבלת ניחוש שנייה), ומחזיר lead_id. `app.py`'s `cmd_deal_from_lead()` קורא ל-`_queue_deterministic_create_deal(..., origin_lead_id=lead_id)` — **אותו writer יחיד** שהמסלול הטקסטואלי כבר משתמש בו, לא endpoint/payload מקביל.
  3. שער CI שביעי, `CRM_CREATE_DEAL_SINGLE_PAYLOAD_BUILDER`, ב-`tools/audit_turn_coordinator_bypass.py` — מוודא (מבוסס AST, לא regex/ספירת מחרוזות — נתפס false positive אמיתי תוך כדי הבנייה: docstring שהזכיר את צורת הקריאה כטקסט כמעט נתפס כ"קריאה שנייה"; הבעלים הצביע בזמן אמת שספירת call-sites היא guard שביר) שכל Call node מהצורה `_queue_approval_detailed("crm_create_deal", ...)` חי בגוף הפונקציה של `_queue_deterministic_create_deal()` בלבד — לא במקום שלישי כלשהו. **נבדק בפועל**: נוצרה פונקציה שנייה שמציעה `crm_create_deal` ישירות (בדיוק צורת הרגרסיה); אומת שהשער נכשל ומצביע בשם על הפונקציה הפוגעת המדויקת, ואז שוחזר ואומת שהשער עובר.
- **Verification:** `test_lead_to_deal_origin_link.py` (קובץ חדש, 11 טסטים) — `_queue_deterministic_create_deal()` כולל/משמיט `origin_lead_id` כנדרש (ללא רגרסיה לצורת ה-payload הקיימת); `resolve_lead_for_deal()` על כל מקרי הקצה (flag כבוי, query חסר, 0/כמה לידים תואמים, ליד בלי שם, domain לא-מוכר/ריק נכשל-סגור בדיוק כמו `DeterministicDealParse`, מקרה הצלחה); הוכחה ש-`resolve_lead_for_deal()` ו-`convert_lead_to_contact()` חולקות את אותו helper חיפוש. `test_audit_turn_coordinator_bypass.py` — 4 טסטים חדשים לשער 7 (33/33 עם הקיימים), כולל טסט ייעודי לתפיסת ה-false-positive האמיתי (docstring/comment שמזכיר את צורת הקריאה לא נתפס בטעות). `python3 -m compileall -q .`, `smoke_tests.py`, `tools/audit_turn_coordinator_bypass.py`, `core/router/test_router.py` (54/54), `test_bug_crm_deal_duplicate_approval_reply.py` (8/8), `test_bug_deterministic_queue_duplicate_reply_suppression.py` (9/9), `test_bug_crm_bypass_create_deal_deterministic_route.py` — כולם ירוקים, אפס רגרסיות.
- **Merged:** כן — PR #1184, `origin/main@903ca48`. אומת ישירות (לא רק סטטוס PR): `git show origin/main:app.py | grep 'origin_lead_id: str = ""'` (שורה 1225) ו-`grep "def cmd_deal_from_lead"` (שורה 727); `git show origin/main:lead_conversion.py | grep "def resolve_lead_for_deal\|FLAG_DEAL"`; `git show origin/main:tools/audit_turn_coordinator_bypass.py | grep "def check_crm_create_deal_single_payload_builder"`; `git show origin/main:feature_flags.py | grep LEAD_TO_DEAL` — כולם קיימים בפועל.
- **Deployed:** ✅ כן — מאומת ישירות מלוג ה-runtime (לא רק הסקה מהתנהגות): `[LeadToDeal] /dealfromlead 'גבי קניקובסקי' → lead=rec67YjUdFVDlLesr deal_name=גבי קניקובסקי domain=recruitment by אליהו חזן` — שורת הלוג החדשה שנוספה ב-`cmd_deal_from_lead()` עצמה מופיעה בפלט production, כלומר הקוד הממוזג רץ בפועל.
- **Verified בפרודקשן:** ✅ כן — קנרית חיה: הבעלים (Eli) שלח `/dealfromlead גבי קניקובסקי` בטלגרם, BOSS השיב **הודעה אחת בלבד** — "הפעולה הושלמה: פתיחת עסקה: גבי קניקובסקי" (אין הודעה שנייה קפואה "ממתין לאישור"). אומת בשלוש שכבות עצמאיות, לא רק לפי הודעת הבוט:
  1. **לוג ה-runtime** (סופק על ידי הבעלים): `[DeterministicCreateDeal] agent_calls=0 ... created_this_turn=True` ו-`duplicate_reply_suppressed=true reason=owner_notification_already_sent` — מאמת גם את DETERMINISTIC-QUEUE-DUPLICATE-REPLY-GUARD; `[Dispatch] crm_create_deal | inputs={'domain': 'recruitment', 'name': 'גבי קניקובסקי', 'origin_lead_id': 'rec67YjUdF...'}` — `origin_lead_id` הגיע בפועל ל-dispatcher; `[ActionGateway] approved: ... payload_keys=['domain', 'name', 'origin_lead_id', 'owner_id']`; `[AUDIT:gateway] op=create table=עסקאות (Deals) record=recs4uu0IpTAad22y keys=['שם העסקה', 'Domain', 'Owner', 'שלב', 'Origin Lead']` — השדה נכתב בפועל ל-Airtable, לא רק הוצע; `[ApprovalTurnMetrics] agent_calls=0 contract_reads=1 final_responses=1 deterministic=True` — **תשובה אחת בלבד** מאומתת ברמת ה-metrics עצמה, לא רק תצפית ב-transcript.
...
### PHASE0-COMMERCIAL-CRM-UPDATE-AUTHORITY — closes the "no canonical update_deal()/update_payment_term()/update_payment() writer" gap BUG-CRM-BYPASS-UPDATE explicitly left open
- **דווח:** 22/09/2026 (TRACK — BUSINESSDRAFT PHASE 0, `docs/architecture/BUSINESSDRAFT_UX_CONTRACT_FREEZE_20260922.md` §16/§17/§27 Phase 0 — closing the commercial CRM UPDATE authority gap before BusinessDraft implementation begins).
- **דווח על ידי:** בעלים — הנחיית טראק מפורשת, כולל דרישה מפורשת ש-Payment יקבל גבול update/correction קנוני ולא יישאר CREATE-only.
- **מסך / מודול:** `commercial_crm.py` (writers חדשים), `tool_registry.py`, `tools/dispatcher.py`, `tools/schemas.py`, `action_validator.py`, `test_bug_crm_bypass_airtable_update.py` (עודכן, לא נכתב מחדש), `test_phase0_commercial_crm_update_authority.py` (חדש).
- **תיאור:** `BUG-CRM-BYPASS-UPDATE` (02/09/2026, למעלה בקובץ זה) תיעד במפורש: "בניגוד ל-Contacts, אין כותב-update קנוני כללי ('update_deal()') להפנות אליו" — וסגר את הפער באמצעות אימות allowlist+role+domain-canonicalization על ה-`airtable_update` הגנרי עצמו, בלי כותב ייעודי. הפער הזה נשאר פתוח מאז. Phase 0 סוגר אותו: שלושה writers קנוניים חדשים (`update_deal()`, `update_payment_term()`, `update_payment()`) ב-`commercial_crm.py`, כל אחד עם allowlist שדות סגור משלו (נגזר מ-, אך לא זהה ל-, allowlist ה-CREATE המקביל — שדות immutable-אחרי-יצירה כמו `Origin Lead`/`deal_id` של Payment Term מוצאים בכוונה). ה-`airtable_update` הגנרי עבור Deals/Payment Terms/Payments מפנה כעת ל-writers האלה (לא לכתיבה גנרית) — יש בדיוק כותב אחד לכל ישות, לא שני נתיבי אימות עצמאיים.
- **Payment Correction Model (הכרעה מפורשת, לא בהמשך ל-code evidence מוקדם):** הבעלים אישר שגבול update/correction קנוני ל-Payment נדרש. המודל שהוטמע: DIRECT_UPDATE למטא-דאטה לא-פיננסית בלבד (reference/method/notes/צד נגדי/document req+status) — שדות פיננסיים (amount/currency/direction/paid_at/charge/deal/status) חסומים במפורש, לא מתעלמים בשקט; תיקון פיננסי אמיתי דורש תשלום מקזז/חדש נפרד (primitive שלא נבנה ב-Phase 0). זו קריאה שמרנית של "נדרש גבול update/correction", לא מדיניות עסקית חדשה שהומצאה — היא מרחיבה את עקרון ה-immutability שכבר קיים בקוד (`calculate_payment()`'s docstring, `create_charge_payment()`'s אימות התאמה מול Charge).
- **Severity:** Governance-critical — סוגר פער סמכות שנשאר פתוח 20 יום; לא regression, השלמת מסלול.
- **תוקן ב-commit:** (ראה מיד לאחר merge)
- **תוקן ב-branch:** `claude/phase0-commercial-crm-update-authority`
- **תיקון:**
  1. `commercial_crm.py`: `update_deal()`, `update_payment_term()`, `update_payment()` — כל אחד: `fields: dict` (שמות kwarg כמו ב-create המקביל, לא שמות עמודה גולמיים), allowlist סגור fail-closed, resolve_live_select_value לשדות select (Deal בלבד, זהה ל-create_deal), אימות עקביות calc_type/fixed_amount/rate_pct+calc_basis על Payment Term (merge מול מצב נוכחי כש-partial update), חסימת שדות פיננסיים על Payment, בדיקת פורמט record-id על כל שדה מקושר, מוטציית Airtable יחידה (`airtable_patch`).
  2. `tool_registry.py`: `crm_update_deal`/`crm_update_payment_term`/`crm_update_payment` — role/approval/risk זהים בדיוק לכלי ה-CREATE המקביל (לא חלש יותר).
  3. `tools/dispatcher.py`: שני נתיבי כניסה לכל writer — (א) case ייעודי (`crm_update_deal` וכו') זהה בצורתו ל-case ה-CREATE המקביל; (ב) ה-`airtable_update` הגנרי המוגן (`_CRM_TABLE_ROUTING`) מפנה כעת ל-writer הקנוני במקום ל-`airtable_update()` גנרי — allowlist ה-CREATE עדיין שכבת סינון שמות-שדות ראשונה (fail-closed), ה-writer עצמו שכבה שנייה עצמאית.
  4. `tools/schemas.py`: סכמות JSON חדשות ל-3 הכלים (Agent-exposed, כמו ה-CREATE המקביל).
  5. `action_validator.py`: נוספו ל-`_REQUIRED`/`_SENSITIVE_TOOLS`/`_TOOLS_WITH_RECORD_ID` — פער נפרד שהתגלה תוך כדי (Unknown-tool gate עצמאי מ-tool_registry.py, לא היה מתועד קודם שצריך עדכון גם כאן לכלי CRM חדש).
- **Verification:** `test_phase0_commercial_crm_update_authority.py` (חדש, 40+ בדיקות — writers ישירות + נתיב dispatcher ייעודי + role gate). `test_bug_crm_bypass_airtable_update.py` עודכן (לא נכתב מחדש) לשקף שה-redirect מגיע כעת ל-writer קנוני, לא לכתיבה גנרית — כל הבדיקות הקיימות (allowlist, domain canonicalization, protected aliases, Tasks unrelated) עדיין ירוקות. רגרסיה מלאה: `smoke_tests.py`, `test_commercial_crm_dispatcher_wiring.py` (43/43), `test_bug_commercial_crm_dispatcher_bypass_closure.py`, `test_bug_crm_bypass_create_deal_deterministic_route.py`, `test_commercial_crm_owner_ssot.py`, `test_bug_organization_create_param_mismatch.py` (6/6), `test_diamond_business_fields_migration.py` (35/35), `test_diamond_d3_final_select_resolver.py` (22/22), `test_bug_diamond_enrichment_runtime_sweep.py` (50/50), `test_a32_enforcement.py` (6/6), `test_c53a.py` (50/50), `test_integration.py` (4/4), `core/router/test_router.py` (59/59), `python3 -m py_compile` על כל קובץ ששונה — כולם ירוקים, אפס רגרסיות. שער ה-registry↔dispatcher grep מ-`docs/governance/SECURITY_CHECKLIST.md` רץ ונקי.
- **Merged:** לא עדיין.
- **Deployed:** לא.
...
### MY-WORK-TASK-READ-CAP — My Work קורא רק את 100 רשומות ה-Tasks הראשונות (HIGH)
- **תאריך דיווח:** 24/09/2026 (נמצא במהלך תכנון Task recurrence; הבעלים סיווג HIGH וביקש לתעד בנפרד, בלי לקפל לתוך ה-PR של recurrence)
- **Root cause:** `tma_api.owner_my_work()` קורא `_at_list(Tables.TASKS, max_records=100)` בלי formula, בלי `paginate` ובלי `strict`. המסך עצמו כן מסתיר משימות `בוצע` (התנהגות נכונה: `_process_owner_tasks` מדלג עליהן, ולכן משימה שסומנה כבוצעה נעלמת מהמסך). אבל הסינון הזה קורה ב-Python רק **אחרי** הקריאה, ושאילתת ה-Airtable עצמה לא מסננת. לכן כל רשומה בטבלה, כולל שבוצעו, תופסת מקום בתקרת ה-100 לפני הסינון. מעבר ל-100 רשומות, משימות פעילות נחתכות בשקט מ-"עבודה שלי", בלי אזהרה. בנוסף, `strict=False` מחזיר `[]` בכל שגיאת Airtable, ולכן ענף ה-`except` (HTTP 500) לא נגיש, ותקלת קריאה מוצגת כ-"אין משימות" (200).
- **מצב חי (read-only, Airtable MCP, 24/09/2026):** בטבלה יש כעת **28** רשומות (ב-audit של BLANK-TASK-TITLES היו 135; 99 הרשומות הריקות כבר אינן בטבלה, והן לא נמחקו במסגרת עבודה זו). הבאג לכן **לטנטי** כרגע, ויתממש כשהטבלה תעבור 100 רשומות מכל סטטוס.
- **Remediation מוצע (הקטן והבטוח ביותר, לא מיושם):** בקריאה היחידה ב-`owner_my_work()`: (1) `formula` שמוציא `בוצע` בצד השרת (`{סטטוס}!='בוצע'`; שורות בלי סטטוס נשארות); (2) `paginate=True` עם תקרה מפורשת (למשל 1000) ו-`logger.warning` כשהתקרה מתמלאת, כך שחיתוך לעולם אינו שקט; (3) `strict=True`, כך שכשל קריאה מחזיר את ה-500 הקיים ולא רשימה ריקה. הסינון ב-Python נשאר (הגנה כפולה), וצורת התשובה וה-UX לא משתנים. עלות: בקשת Airtable אחת לכל 100 משימות פתוחות. בדיקות: הרחבת `test_my_work_end_to_end.py` (ה-fake של `_at_list` צריך לקבל `paginate`) כך שתוודא formula/paginate/strict, תוצאה של יותר מ-100 רשומות, ו-500 בכשל.
- **Merged:** לא.
- **Deployed:** לא.
- **Verified בפרודקשן:** לא.
