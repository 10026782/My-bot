...
## BUG-147 — RP5 marker-only messages strip to empty text, reaching Anthropic as an empty user message — ✅ VERIFIED IN STAGING / CLOSED
...
- **תסמין:** הודעה שהיא **רק** ה-marker עצמו — `[rp5-test:write-validation-400]`, בלי שום טקסט אחר — לאחר `begin_turn()`'s הסרת ה-marker הופכת למחרוזת ריקה `""`. שום שער קיים לא בדק את זה: `IngressClassifier` כבר מסווג `reason=empty_text` (Tier 5) אבל זו רק תוצאת-סיווג, לא short-circuit; ההודעה הריקה המשיכה עד ל-`messages = history + [{"role": "user", "content": clean_msg}]` ונשלחה בפועל ל-Anthropic, שדחה אותה: `Anthropic 400: messages.14: user messages must have non-empty content`.
- **Root cause (מאומת בקוד):** `app.py`'s `_run_agent_impl()` משבץ מחדש את `user_text` מ-`_rp5_begin_turn()` (התיקון הקודם, BUG-RP5-MARKER-NOT-STRIPPED) אך מעולם לא בדק אם התוצאה ריקה — לפני RP5 זה היה בלתי-אפשרי (טלגרם/וואטסאפ לעולם לא שולחים הודעת-טקסט ריקה), אז לא היה קיים שער כזה בכלל. RP5 יצר תרחיש-קלט חדש שלא היה קיים קודם.
- **תיקון:** בדיקה מיד אחרי `_rp5_begin_turn()`, לפני כל דבר אחר (Pending Approval Gate, confirm-word matching, Router, Agent loop) — אם `user_text.strip()` ריק, מחזיר תשובה מקומית בטוחה **בלי** לקרוא ל-Router/Context/Anthropic/dispatch_tool/ActionGateway בכלל. זהו **אינווריאנט כללי** (Anthropic אסור שיקבל תוכן ריק) לא התנהגות RP5-ספציפית — היה בלתי-נגיש בפרודקשן, RP5 רק חשף אותו. לוג מובחן: `[RP5FaultInjection] marker_only_empty_after_strip` כשה-marker הגולמי הוא שהתרוקן, לעומת `[Agent] empty_user_text_after_processing` הגנרי (לא צפוי לקרות בפרודקשן). `EvidenceFinalizerShadow` עדיין נקרא במפורש (`TurnEvidenceSummary()` ריק → `no_evidence`, טקסט ניטרלי → `neutral`, אין mismatch) כדי שדגימות RP5 לא יעלמו לגמרי עבור ה-turn shape הזה.
