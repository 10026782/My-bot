...
## BUG-127C — A32 Single-Speaker מדכא תוצאת קריאה אמיתית ומאומתת (verified_read_only) ומחליף אותה בהודעת כישלון גנרית — 🔴 נחקר עד הסוף, נמצא Contract Chain מלא, **אין תיקון בטוח וצר** (נסיון תיקון בוצע ונדחה — ראה למטה)
...
- **מסקנה:** אותה בעיית-שורש בדיוק כמו BUG-127B — הבחנה בין "תיאור אמיתי של נתונים קיימים" לבין "טענת-פעולה מומצאת" היא בעיית הבנת-שפה סמנטית, לא בעיה מבנית שregex/evidence-presence יכולים לפתור בבטחה. פתרון אמיתי דורש כנראה שינוי בניסוח ה-system prompt (למשל: להנחות את המודל לעולם לא להשתמש בפועלי-השלמה פסיביים כשמתאר רשומות קיימות מ-read, גם כשמדויק) — לא תיקון קוד דטרמיניסטי בצד ה-sanitizer, בלי לפגוע בהגנה הקיימת.
- **סטטוס:** 🔴 נחקר עד הסוף (Contract Chain מלא + ניסיון תיקון + דחייה מנומקת מול precedent אמיתי) — **לא תוקן, ואין כרגע כיוון-תיקון צר ובטוח מזוהה**. אין שינוי קוד בפועל (הניסיון הוחזר במלואו). ממתין להחלטת המשתמש: לקבל את זה כמגבלה ידועה, לחקור כיוון system-prompt, או משהו אחר.
