...
- **תיקון (ממוקד רק בכשל שדווח, לא הרחבה כללית):** `_is_stopword(word)` חדשה (`ingress_classifier.py:243-250`) — בודקת גם התאמה מדויקת (כמו קודם) וגם: אם המילה מתחילה באחת מ-7 מילות-היחס/חיבור החד-אותיות העבריות (ב/ל/כ/מ/ש/ו/ה) **וגם** השארית (בלי האות הראשונה) נמצאת ב-`_NAME_STOP` בעצמה. פונקציה זו מחליפה את הבדיקה הישירה `w in _NAME_STOP` בלולאת הפיצול-לסגמנטים ב-`_extract_name_from_window()` — **שינוי יחיד, ממוקד**. אין שינוי בחלון ±80-התווים-סביב-הטלפון, ב-neighbor-phone clipping, או ב-`_BLOCK_SEP`.
- **מקרה קצה שנבדק ואומת לא-נפגע:** שם אמיתי המתחיל באחת מ-7 האותיות (למשל "בר"/"הדס") **לא** נפסל — הבדיקה נכשלת (לא stop-word) אלא אם השארית (בלי האות הראשונה) שווה בדיוק למילה שכבר קיימת ב-`_NAME_STOP` — אין סיכון סביר להתנגשות עם שמות אמיתיים לפי בדיקה ישירה מול הרשימה הקיימת (בעיקר שמות ערים/פעלים/מילות-קישור, לא הברות קצרות).
- **בדיקה:** `test_bug099b1_no_name_validation.py` (חדש, 12/12) — שחזור מדויק של מקרה 4 (0 candidates, לא "בקומה"), `classify_ingress` יורד ל-Tier 5, 4 בדיקות ישירות על `_is_stopword()` (כולל regression על מילים לא-מקודמות ועל שמות אמיתיים), ורגרסיה מלאה על 4 המקרים האחרים מאותו סבב בדיקות (3 single-lead + batch) + שני תרחישי BUG-099b המקוריים (תיאור לפני/אחרי הטלפון). אפס רגרסיה: `test_bug096` (29/29), `test_bug098` (16/16), `test_bug099a` (9/9), `test_bug099b_name_window_segmentation.py` (14/14), `test_bug101` (19/19), `core/router/test_router.py` (44/44), `smoke_tests.py`, כל שאר `test_*.py` בריפו.
- **PR:** לא נפתח עדיין.
- **Merged:** לא.
- **סטטוס:** ✅ תוקן בקוד, בדיקות עברו — ממתין ל-push/PR+production verification.
