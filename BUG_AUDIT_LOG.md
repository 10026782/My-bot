...
- **תיקון:** לא בוצע — לפי בקשה מפורשת, וריפיקציה בלבד ללא implementation.
- **בדיקה:** אין עדיין. וריפיקציה בוצעה ידנית (קריאת קוד + grep, ללא הרצת טסטים חדשים) — ראה שורות המצוינות מעלה לכל ממצא.
- **PR:** אין עדיין
- **Merged:** לא רלוונטי (לא תוקן)
- **Deployed:** לא רלוונטי
- **Verified בפרודקשן:** לא רלוונטי
- **סטטוס:** 🟡 פתוח — שורש מאומת בקוד ל-3 מנגנונים נפרדים; ממתין להחלטת המשתמש על סדר/scope תיקון (סביר שזה 2-3 תיקונים נפרדים, לא אחד: (א) parsing למנגנון הקיים ב-ActionGateway, (ב) reject-by-index חדש ב-ActionGateway, (ג) בנייה מאפס של multi-item support ב-`_pending_approvals` וב-`daily_collector`, או הסרת ההבטחה הטקסטואלית שם).
