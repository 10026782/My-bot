...
- **PR:** לא נפתח עדיין.
- **Merged:** לא.
- **סטטוס:** 🟡 **שורש מאומת (שני מנגנונים + DeterministicDenial מוסבר) — קוד לא שונה עדיין.** תוכנית מימוש מפוצלת (לא כגוש אחד) נרשמת ב-§3.5: (1) הרחבת `_NAME_STOP` בעברית עם אוצר-מילים של תיאורי-נכס — **אומת בנפרד שזה כבר מספיק** כדי לדחות את שני מקרי ה-garbage-name שנצפו, דרך מנגנון הדחייה הקיים כבר ב-`_extract_name_from_window()` (`if any(w in _NAME_STOP for w in name.split()): continue`) — לא נדרש ארכיטקטורה חדשה לזה; (2) הרחבת חיפוש השם מעבר לחלון ±80-התווים-סביב-הטלפון (למצוא את "יעל רייס" בפועל, לא רק לדחות תיאור-נכס) — שינוי גדול וסיכוני-רגרסיה יותר, חולק תשתית עם לוגיקת ה-batch-extraction שכבר תוקנה בזהירות ב-BUG-096/097, דורש Contract Chain נפרד; (3) fallback form כש-LCH לא מצליח לחלץ אבל ה-Router בטוח שזו כוונת create_lead — לא reuse של `core/lead_buffer.py` (מחובר לזרימת `capture_inbound_lead` החיצונית, לא ל-LCH כלל, אומת בקוד).