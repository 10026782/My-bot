...
- **PR:** לא נפתח עדיין.
- **Merged:** לא.
- **סטטוס:** 🔴 **שורש מאומת במלואו — hotfix דחוף טרם ממומש.** ה-hotfix עצמו (word-boundary matching + TTL/consumed flag ל-`last_lead_candidate_batch`) רשום ב-§3.5 כ-item נפרד מ-3 ה-follow-up items (broadening trigger, escalation-to-agent path, session non-determinism/correlation ID) — הפרדה מכוונת כדי שלא לערבב hotfix דחוף עם שינויים רחבים יותר שדורשים Contract Chain משלהם.
