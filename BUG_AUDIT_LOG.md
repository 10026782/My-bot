...
- **תופעת-לוואי חשובה שיש להביא לתשומת לב הבעלים (לא הוחלט כאן, נדרשת החלטה מוצרית):** אחרי התיקון, זרימת ה-Tier-1 lead-preview (`_propose_lead_write`) **לא נותנת** דרך ל-manager/partner/employee לאשר בעצמם כתיבת/עדכון ליד דרך "כן" — רק owner יכול. אין כרגע מנגנון להודיע ל-owner על contract ממתין מהזרימה הזו (בניגוד ל-`_queue_approval` הרגיל שכן שולח כפתור לowner) — כך שכתיבת ליד ע"י staff שאינו owner תישאר "pending" עד תפוגה (TTL) בלי דרך לסיים אותה. מומלץ batch נפרד: להוסיף owner-notification ל-`_propose_lead_write` (לחקות את `_queue_approval`), או לקבל החלטה מוצרית מפורשת שרק owner מאשר כתיבות ליד.
