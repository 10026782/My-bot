...
- **כיסוי בפועל:** החלופה `r"\+?972[-\s]?\d{2}[-\s]?\d{3}[-\s]?\d{4}"` תופסת את `+972 54-211-6211`; `_normalize_phone()` מחזירה `0542116211`. אין צורך בשינוי קוד נוסף.
...
- **Evidence שנשמר:** בדיקה ממוקדת — 3/3; `test_bug101_whatsapp_export_bleed.py` — 19/19; `test_bug111_lead_domain_and_sender_prefix.py` — 50/50; BUG-096 — 29/29; BUG-094 — 31/31; BUG-099a — 9/9; BUG-099b — 14/14; BUG-099b.1 — 20/20; BUG-111 follow-up — 29/29; מטריצת פורמטים — 7 מתקבלים ו-4 נדחים; `py_compile` עבר. בחלק מה-suites הודפסו שגיאות DNS צפויות ל-Airtable, אך ה-suites הסתיימו בהצלחה והבדיקות הממוקדות היו מקומיות.
