...
- **תוקן (09/07/2026):** אופציה (ג) יושמה בפועל — `inbound_handler.py`'s `_update_existing()` הנוכחי כותב רק `SUMMARY`+`EXTERNAL_ID`, בלי `UPDATED_AT` בכלל (מאומת בקריאת קוד ישירה, 09/07/2026). **שקיפות:** `git log --all -S "LeadFields.UPDATED_AT" -- inbound_handler.py` לא מחזיר שום commit — אי אפשר לצטט commit ספציפי לחלק הזה, רק לאשר שהקוד הנוכחי נקי. PR #283 (`ba5ad6d`, merge `d3d0fc5`) תיקן בנוסף allowlist ישן ב-`tools/airtable_tools.py::_TABLE_FIELDS` שעדיין הכיל `"created_at"`/`"updated_at"` כשדות מותרים ל-Leads.
