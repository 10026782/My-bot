...
- **Deployed:** לא אומת ישירות מול Render dashboard.
- **Verified בפרודקשן:** לא עדיין — `LEAD_TO_DEAL` כבוי כברירת מחדל; הפעלה + `/dealfromlead <שם/טלפון קיים>` היא הקנרית הבאה (יש לאמת: עסקה אחת נוצרת עם Origin Lead מאוכלס ב-Airtable, ו-agent_calls=0 בלוג).
- **סטטוס:** ✅ CODE MERGED TO MAIN (מאומת ב-grep מול origin/main) — לא deployed/runtime-verified באופן עצמאי.
