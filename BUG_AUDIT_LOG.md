...
## BUG-130 — עדכון-שדה לליד קיים ("תעדכן את הטלפון של X") מנותב כיצירת ליד חדש במקום עדכון הקיים — ✅ תוקן
...
- **תוקן (שני שינויים עצמאיים, ממוקדים):**
  1. `core/router/intent_router.py` — כלל UPDATE_LEAD **חדש** (בנוסף לכלל הקיים, לא במקומו): `(עדכן|שנה).*(טלפון|מספר|פלאפון|נייד).*של` — פועל-עדכון + מילת-טלפון + "של" (מעגן את השדה ל**אדם**, לא כל משפט שמכיל פועל-עדכון ומילת-טלפון בלי הקשר). לא דורש יותר את המילה המילולית "ליד"/"lead".
  2. `core/lead_candidate_handler.py` — `intent` (route.intent, לא זוהה מחדש) עובר עכשיו גם ל-`_handle_single_candidate()` (Tier-1 dispatch שקודם השמיט אותו לגמרי) וגם ל-`_propose_lead_write()` (הקריאה שבאמת מחליטה `airtable_update`/`airtable_add` על ה-ActionContract שנשלח בפועל). פונקציה **חדשה**, `_at_find_lead_by_name_only(name)`, מתפקדת כ-fallback שנבדק **רק** כש-`intent == Intent.UPDATE_LEAD` וגם `_at_find_lead()` (ההתאמה-המדויקת הקיימת) לא מצאה כלום — ומחזירה record רק על התאמת-שם **חד-משמעית** (בדיוק 1 רשומה; 0 או 2+ = None, לא מנחשת). הנתיב הרגיל (בלי intent מאושר) **אף פעם לא** קורא ל-fallback הזה — הגנת BUG-094 (טלפון-מדויק, מניעת זיהום-חוצה-לידים) נשארת שלמה במלואה בנתיב הרגיל. גם עדכון ליד קיים ממשיך לעבור אישור מפורש תמיד (ללא שינוי בהתנהגות הזו).
- **Out of scope:** אין UI-לפתרון-אי-בהירות למקרה של 2+ התאמות-שם (עדיין fallback ל"לא מוצא, מתייחס כחדש" — בטוח אך לא-אופטימלי). אין שינוי ל-Tier 2/3 (batch) lookup, ואין שינוי ל-`_write_one_lead()`'s own (נפרד, auto-write-בלבד) `_at_find_lead()` call.
- **בדיקות:** `test_bug130_update_lead_name_lookup.py` (19 assertions חדש) — router classification (4 repro-texts + 3 regressions), `_at_find_lead_by_name_only()` ישיר (1/0/2+ matches), end-to-end דרך `handle_lead_candidate()`: intent מאושר פותר ליד קיים נכון (T8-T11), regression שבלי intent מאושר ה-fallback **אף פעם לא נקרא** (T12-T13), regression שהתאמה-מדויקת-קיימת "מנצחת" ולא נוגעת ב-fallback (T14-T15), regression שהתאמה-מעורפלת (2+) לא מנחשת (T16). Full sweep (`test_*.py` — 167 קבצים, `smoke_tests.py`, `test_integration.py`, `core/router/test_router.py`, `python3 -m compileall -q .`) — נקי. `test_capture_router_wiring.py`'s 2 קבועי-mock עודכנו (`_fake_single` קיבל `intent=""`) כדי להתאים לחתימת `_handle_single_candidate()` המורחבת — אין שינוי בבדיקות/assertions עצמם. שני כשלים קיימים-מראש (`test_bug_canonical_tool_wiring.py`, `test_pa01_phantom_approval_enforcement.py`) — לא רגרסיה.
- **Verified בפרודקשן:** 🟡 לא עדיין — קוד תוקן ונבדק מקומית (unit-level, full sweep). אימות מול production/RP5 בפועל ממתין להרצת הבעלים אחרי merge+deploy.
- **Merged:** ⏳ טרם — branch `claude/zihuiti-name-extraction-22qitu`.
