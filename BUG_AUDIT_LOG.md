...
- **Root cause:** `tma_api.owner_my_work()` קורא `_at_list(Tables.TASKS, max_records=100)` בלי formula, בלי `paginate` ובלי `strict`. הקריאה לא מסננת רשומות `בוצע` (הסינון קורה רק ב-Python, ב-`_process_owner_tasks`), ולכן כל רשומה בטבלה, כולל שבוצעו, תופסת מקום בתקרה. מעבר ל-100 רשומות, משימות פעילות נחתכות בשקט מ-"עבודה שלי", בלי אזהרה. בנוסף, `strict=False` מחזיר `[]` בכל שגיאת Airtable, ולכן ענף ה-`except` (HTTP 500) לא נגיש, ותקלת קריאה מוצגת כ-"אין משימות" (200).
