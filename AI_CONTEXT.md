...
<<<<<<< HEAD
- **BUG-141 (AG-01, 24/07/2026) — ✅ תוקן, ממוזג ל-main (PR #457, `9d156d9`), טרם אומת ב-production/staging בפועל.** שאלת pending-queue טבעית עם `"?"` נסוף ("מה ממתין כרגע לאישור?") נפלה ל-Agent במקום להיות מנותבת ל-`ActionGateway.describe_pending_queue()` — root cause: branch-ordering ב-`run_agent()` (`if "?" in _stripped:` נבדק לפני `_PENDING_QUERY_RE`). תוקן ע"י בדיקת ה-regex לפני ה-branch הכללי; 15 בדיקות חדשות (`test_bug141_pending_query_dispatch_order.py`), full sweep (166/169, 3 כשלים קדם-קיימים לא-קשורים) ירוק. **טרם ✅ VERIFIED IN PROD** — לפי "כלל ברזל", דורש אימות runtime בפועל אחרי deploy.
- **BUG-142/143/144/145 (24/07/2026) — נרשמו ל-`BUG_AUDIT_LOG.md` (PR #456, `585a6f6`), לא תוקנו עדיין.** Sessions stale linked-lead sync (BUG-142), `resolve_canonical_tool()` payload mismatch (BUG-143, CB-02A), reject callback לא סוגר `ActionContracts.status` (BUG-144, CB-02B), הודעות כפולות ב-approve/reject (BUG-145, CB-01+CB-02B). BUG-146 מוזג לתוך BUG-122 (ראיות CB-01/CB-02 מצטברות).
- **ממצא תכנוני נפרד, ללא מספר BUG (24/07/2026):** "Cost Telemetry Coverage and Per-Turn Attribution" — `cost_monitor.record_call()`'s call site היחיד (`app.py:3174`, בתוך `run_agent()`) חושף ש-Daily Collector/`interaction_engine`'s עלויות LLM לעולם לא מגיעות ל-live alerting accumulator, רק ל-`usage_events` ה-shadow-only. הצעת fix shape בלבד, אין קוד.
- **RP5 staging fault-injection helper — קוד + אינטגרציה, branch זה (`claude/rp5-staging-fault-injection-v4akit`, PR #407), staging-only, ✅ VERIFIED ב-staging (19/07/2026), הקוד עצמו לא ממוזג ולא יתמזג ל-main.** `core/rp5_fault_injection.py` + חיווט צר ב-`tools/dispatcher.py`/`app.py`. **לא נוגע ב-production בשום צורה** — ארבעה שערי בטיחות (APP_ENV, flag, allowlist, marker מפורש בהודעה) כולם חייבים להתקיים; ברירת מחדל בקוד `RP5_FAULT_INJECTION_ENABLED=false`. ענף זה עבר rebase על `main` (כולל PR #456/#457, BUG-141 fix) והועלה מחדש (force-push) ב-24/07/2026 — staging מריץ עכשיו את התיקון.
=======
