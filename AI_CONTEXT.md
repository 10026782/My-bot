...
8. **BUG-114 — מבוקר, ממתין להחלטת owner** — ה-burst שנצפה 19/07/2026 אובחן בנפרד: `ExecutionLedger.mark_context_interrupted()` (`core/action_gateway.py:558`) מסמן מחדש כל contract "pending" כולל כאלה שכבר `context_interrupted=True`, מפיק 3 קריאות Airtable מיותרות per-contract על כל הודעה בלתי-קשורה. תיקון צר מוצע (סינון RAM טהור) ב-`docs/architecture/action-gateway/BUG-114_CONTEXT_INTERRUPT_CALL_AMPLIFICATION_AUDIT.md` — לא ממומש עדיין. שתי המלצות נוספות (reuse PATCH response; מדיניות TTL ל-contracts ישנים — ראו גם שאלה 6 באותו audit, קשורה ל-"למה 6 contracts חיים בו-זמנית") ממתינות להחלטה נפרדת.
