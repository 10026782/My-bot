...
**עודכן:** 2026-07-19 · **main:** `8bdd504` (אחרי מיזוג PR #400, "Add TurnOwnershipShadow invariant") · **סטטוס:** אין ענף פעיל פתוח כרגע (כל PRs עד #400 ממוזגים ב-`main` — מאומת ב-git log + git cat-file). **PR #397 (branch `claude/docs-sync-bug-audit-roadmap-drift`) נסגר במפורש כ-superseded ע"י #398, לא מוזג במכוון.**
...
9. **BUG-115 (חדש) — 🔴 מבוקר, לא תוקן.** אותה דגימת production ("צור ליד חדש... כן") חשפה ממצא **נפרד** מ-BUG-114 (לפי בקשה מפורשת לא לערבב, למרות שורש-תרומה משותף): תצוגת "📋 זיהיתי ליד..." יוצרת contract Tier-1 אמיתי (עיצוב BUG-056 מכוון), אבל "כן" שאחריה נותב ל-disambiguation גנרי של 8 contracts ישנים ולא-קשורים (חלקם מדגימות BUG-114/#393 קודמות) במקום לאשר את הליד שהוצג הרגע — `route_confirmation_word()` (`core/action_gateway.py:1010`) סופר contracts חיים בלבד, ללא זיהוי "לאיזה contract ה-'כן' מתייחס". גם חושף tool_name/id גולמיים למשתמש. `TurnEnvelope.active_queue_id` נבדק ונשלל כגורם בפועל (תצפית-בלבד, Phase 0, לא נצרך בניתוב האמיתי). תיקון מוצע (לא ממומש) ב-`docs/architecture/action-gateway/BUG-115_CONFIRMATION_ROUTING_HIJACK_AUDIT.md`.
