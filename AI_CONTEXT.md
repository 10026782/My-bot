...
- **RP5 staging fault-injection helper — קוד + אינטגרציה, branch זה (`claude/rp5-staging-fault-injection-v4akit`), staging-only, ✅ VERIFIED ב-staging, הקוד עצמו לא ממוזג ולא יתמזג ל-main.** `core/rp5_fault_injection.py` + חיווט צר ב-`tools/dispatcher.py`/`app.py`. **לא נוגע ב-production בשום צורה** — ארבעה שערי בטיחות (APP_ENV, flag, allowlist, marker מפורש בהודעה) כולם חייבים להתקיים; ברירת מחדל בקוד `RP5_FAULT_INJECTION_ENABLED=false`. **26/07/2026:** הענף אותחל מחדש (`git checkout -B` על `origin/main`, כולל PR #460/#461/#467/#469 — BUG-143/144/145/147 כולם) ורק commit-ים ייחודיים ל-RP5 (מנגנון ה-fault-injection עצמו, marker-stripping, BUG-120) שוחזרו מעליו — commit-ה-PM460 העצמאי של הענף (`a31d3c9`/`cf890ad`) **לא** שוחזר, כי ההתנהגות שלו כבר קיימת ב-`main` דרך PR #460/#461 (מאומת בסבב audit קודם: זהה מבחינת runtime behavior, לא רק commit ancestry) — הכרעה לפי התנהגות-בפועל, לא לפי שרשרת commit-ים, כדי למנוע מקור-אמת מקביל.
