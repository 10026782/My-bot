...
**עודכן:** 2026-07-08 · **main:** `eaa01fa` (PR #263) · **סטטוס:** 🟡 CODE DONE, לא מאומת בפרודקשן
...
- הסבב האחרון (07-08/07) סגר שרשרת של 6+ תיקוני `/update`/Business Memory (BUG-078..081, C97-C101) — **כולם ממוזגים ל-main, אף אחד לא מאומת בפרודקשן עדיין.**
- PR #263 (החדש ביותר) סגר את הסאגה הזו לגמרי: `domain` כבר לא נכתב ל-`Tags` בשום נתיב (רק לשדה `Domain` הייעודי), תוקן מיפוי `media`→Title Case, ותוקנה קריאת ההקשר (`get_recent_business_context`) לסנן לפי `Domain`.
...
שרשרת תיקוני `/update` + Business Memory (07-08/07/2026, PRs #255-#263, כולם ✅ merged, ❌ לא deployed/verified):
...
3. **Production verification** של כל שרשרת BUG-078..081/C97-C99 (סעיף 3 למעלה) — merged אך אפס אימות live ב-Telegram/WhatsApp אמיתיים + בדיקת commit hash מול Render.
