...
**תוספת (26/07/2026, C173):** **BUG-143/144/145 כולם ✅ תוקנו ב-`main` בפועל** — לא ע"י הענף הזה, אלא ע"י שני PRs מקבילים (`codex/`) שמוזגו ל-`main` בין הכתיבה של C172 לעכשיו: **PR #460** (`006506d`) סגר BUG-144 (reject קורא ל-`ActionGateway.reject()`) ו-BUG-145 (הודעה-סופית-יחידה, גם ב-approve וגם ב-reject); **PR #461** (`70093f0`) סגר BUG-143 עם המרת Sheets→Airtable אמיתית (`_sheets_payload_to_airtable()`, field-allowlist + fail-closed). `BUG_AUDIT_LOG.md`/`CHANGE_CONTROL_LOG.md` (C173) עודכנו בהתאם — שני ה-PRs עצמם לא נגעו בקבצי governance. **BUG-147 תוקן בסבב הזה בפועל (Patch A, `tools/dispatcher.py` בלבד)** — וה-root cause שנרשם ב-C172 עבורו התברר כשגוי; השורש האמיתי (ותיקונו) מתועד ב-BUG_AUDIT_LOG.md's BUG-147. **הענף `claude/telegram-task-approval-audit-il29sj` אותחל מחדש מ-`origin/main`** (הקומיט הקודם שלו כבר מוזג דרך PR #467) לפני שהמשך העבודה בוצע — ראו `docs/architecture/action-gateway/PM460_POSTMERGE_MINIMAL_PATCH_GATE.md` להיקף המדויק (Patch A בלבד; Patch B/C המתוכננים במקור ירדו כי היו מיותרים מול PR #460/#461).
