...
**TMA frontend fix (PR #436, `claude/n15-owner-decision-p73c3k`, טרם ממוזג):** `SystemHealth.tsx` — כפתור Stop AI, כפתור Clear לכל דגל פעיל (קורא `operation_id` מ-`GET /api/health`, שולח `expected_operation_id`), טיפול ב-409 (רענון + הודעה), הסרת הטקסט השגוי "לביטול: הפעל מחדש את השרת" (restart לא מבטל דגל durable). אומת בדפדפן אמיתי (Playwright/Chromium מול backend מקומי מזויף, ללא נגיעה בפרודקשן) — לא רק `tsc`/build.
