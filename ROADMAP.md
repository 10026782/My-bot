...
| MEDIA_LAYER_F16 | Media Layer (F16) | IN_PROGRESS | MERGED_STATIC — PARTIALLY REMEDIATED / NOT STATICALLY CLOSED — F16-M1 (wrong Meta media host) and F16-M4 (stale/non-hermetic self-test) statically verified fixed (Remediation Slice 1, 30/08/2026); F16-M2/M3 remain OPEN / OWNER DECISION REQUIRED, F16-M5/M6/M7 remain open low-risk cleanup; full record in `BUG_AUDIT_LOG.md` | Owner decisions on F16-M2 (MIME policy) and F16-M3 (durability/orphan-recovery policy), then low-risk F16-M5/M6/M7 cleanup — only then the gated deployed-SHA canary | [`HORIZON.md`](docs/governance/HORIZON.md) |
...
| DECISION_HUB | Decision Hub | IN_PROGRESS | ACTIVE / STATIC AUDIT COMPLETE / REMEDIATION REQUIRED / RUNTIME NOT ESTABLISHED — DH-S1 formula safety CLOSED / STATIC VERIFIED; DH-S2 access-policy wording DOC/POLICY DRIFT / REMEDIATION REQUIRED; DH-S3 fail-closed reads STATIC VERIFIED; DH-S4 partial write observability OPEN | Align shared-capability policy/code/tests around permission, tenant/data scope and action authorization; remediate partial-persistence observability | [`HORIZON.md`](docs/governance/HORIZON.md) |
...
| SCHEMA_DATA_CONTRACTS | Schema / Data Contracts | IN_PROGRESS | MERGED_STATIC — live verification pending; corrected a stale Leads tier-field comment in tma_api.py against live Airtable schema (30/08/2026); added the Payment Terms table plus Deals/Payments schema fields in live Airtable and the corresponding airtable_schema.py constants, with commercial_crm.py (create_deal/create_payment/create_payment_term writers + calculate_payment() calculation contract, 54/54 tests) as a not-yet-wired foundation slice (30/08/2026). Track 8 static reconciliation complete (30/08/2026, see `MAINTENANCE_AUDIT_LEDGER.md` §"Track 8"): no owned MEDIUM/HIGH static gap remains; commercial_crm.py's writer field-name + linked-record contract statically verified against schema_cache.json, now machine-checked (97/97 tests, +43 snapshot-fidelity checks); live schema verification still required | Perform live schema/contract verification (full schema_audit.py live refresh — schema_cache.json currently covers only 19/29 tables in schema_audit.py's own TABLE_CLASS_MAP); register the 3 new Deal/Payment tools in tool_registry.py/tools/dispatcher.py/tools/schemas.py; two LOW-risk dead/stale-code findings from Track 8 (crm.py's unreachable crm_add_payment() referencing a phantom PaymentFields.CONTACT field; tools/airtable_tools.py's dead _TABLE_FIELDS Payments mapping holding the wrong table's fields) are reported but NOT fixed — separate, explicitly-approved cleanup slice only | [`MAINTENANCE_AUDIT_LEDGER.md`](docs/governance/MAINTENANCE_AUDIT_LEDGER.md) |
