...
עודכן: 03/07/2026 — BUG-056 (C89-ROUTER-PREVIEW-HARDENING): 4 ממצאי QA ידני על C89/BUG-IC-01 — BUG-IC-01 regex coverage gap ("בדיקת מערכת" יחיד לא נתפס), C89 preview confirmation dead-end (pending_lead_preview נשמר אך לא נקרא בחזרה, "כן" מחזיר "אין פעולה שממתינה"), C89 Tier 4 לא עוצר routing (טקסט מודבק/table/bot-output ממשיך ל-Agent, מפעיל intent שגוי), double-classification cleanup (ingress_classifier רץ פעמיים). תיקון ב-`branch fix/c89-router-preview-hardening`. ראה BUG_AUDIT_LOG.md BUG-056.
