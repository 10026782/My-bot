...
| MEDIA_LAYER_F16 | Media Layer (F16) | IN_PROGRESS | RUNTIME_VERIFIED for the deployed F16 scope: image/video Drive + Airtable persistence, voice transcription, approval, and approved Business Memory write verified 03/09/2026 on deployed SHA `0f80122525c5bbc9e3b115a4f7c4e131ac47070b`; M5 (content-hash architecture for non-TMA sources) is DEFERRED / ACCEPTED by owner decision; Telegram `video_note`/`audio` routing and saving a voice recording as a file remain unbuilt extensions, not current F16 requirements; Mini App and other unrelated surfaces remain separately deferred | Monitor the verified scope; only owner-approved extensions require new implementation and canary | [`HORIZON.md`](docs/governance/HORIZON.md) |
