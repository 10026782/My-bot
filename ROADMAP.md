...
| MEDIA_LAYER_F16 | Media Layer (F16) | IN_PROGRESS | MERGED_STATIC — PARTIALLY REMEDIATED / NOT STATICALLY CLOSED — F16-M1/M4 statically verified fixed (Remediation Slice 1, 30/08/2026); F16-M2 OWNER DECISION COMPLETE / REMEDIATION REQUIRED (canonical MIME allowlist, fail-closed); F16-M3 OWNER DECISION COMPLETE / REMEDIATION REQUIRED (durable failure trace without a reservation id); neither decision implemented yet; F16-M5/M6/M7 remain open low-risk cleanup; full record in `BUG_AUDIT_LOG.md` | Implement the recorded MIME-allowlist (M2) and durable-failure-trace (M3) remediation as the next Media slice, then low-risk cleanup — only then the gated deployed-SHA canary | [`HORIZON.md`](docs/governance/HORIZON.md) |
...
| DECISION_HUB | Decision Hub | IN_PROGRESS | ACTIVE / STATIC AUDIT COMPLETE / REMEDIATION REQUIRED / RUNTIME NOT ESTABLISHED — DH-S1 formula safety CLOSED / STATIC VERIFIED; DH-S2 access-policy wording DOC/POLICY DRIFT / REMEDIATION REQUIRED; DH-S3 fail-closed reads STATIC VERIFIED; DH-S4 partial write observability OPEN; focused authorization/data-scope tests added and passing | Remediate partial-persistence observability; complete broader callback/record-scope review; then pursue runtime evidence without broadening permissions | [`HORIZON.md`](docs/governance/HORIZON.md) |
