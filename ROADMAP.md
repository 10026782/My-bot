...
עודכן: 19/07/2026 — **C84 (TMA Approvals TTL) מיושם, tests ירוקים, טרם ממוזג ל-main (branch `claude/rp5-staging-fault-injection-v4akit`):** `tma_api.py`'s `_claim_and_execute_approval()` — הנקודה היחידה ש-`act_on_approval()` וגם `bulk_approve()` עוברים דרכה לביצוע — אוכפת עכשיו freshness check לפני `ActionGateway.approve()`: contract עם `created_at` ישן מ-`_TMA_APPROVAL_TTL_SECONDS` (24 שעות) נדחה (`reject()`, מאומת, לא מונח כמובן מאליו — אותו עיקרון כמו `_reject_stale_telegram_approval()` ב-BUG-112) במקום להתבצע, ומחזיר `ok=False status_code=410`. **סטיה מכוונת מהצעת ה-ROADMAP המקורית** (`expires_at` שדה חדש על רשומת Approvals): נעשה שימוש ב-`ActionContract.created_at` הקיים במקום — Approvals מתועד כ-"non-authoritative TMA display projection" (`ApprovalsFields` docstring), אז ה-contract הקנוני הוא מקור האמת ל"כמה זמן זה באמת", לא שדה חדש שדורש יצירה ידנית ב-Airtable + resync של `schema_cache.json`. 20 בדיקות חדשות (`test_c84_tma_approval_ttl.py`) + 3 קבצי test קיימים עודכנו (`test_approval_concurrency.py`/`test_phase_4b2_wiring.py`/`test_pr0c0_tma_approval_truthfulness.py`) כדי ש-`_FakeContract`'s test doubles יכללו `created_at` (ברירת מחדל "עכשיו" — משמר את כל ההתנהגות הקיימת ללא שינוי). Full sweep (143 קבצי `test_*.py`) + smoke + compileall נקיים. ראה `CHANGE_CONTROL_LOG.md` C144.
...
### C84 — ✅ קוד הושלם, tests ירוקים, טרם ממוזג ל-main — TMA Approvals: TTL + freshness check
**עדיפות:** 🟡 גבוה
**בעיה:** רשומת `PENDING` יכולה להישאר פעילה ללא הגבלת זמן.
**פעולה (כפי שיושמה בפועל):** לא `expires_at` חדש — `_claim_and_execute_approval()` ב-`tma_api.py` בודקת `time.time() - ActionContract.created_at` (השדה הקנוני הקיים) מול `_TMA_APPROVAL_TTL_SECONDS` (24h) לפני `ActionGateway.approve()`; contract שחצה את החלון נדחה (`reject()`, מאומת) והתגובה `ok=False status_code=410`. מכסה גם את `act_on_approval()` וגם `bulk_approve()` (שתיהן עוברות דרך אותה פונקציה יחידה). Approvals projection (STATUS/PROJECTED_LIFECYCLE_STATUS) מסונכרן לדחייה גם כן, כדי שה-TMA UI לא ימשיך להראות "ממתין" שקרי.
**קובץ ראשי:** `tma_api.py` (לא `airtable_gateway` — אין כתיבת שדה סכמה חדשה)
**בדיקות:** `test_c84_tma_approval_ttl.py` (20 checks) + עדכון תואם ל-3 קבצי test קיימים.
**Production verification:** לא עדיין — ממתין למיזוג.
