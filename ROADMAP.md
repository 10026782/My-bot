...
עודכן: 05/07/2026 — C94 audit: אין Feature Flag ל-C94 עצמו (במכוון — equivalence-preserving בכל שלב; סוטה מ-RELEASE_CHECKLIST.md's "flag כבוי כברירת מחדל", מתועד כפער ולא מוסתר), אין Render env vars חדשים (נבדק grep מלא), production verification עדיין לא בוצע (checklist מפורט נוסף). ראה AI_CONTEXT.md §0.12 + סעיף C94 למטה.
...
- **אין Feature Flag ל-C94 עצמו — במכוון.** כל שלב תוכנן להיות equivalence-preserving (טסט ייעודי מוכיח תוצאה זהה עם/בלי ה-wiring, בכל שלב) ולהתדרדר בעדינות בכשל — נחשב "always-on plumbing", לא feature הדורש הפעלה. **זה סוטה מ-`RELEASE_CHECKLIST.md`'s "Feature flag הוגדר וכבוי ברירת מחדל"** — סעיף זה לא קוים באף אחד מ-4 ה-PR (#236-#239), מתועד כאן כפער מודע, לא מוסתר. אם רוצים kill-switch עתידי — צריך להוסיף בנפרד.
...
- **Production verification — עדיין לא בוצע, פתוח בפירוש:** (1) commit hash ב-Render מול `main`; (2) הודעת Telegram אמיתית — תשובה זהה + אין `[C94] telegram envelope build/validate failed` בלוגים; (3) הודעת WhatsApp/Twilio אמיתית — אותו דבר עם `[C94] whatsapp envelope build/validate failed`; (4) קובץ xlsx/csv אמיתי (רלוונטי רק אם `FEATURE_STRUCTURED_FILE_CAPTURE` יופעל); (5) מסלול "חריגת classify_ingress מתדרדרת בעדינות" — **לא כדאי** לבדוק על תעבורה אמיתית (=לשבור prod בכוונה) — ההוכחה נשענת על 138 הבדיקות (`test_c94_*.py`), לא על trigger live.
**ראה:** AI_CONTEXT.md §0.12 לפירוט המלא של האודיט הזה.
