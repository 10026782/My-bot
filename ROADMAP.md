...
עודכן: 05/07/2026 — C90 (Structured File Capture, PR #228): file upload כ-ingress source adapter בלבד — `core/file_ingress_adapter.py` חדש מפרסר xlsx/csv לשורות, כל שורה עוברת ללא special-casing דרך אותו `classify_ingress()`/`handle_lead_candidate()` כמו טקסט (שורה ברורה יכולה להיות Tier1 לגיטימי). הוחלט לבנות למרות ש-C89 עדיין לא production-verified, כי C90 לא תלוי בנתיב auto-write. ראה סעיף C90.
...
### C90 — ✅ קוד הושלם (PR #228) — Stage 3.1: Capture Policy — קבצים מובנים (xlsx/csv)
