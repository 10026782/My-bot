...
עודכן: 08/07/2026 — BUG-078/079/080/081 (שרשרת `/update`+Business Memory, PR #255/#256/#257/#258/#259/#260/#261/#263/#265) **✅ PRODUCTION VERIFIED**: `/update` → `נדל"ן` (real_estate) → `Other` → טקסט חופשי → נשמר בהצלחה, "📌 Other | נדל"ן" מוצג נכון, אין 422. מכסה: BUG-078 (זרימת `/update` הכללית), BUG-079 (`capture_text`), BUG-080 (`cmd_update.py`'s Event Date בלבד — שאר 6 נקודות הכתיבה טרם נבדקו), BUG-081 המלא כולל PR #263 (root cause — domain לא נכתב ל-Tags בכלל) ו-PR #265 (רווח בסוף ב-"Real Estate "/"SaaS ", מאומת מול Meta API). **שאר ה-domains (media/saas/import/finance/general) ו-C99 (חילוץ מסמך) טרם נבדקו.** ראה `BUG_AUDIT_LOG.md` BUG-078..081 ו-`CHANGE_CONTROL_LOG.md` C97-C101 לפירוט מלא.
