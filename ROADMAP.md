...
עודכן: 19/07/2026 — **סנכרון תיעוד גדול (PR #373–#396, 24 PRs) + BUG-111 מלא + BUG-112 מלא + F52 PR1-PR6 + BUG-113 חדש:** מסמך זה פיגר מ-17/07/2026 עד עכשיו — הפער עצמו תועד מראש ב-`AI_CONTEXT.md` ("MAIN > DOCS עד שיתוקן") ונסגר בעדכון הזה, מאומת ב-grep/git log מול `main` לפני כתיבה. **BUG-111 (פענוח batch לידים)** נסגר בשני סבבים (PR #386, #390) — כולל safety-net שמבטל candidate יחיד ל-clarification כשיש יותר מטלפון אחד בטקסט הגולמי; **VERIFIED IN PROD** (production evidence אמיתי). **BUG-112 (Telegram approval TTL)** — המנגנון עצמו (PR #387) **VERIFIED IN PROD** (לחיצה אמיתית על כפתור פג-תוקף הפיקה את ההודעה הצפויה, 0 dispatch חוזר); סבב UX נוסף (PR #394) איחד שלושה ניסוחים חופפים של "stale/missing callback" לביטוי אחד — קוד מוכן, טרם production-verified מחדש אחרי ה-deploy. **F52 Unified Approval Runtime** התקדם מ-PR0 (draft) ל-**PR1–PR6 שלמים**: PR1 Message Contract Foundation (`core/agent_message_formatter.py`, standalone, לא מחובר), PR2 איחוד `compose_status_reply` מאחורי `FEATURE_UNIFIED_STATUS_FORMATTER` (off/shadow/on, **עדיין off**), PR3 אימוץ `display_payload` קנוני, PR4 shadow verification לנתיב executed/status-query, PR5 shadow ל-rejection/cancellation, PR6 shadow ל-approval_pending prompt + תיקון עוקב לטקסונומיית EvidenceFinalizer עבור turns מעורבים (read+pending). **PR6 חשף וסגר 3 פערים אמיתיים** (הודעת gateway לא עברה דרך ה-formatter כלל; דיכוי A32 תקין נקרא כ-false mismatch; ownership signal לא סימן `reply_owner="gateway"`) — **VERIFIED IN PROD**. **BUG-113 (חדש)** — פער נפרד שנחשף מיד אחרי: A32's Single-Speaker gate לא דיכא פרוזת approval-invite של ה-agent ("✅ מוכנה להוספה... שלח מאשר") כשאישור אמיתי כבר בתור, כי ה-pattern הרלוונטי (`_AGENT_APPROVAL_INVITE_PATTERN`) נבדק רק בכיוון ההפוך (ללא ראיה). תוקן ב-PR #396, **VERIFIED IN PROD**. גם הושלמו: BUG-104 Phase 2A.1/2A.2 (יישום מדיניות Business Outcome precedence + ניסוח ספציפי-ל-Lead, שני הדגלים off/shadow), RP5 preflight audit (חוסם — ממתין ל-operator shadow rollout). ראה `AI_CONTEXT.md` §1/§3, `BUG_AUDIT_LOG.md` BUG-111/BUG-112/BUG-113, ו-`CHANGE_CONTROL_LOG.md` C125–C148.
