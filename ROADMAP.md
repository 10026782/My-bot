...
עודכן: 05/07/2026 — C89 סטטוס סגור: ✅ CLOSED/VERIFIED עם `FEATURE_AUTO_CAPTURE=false` (החלטה מפורשת של הבעלים — קוד+טסטים מאומתים מחדש, כולל RAW-OBS 15/15; flag נשאר כבוי בפרוד בכוונה, לא production-verification במובן המקורי). C90 לא נוגע — כבר בנוי ומוזג מקודם (PR #228). ראה סעיף C89 למטה.
...
- **Production verification — ✅ בוצע (2/5) ע"י הבעלים, 05/07/2026:** (1) הודעת Telegram אמיתית — נכנסה תקין, Identity resolved, Router עבד, `classify_ingress` לא הפיל את ה-router, **אין** `[C94] telegram envelope build/validate failed` בלוגים. (2) הודעת WhatsApp/Twilio אמיתית — אותו דבר: נכנסה תקין, Identity resolved, Router עבד, **אין** `[C94] whatsapp envelope build/validate failed`, `MessageSid` אמיתי נראה כ-`raw_ref` (`ref=SM...`). **נותר פתוח:** (3) קובץ xlsx/csv אמיתי (רלוונטי רק אם `FEATURE_STRUCTURED_FILE_CAPTURE` יופעל — עדיין כבוי); (4) commit hash ב-Render מול `main` לא אומת במפורש בסבב הזה (אך ההתנהגות הנצפית תואמת קוד מעודכן); (5) מסלול "חריגת classify_ingress מתדרדרת בעדינות" — עדיין לא נבדק על תעבורה אמיתית בכוונה (=לשבור prod), נשען על 138 הבדיקות.
