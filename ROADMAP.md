...
| MEDIA_LAYER_F16 | Media Layer (F16) | IN_PROGRESS | MERGED_STATIC — mechanical findings CLOSED / M5 OWNER DECISION REQUIRED / RUNTIME NOT ESTABLISHED — M1, M2, M3, M4, M6, M7 all statically verified and merged (Slices 1-3, PRs #1124/#1131/pending); M5 (content-hash architecture for non-TMA sources) remains open, blocked on a separate owner decision, not a mechanical cleanup item; M8 remains cross-track, not owned by this program; full record in `BUG_AUDIT_LOG.md` | Owner decision on the M5 content-hash question, or an explicit decision to defer it; the deployed-SHA Media canary / runtime verification gate is separate and comes after | [`HORIZON.md`](docs/governance/HORIZON.md) |
