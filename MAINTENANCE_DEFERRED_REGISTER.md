...
| R-C10-DG7 | `google-auth-httplib2` / `google-auth-oauthlib` unused direct imports | No direct imports in application code, but transitive dependencies of `google-api-python-client`; removing risks `discovery.build()` failure | Intentional: transitive deps of active dependency; confirmed no direct use | Audit #10 lifecycle verification pass; remove only if `google-api-python-client` is also removed | None (intentional architectural choice) |
| R-C10-DG8 | `psycopg2-binary` conditional activation | Feature-flag gated (`FEATURE_POSTGRES_ENABLED`); conditional import in `core/database.py:37` | Intentional: architectural choice per `BOSS_UNIFIED_MASTER_PLAN`; feature-flag scoped | Audit #10 lifecycle verification pass; activation depends on product decision to enable Postgres | Feature flag activation + Postgres infrastructure |
...
| E-H | Code→docs gaps: media result contract, WhatsApp ACK behavior, last_uploaded_file architecture, GoogleDriveArtifactStore boundary, Media Files table responsibility, session durability semantics, and idempotency overview | **RESOLVED / #20 CLOSED — STATIC VERIFIED** via `docs/architecture/MEDIA_FILE_SESSION_IDEMPOTENCY_CONTRACT.md`; historical snapshots and the absent `SPEC_File_Context_Reference.md` citation remain non-authoritative evidence | #20 |
